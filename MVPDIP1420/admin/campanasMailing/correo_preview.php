<?php
	...
	if(!empty($_POST)){
		...
		$id_correo_mailing = $_POST['correo_electronico_prueba'][0]['id_correo_mailing'];
		$correo_mailingDatos = correo_mailingDatos($id_correo_mailing);
		...
		$correo_electronico = array(
			"[*__Correo_Electronico_Repuesta__*]" => $correo_mailingDatos['correo_electronico'],
			"[*__Correo_Electronico_Envio__*]" => $correo_mailingDatos['usuario'],
		...
		$seccion_ine_ciudadanoDatos = seccion_ine_ciudadanoPrimeroDatos();
		...
		echo $correoEnvioMailing = correoEnvioMailing($correo_mailingDatos,$bodyHTML,$asuntoHTML,$para,'',$unsubscribe);
	}