<?php
	...
	if(!empty($_POST)){
		...
		$update_secciones_ine_ciudadanos_campanas_sms_programadas = "UPDATE secciones_ine_ciudadanos_campanas_sms_programadas SET status=4 WHERE id <>0 AND id_campana_sms=".$id." AND status = 0 ";
			$conexion->autocommit(FALSE);
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],"secciones_ine_ciudadanos_campanas_sms_programadas",$id,'cancelar_masivos','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
