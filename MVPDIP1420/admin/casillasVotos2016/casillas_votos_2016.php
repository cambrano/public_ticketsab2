<?php
	...
	$sql = "SELECT count(*) total FROM casillas_votos_2016 WHERE 1 = 1   "; 
	...
	$sql = "SELECT * ,
			...
			CASE
				...
				WHEN cv.tipo = '2' THEN 'distrito federal'
				WHEN cv.tipo = '3' THEN 'gobernador'
				ELSE 'senador'
			END as tipo
			FROM casillas_votos_2016 cv WHERE 1 = 1 "; 
	...
	*/
	...
	if($tipo_uso_plataforma=='municipio'){
		...
		$sql.= " AND cv.tipo =3 ";
		$sqlContador.= " AND cv.tipo =3 ";
	}elseif($tipo_uso_plataforma=='senador'){
		$sql.= " AND cv.tipo =4 ";
		$sqlContador.= " AND cv.tipo =4 ";
	}
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM casillas_votos_2016 cv WHERE 1 = 1   "; 
