<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'casillas_votos_2016','casillasVotos2016','index');
	//var_dump($redirectSecurity);
	if($redirectSecurity!=""){
		die;
	}

	$claveF= clave2('casillas_votos_2016');
	$casilla_voto_2016Datos=casilla_voto_2016Datos($id);
	if($casilla_voto_2016Datos['clave']==""){
		$casilla_voto_2016Datos['clave']=$claveF['clave'];
	}

	$tipo = $casilla_voto_2016Datos['tipo'];

	$partidos_2016Datos = partidos_2016Datos('','',$tipo);

	$casillas_votos_partidos_2016Datos = casillas_votos_partidos_2016Datos('',$id,'');

	$votos_validos = 0;
	foreach ($partidos_2016Datos as $key => $value) {
		foreach ($casillas_votos_partidos_2016Datos as $keyT => $valueT) {
			if($value['id']==$valueT['id_partido_2016']){
				$partidos_2016Datos[$key]['votos'] = $valueT['votos'];
	...
	$votos_totales = $votos_validos + $casilla_voto_2016Datos['votos_can_nreg'] + $casilla_voto_2016Datos['votos_nulos'];
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			casilla_voto_2016.push(data);

			///partidos
			var votos_partidos_2016 = [];
			<?php
			foreach ($partidos_2016Datos as $key => $value) {
				?>
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2016/db_edit.php",
				data: {casilla_voto_2016: casilla_voto_2016,votos_partidos_2016:votos_partidos_2016},
				success: function(data) {
			...
			<div class= "bodyheader">
				<label class="tituloForm">
					...
					if($tipo==0){
						...
						?>
						<font style="font-size: 25px;">Gobernador</font>
						<?php
					}else{
						?>
						<font style="font-size: 25px;">Senador</font>
						<?php
					}
				...
				<label class="descripcionForm">
					<font style="font-size: 13px;">Campos para modificar a casilla voto 2016.</font><br><br>
				</label><br>
