...
if($id_municipio!=''){
	...
	$sql = 'SELECT COUNT(*) total FROM casillas_votos_2021 cv WHERE cv.tipo=0 '.$sqlMunicipip;
	...
	$sql = "SELECT 
				...
				END as status_data,
				COUNT(*) registros
			FROM casillas_votos_2021 cv WHERE 1 = 1  AND cv.tipo =  0 $sqlMunicipip
	...
	$sql = 'SELECT COUNT(*) total FROM casillas_votos_2021 cv WHERE cv.tipo=1';
	$resultado = $conexion->query($sql);
	...
	$sql = "SELECT 
				...
				END as status_data,
				COUNT(*) registros
			FROM casillas_votos_2021 cv WHERE 1 = 1  AND cv.tipo =  1 $sqlDLocal
			GROUP BY status_data";
	$result = $conexion->query($sql);  
	while($row=$result->fetch_assoc()){
	...
	$sql = 'SELECT COUNT(*) total FROM casillas_votos_2021 cv WHERE cv.tipo=2 '.$sqlDFederal;
	$resultado = $conexion->query($sql);
	...
	$sql = "SELECT 
				...
				END as status_data,
				COUNT(*) registros
			FROM casillas_votos_2021 cv WHERE 1 = 1  AND cv.tipo =  2 $sqlDFederal
			GROUP BY status_data";
	$result = $conexion->query($sql);  
	while($row=$result->fetch_assoc()){
