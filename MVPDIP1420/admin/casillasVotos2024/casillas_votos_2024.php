<?php
	...
	$sql = "SELECT count(*) total FROM casillas_votos_2024 WHERE 1 = 1   "; 
	if($tipo_uso_plataforma=='municipio'){
		$sql.= " AND id_municipio ='{$id_municipio}' AND tipo=0 ";
	}elseif($tipo_uso_plataforma=='distrito_local'){
		$sql.= " AND id_distrito_local ='{$id_distrito_local}' AND tipo=1 ";
	}elseif($tipo_uso_plataforma=='distrito_federal'){
		$sql.= " AND id_distrito_federal ='{$id_distrito_federal}' AND tipo=2 ";
	}
	...
	$sql = "SELECT * ,
			...
			CASE
				WHEN cv.tipo = '0' THEN 'municipio'
				WHEN cv.tipo = '1' THEN 'distrito local'
				ELSE 'distrito federal'
			END as tipo,
			CASE
				WHEN (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = 1 THEN 'Abierto'
				WHEN (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = 2 THEN 'Cerrado Con Gente'
				WHEN (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = 3 THEN 'Cerrado'
				WHEN (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = 4 THEN 'Inicio Conteo'
				ELSE 'Sin Estatus'
			END as status_data,
			( SELECT CONCAT(ch.hora,' ',FORMAT(ch.distancia_m,2),'m') FROM casillas_votos_2024_checks ch WHERE ch.id_seccion_ine = cv.id_seccion_ine ORDER BY fecha_hora DESC LIMIT 1 ) check_in,
			( SELECT ch.alerta FROM casillas_votos_2024_checks ch WHERE ch.id_seccion_ine = cv.id_seccion_ine ORDER BY fecha_hora DESC LIMIT 1 ) check_in_alert
			FROM casillas_votos_2024 cv WHERE 1 = 1  "; 
	...
	$status_data=$search_database['status_data'];
	if( $status_data!="" ){   //name
		if($status_data=='x'){
			$post_search=true;
			$sql.=" AND NOT EXISTS (SELECT * FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ) ";
			$sqlContador .= "AND NOT EXISTS (SELECT * FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ) ";
		}else{
			$post_search=true;
			$sql.=" AND (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = '{$status_data}' ";
			$sqlContador.=" AND (SELECT cs.status FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id ORDER BY fecha_hora DESC LIMIT 1 ) = '{$status_data}' ";
			//$sql.=" AND EXISTS (SELECT * FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id AND cs.status='$status_data' ) ";
			//$sqlContador .= "AND EXISTS (SELECT * FROM casillas_votos_2024_status cs WHERE cs.id_casilla_voto_2024 = cv.id AND cs.status='$status_data' ) ";
		}
	} 

	$check_in=$search_database['check_in'];
	if( $check_in!="" ){   //name
		if($check_in=='0'){
			$post_search=true;
			$sql.=" AND NOT EXISTS ( SELECT * FROM casillas_votos_2024_checks ck WHERE ck.id_seccion_ine = cv.id_seccion_ine ) ";
			$sqlContador.=" AND NOT EXISTS ( SELECT * FROM casillas_votos_2024_checks ck WHERE ck.id_seccion_ine = cv.id_seccion_ine ) ";
		}else{
			$post_search=true;
			$sql.=" AND ( SELECT count(*) FROM casillas_votos_2024_checks ck WHERE ck.id_seccion_ine = cv.id_seccion_ine )  > 0 ";
			$sqlContador.=" AND ( SELECT count(*) FROM casillas_votos_2024_checks ck WHERE ck.id_seccion_ine = cv.id_seccion_ine ) > 0 ";
		}
	}
	...
	*/
	...
	if($tipo_uso_plataforma=='municipio'){
		$sql.= " AND cv.id_municipio ='{$id_municipio}' AND cv.tipo =0 ";
		$sqlContador.= " AND cv.id_municipio ='{$id_municipio}' AND cv.tipo =0 ";
	}elseif($tipo_uso_plataforma=='distrito_local'){
		$sql.= " AND cv.id_distrito_local ='{$id_distrito_local}' AND cv.tipo =1 ";
		$sqlContador.= " AND cv.id_distrito_local ='{$id_distrito_local}' AND cv.tipo =1 ";
	}elseif($tipo_uso_plataforma=='distrito_federal'){
		$sql.= " AND cv.id_distrito_federal ='{$id_distrito_federal}' AND cv.tipo =2 ";
		$sqlContador.= " AND cv.id_distrito_federal ='{$id_distrito_federal}' AND cv.tipo =2 ";
	}
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM casillas_votos_2024 cv WHERE 1 = 1   "; 
