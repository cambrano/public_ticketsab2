<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'casillas_votos_2024_incidencias','casillasVotos2024Incidencias','index');
	if($redirectSecurity!=""){
		die;
	}
	
	$casilla_voto_2024_incidenciaDatos=casilla_voto_2024_incidenciaDatos($id);
	$id = $casilla_voto_2024_incidenciaDatos['id'];

	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','casillas_votos_2024_incidencias',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('casillas_votos_2024_incidencias',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2024Incidencias/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("sumbmit").disabled = true;
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Estatus Casilla </font>
				</label><br>
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?=$casilla_voto_2024_incidenciaDatos['fechaR']; ?></strong>
			</label><br><br>
			<label class="labelForm" id="labeltemaname">Estatus</label><br>
			<label class="descripcionForm">
				<?php
				$casilla_voto_2024_incidenciaDatos['semaforo'];
				if($casilla_voto_2024_incidenciaDatos['semaforo'] == 1){
					$casilla_voto_2024_incidenciaDatos['semaforo'] = 'Verde';
				}elseif ($casilla_voto_2024_incidenciaDatos['semaforo'] == 2) {
					$casilla_voto_2024_incidenciaDatos['semaforo'] = 'Amarillo';
				}elseif ($casilla_voto_2024_incidenciaDatos['semaforo'] == 3) {
					$casilla_voto_2024_incidenciaDatos['semaforo'] = 'Rojo';
				}else{
					$casilla_voto_2024_incidenciaDatos['semaforo'] = 'No tiene';
				}

				?>
				<strong><?=$casilla_voto_2024_incidenciaDatos['semaforo']; ?></strong>
			</label><br><br>
