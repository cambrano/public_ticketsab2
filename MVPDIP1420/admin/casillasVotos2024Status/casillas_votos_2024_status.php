<?php
	...
	$sql = "SELECT count(*) total FROM casillas_votos_2024_status WHERE 1 = 1   "; 
	...
	id,fechaR,
	CASE 
		WHEN status = 1 THEN 'Abierto'
		WHEN status = 2 THEN 'Cerrado Con Votantes'
		WHEN status = 3 THEN 'Cerrado'
		WHEN status = 4 THEN 'Inicio Conteo'
		ELSE 'No Tiene'
	END AS status,
	IF(tipo =1 , 'Panel','Casilla') tipo, fecha_hora
	FROM casillas_votos_2024_status WHERE 1 = 1";
	$sql.= " AND id_casilla_voto_2024 ='{$id_casilla_voto_2024}' ";
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM casillas_votos_2024_status sia WHERE 1 = 1   "; 
