<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'casillas_votos_2024_status','casillasVotos2024Status','index');
	if($redirectSecurity!=""){
		die;
	}
	
	$casilla_voto_2024_statusDatos=casilla_voto_2024_statusDatos($id);
	$id = $casilla_voto_2024_statusDatos['id'];

	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','casillas_votos_2024',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('casillas_votos_2024_status',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		function cerrar(){
			$("#homebody").load('casillasVotos2024Status/index.php');
		}
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2024Status/db_delete.php",
				data: dataString,
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?=$casilla_voto_2024_statusDatos['fechaR']; ?></strong>
			</label><br><br>
			<label class="labelForm" id="labeltemaname">Estatus</label><br>
			<label class="descripcionForm">
				<?php

				if($casilla_voto_2024_statusDatos['status'] == 1){
					$casilla_voto_2024_statusDatos['status'] = 'Abierto';
				}elseif ($casilla_voto_2024_statusDatos['status'] == 2) {
					$casilla_voto_2024_statusDatos['status'] = 'Cerrado Con Gente';
				}elseif ($casilla_voto_2024_statusDatos['status'] == 3) {
					$casilla_voto_2024_statusDatos['status'] = 'Cerrado';
				}else{
					$casilla_voto_2024_statusDatos['status'] = 'Inicio Con Conteo';
				}

				?>
				<strong><?=$casilla_voto_2024_statusDatos['status']; ?></strong>
			</label><br><br>
