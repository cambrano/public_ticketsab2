<?php
	...
	if($tipo_uso_plataforma=='all'){
		...
		$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','cartografias_senador_2016',$_COOKIE["id_usuario"]);
	}elseif($tipo_uso_plataforma=='senador' || $forzar_senador=='true'){
		$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','cartografias_senador_2016',$_COOKIE["id_usuario"]);
	}else{
...
?>
	...
	<div id="bodymanager" class="bodymanager">
		...
		<?php
		if($tipo_uso_plataforma=='all'){
			echo '<div class="submenux" onclick="subConfiguracionSenadorReportes2016()">Goberandor Reporte '.$ano.'</div> /';
		}
		?>
		<div class="submenux" onclick="subConfiguracionSenadorSeccionesIneReportes2016()">Secciones INE Reportes <?= $ano ?></div> / 
