...
?>
	<div style="text-align: left;padding: 0px 0px 0px 5px;font-size: 8px;color: black;margin-bottom: 20px;margin-top: -13.6px;background-color: rgba(176,176,176,0.6);padding: 5px">
		Votos Totales: <b><?=number_format($votos_totales_2024, 0, '.', ','); ?></b>
		<br>
		Votos Válidos: <b><?=number_format($votos_validos_2024, 0, '.', ','); ?></b>
		<br>
		Porcentaje: <b><?=number_format($votos_validos_porcentaje_2024, 2, '.', ''); ?>%</b>
		<br>
		<br>
		Votos Nulos: <b><?=number_format($votos_nulos_2024, 0, '.', ','); ?></b>
		<br>
		Porcentaje: <b><?=number_format($votos_nulos_porcentaje_2024, 2, '.', ''); ?>%</b>
		<br>
		Votos Can NReg: <b><?=number_format($votos_can_nreg_2024, 0, '.', ','); ?></b>
		<br>
		Porcentaje: <b><?=number_format($votos_can_nreg_porcentaje_2024, 2, '.', ''); ?>%</b>
	...
	var config = {
		...
		data: {
			...
			datasets: [{
				data: [
					//300, 
					<?=number_format($votos_validos_2024, 0, '.', ''); ?>,
					<?=number_format($votos_nulos_2024, 0, '.', ''); ?>,
					<?=number_format($votos_can_nreg_2024, 0, '.', ''); ?>,
				],
				dataX: [
					//300, 
					"<?=number_format($votos_validos_2024, 0, '.', ','); ?>",
