<?php
	...
	$sql="
		...
		GROUP BY cvp.id_municipio,cvp.id_seccion_ine,cvp.id_casilla_voto_2024,cvp.id_partido_2024
	";
	$sql;
	...
	while($row=$result->fetch_assoc()){
		...
		if($row['clave_partidos_coaliciones'] != ''){
			$partidos_coaliciones[$row['id_casilla_voto_2024']][$row['clave_partidos_coaliciones']]=$row;
		}else{
			$partidos_sin_coaliciones[$row['id_casilla_voto_2024']][$row['clave']]=$row;
		} 
		$num=$num+1;
	}
	...
	$sql="
		SELECT
			...
			(SELECT COUNT(cv.id) FROM casillas_votos_2024 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') casillas,
			(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2024 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}' ) lista_nominal,
			(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2024 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_nulos,
			(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2024 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_can_nreg,
			(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2024 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_validos
		FROM casillas_votos_2024 si
