<?php
	...
	$sql = "SELECT count(*) total FROM correos_mailing WHERE 1 = 1   "; 
	...
	////////////////////////////
	////////////////////////////
	$sql = "SELECT id,fechaR,nombre,IF(status=1,'Activo','No Activo') status,usuario FROM correos_mailing WHERE 1 = 1  "; 
	...
	while($row=$result->fetch_assoc()){
		...
		$nestedData[] = "<div style='text-transform: none;'>".$row["status"]."<div>"; 
		if($option_delete){
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM correos_mailing WHERE 1 = 1   "; 
