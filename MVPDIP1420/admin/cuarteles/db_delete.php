<?php
	...
	if(!empty($_POST)){
		...
		$delete_cuarteles = "DELETE FROM cuarteles  WHERE  id='{$id}' ";
		$delete_cuarteles=$conexion->query($delete_cuarteles);
		$num=$conexion->affected_rows;
		if(!$delete_cuarteles || $num=0){
			$success=false;
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'cuarteles',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
