<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'cuarteles','cuarteles','index');
	if($redirectSecurity!=""){
		die;
	}
	$cuartelDatos=cuartelDatos($id);
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','cuarteles',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('cuarteles',$id);
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Cuartel</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Cuartel</label><br>
			<label class="descripcionForm">
				<strong><?= $cuartelDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
