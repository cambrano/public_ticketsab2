<?php
	...
	$sql = "SELECT count(*) total FROM cuestionarios WHERE 1 = 1   "; 
	if($id_encuesta!=''){
	...
	////////////////////////////
	////////////////////////////
	$sql = "SELECT id,campo,orden,tipo,pregunta FROM cuestionarios e WHERE 1 = 1   "; 
	...
	while($row=$result->fetch_assoc()){
		$nestedData=array(); 
		$nestedData[] = $row["orden"];
		$nestedData[] = $row["tipo"];
		$nestedData[] = $row["campo"];
		$nestedData[] = $row["pregunta"];
		if($option_delete){
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM cuestionarios e WHERE 1 = 1   "; 
