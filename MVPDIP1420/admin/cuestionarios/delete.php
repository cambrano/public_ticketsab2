<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'cuestionarios','cuestionarios','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('cuestionarios',$id);
	$registros_titutlo = 'Este registro esta ligado a : <br>';
	foreach ($tablasRelacionadas['tablas'] as $key => $value) {
		if($value['registros']>0 && $value['tabla'] != 'cuestionarios_respuestas' ){
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "cuestionarios/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("sumbmit").disabled = true;
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Pregunta </font>
				</label><br>
		...
		<div class="bodyinput">
			<br>
			<label class="labelForm" id="labeltemaname">Pregunta</label><br>
			<label class="descripcionForm">
				<strong><?=$cuestionarioDatos['pregunta']; ?></strong>
			</label><br><br>
