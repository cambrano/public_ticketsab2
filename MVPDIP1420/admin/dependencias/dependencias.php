<?php
	...
	$sql = "SELECT count(*) total FROM dependencias WHERE 1 = 1   "; 
	...
	////////////////////////////
	////////////////////////////
	$sql = "SELECT id,clave,nombre_corto,nombre FROM dependencias WHERE 1 = 1  "; 
	...
	while($row=$result->fetch_assoc()){
		...
		$nestedData[] = "<div style='text-transform: none;'>".$row["nombre_corto"]."<div>";
		...
		$sub_dependencias='<button class="btn btn-primary bt_responsive"  onClick="sub_dependencias('.$row["id"].');" >Sub Nivel</button>';
		$nestedData[] =  "<div class='opciones_botones_2'>{$edit}{$delete}{$select}{$sub_dependencias}</div>";
		$data[] = $nestedData;
	}
	////////////////////////////
	///numero total de filtrados
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM dependencias WHERE 1 = 1   "; 
