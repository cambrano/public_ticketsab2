<?php
	...
	if(!empty($_POST)){
		...
		$delete_directorios = "DELETE FROM directorios  WHERE  id='{$id}' ";
		$delete_directorios=$conexion->query($delete_directorios);
		$num=$conexion->affected_rows;
		if(!$delete_directorios || $num=0){
			$success=false;
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'directorios',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
