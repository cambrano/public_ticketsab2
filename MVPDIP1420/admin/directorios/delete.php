<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'directorios','directorios','index');
	if($redirectSecurity!=""){
		die;
	}
	$directorioDatos=directorioDatos($id);
	$moduloAccionPermisos = moduloAccionPermisos('operatividad','directorios',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('directorios',$id);
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Directorio Institucional</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Directorio Institucional</label><br>
			<label class="descripcionForm">
				<strong><?= $directorioDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
