<?php
	...
	/*
	for ($i=0; $i < 1822; $i++) { 
		$arrayasd[] = celda_excel($i);
	}
	echo $values_pdo = "'".implode("','", $arrayasd)."'";
	*/
?>
		...
		function searchTable(value){
			...
			for (var i = 0; i < id_distrito_federal_input.length; i++) {
				if (id_distrito_federal_input.options[i].selected){
					...
					id_distrito_federal_array_table.push("(^" + alphabet[id_distrito_federal_input.options[i].value] + "$)");
			...
			for (var i = 0; i < partido_ganador_id_input.length; i++) {
				if (partido_ganador_id_input.options[i].selected){
					...
					console.log(alphabet[partido_ganador_id_input.options[i].value]);
			...
			if(id_distrito_federal==""){
				$('#distritos_federales_reportes-tabla').DataTable().column(0).search(id_distrito_federal).draw();
			}else{
				$('#distritos_federales_reportes-tabla').DataTable().column(0).search(id_distrito_federal_array_table.join('|'), true, false).draw();

			}

			if(partido_ganador_id==""){
				$('#distritos_federales_reportes-tabla').DataTable().column(1).search(partido_ganador_id).draw();
			}else{
				//$('#distritos_federales_reportes-tabla').DataTable().column(1).search("(^"+partido_ganador_id+"$)",true,false).draw();
				$('#distritos_federales_reportes-tabla').DataTable().column(1).search(partido_ganador_id_array_table.join('|'), true, false).draw();
			}
			...
			$.ajax({
				type: "POST",
				url: "municipiosReportes2024/table.php",
			...
			*/
			$.ajax({
				type: "POST",
				url: "distritosFederalesReportes2024/mapa.php",
				data: {searchTable: searchTable,mapa:mapa},
				async: true,
				success: function(data) {
					$("#mapaLoad").html(data);
