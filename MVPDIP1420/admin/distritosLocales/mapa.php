<?php
	...
	if(!empty($_POST)){
		...
		$distritos_locales_parametrosDatos = distritos_locales_parametrosDatos('','',' id_seccion_ine,orden ASC',$limit);
		...
		$orderby = ' ORDER BY numero DESC';
		$limit = 'LIMIT 0,11';
		$distritos_localesDatosMapa = distritos_localesDatosMapa();
		...
		function myMap(){
			...
			foreach ($distritos_locales_parametrosDatosMapa as $key => $value) {
				...
				$div = '<div class="divMapaSeccion">
							<h4>Distrio Local: '.$distritos_localesDatosMapa[$key]['numero'].'</h4>
						</div>';
				$div = preg_replace("/[\r\n|\n|\r]+/", " ", $div);
				...
				distritos_area<?= $key ?>.addListener("click", (function(event){
					myLatlng = new google.maps.LatLng("<?= $distritos_localesDatosMapa[$key]['latitud'] ?>","<?= $distritos_localesDatosMapa[$key]['longitud'] ?>"); 
			...
			if($busqueda){
				...
				var marcadores = [
				<?php
					foreach ($distritos_localesDatosMapa as $key => $value) {
