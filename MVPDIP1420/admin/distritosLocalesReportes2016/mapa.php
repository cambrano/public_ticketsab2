<?php
	...
	if(!empty($_POST)){
		...
		$sql="
			...
			FROM  casillas_votos_partidos_2016 cvp
			LEFT JOIN partidos_2016 p
			ON p.id = cvp.id_partido_2016
			WHERE cvp.tipo = '{$tipo}' {$sqlPartidos}
			GROUP BY cvp.id_distrito_local,cvp.id_partido_2016
		";
		...
		while($row=$result->fetch_assoc()){
			...
			if($row['clave_partidos_coaliciones'] != ''){
				$partidos_coaliciones[$row['id_distrito_local']][$row['clave_partidos_coaliciones']]=$row;
			}else{
				$partidos_sin_coaliciones[$row['id_distrito_local']][$row['clave']]=$row;
			} 
			$num=$num+1;
		}
		$sql="
			SELECT
				...
				si.latitud,
				si.longitud,
				(SELECT COUNT(cv.id) FROM casillas_votos_2016 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') casillas,
				(SELECT COUNT(cv.id) FROM secciones_ine cv WHERE cv.id_distrito_local = si.id ) secciones_ine,

				(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2016 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}' ) lista_nominal,
				(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2016 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_nulos,
				(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2016 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_can_nreg,
				(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2016 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_validos
