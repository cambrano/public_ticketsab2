		...
		function searchTable(value){
			...
			for (var i = 0; i < id_distrito_local_input.length; i++) {
				if (id_distrito_local_input.options[i].selected){
					...
					id_distrito_local_array_table.push("(^" + alphabet[id_distrito_local_input.options[i].value] + "$)");
			...
			if(id_distrito_local==""){
				$('#distritos_locales_reportes-tabla').DataTable().column(0).search(id_distrito_local).draw();
			}else{
				$('#distritos_locales_reportes-tabla').DataTable().column(0).search(id_distrito_local_array_table.join('|'), true, false).draw();

			}

			if(partido_ganador_id==""){
				$('#distritos_locales_reportes-tabla').DataTable().column(1).search(partido_ganador_id).draw();
			}else{
				//$('#distritos_locales_reportes-tabla').DataTable().column(1).search("(^"+partido_ganador_id+"$)",true,false).draw();
				$('#distritos_locales_reportes-tabla').DataTable().column(1).search(partido_ganador_id_array_table.join('|'), true, false).draw();
			}
			...
			*/
			$.ajax({
				type: "POST",
				url: "distritosLocalesReportes2018/mapa.php",
				data: {searchTable: searchTable,mapa:mapa},
				async: true,
				success: function(data) {
					$("#mapaLoad").html(data);
