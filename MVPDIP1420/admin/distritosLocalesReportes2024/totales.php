<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2024 WHERE tipo = '{$tipo}') votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2024 WHERE tipo = '{$tipo}') votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2024 WHERE tipo = '{$tipo}') casillas,
				(SELECT COUNT(id) FROM distritos_locales ) distritos_locales,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2024 WHERE tipo = '{$tipo}') total_lista_nominal
			FROM
			(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2024 WHERE tipo = '{$tipo}')t";
