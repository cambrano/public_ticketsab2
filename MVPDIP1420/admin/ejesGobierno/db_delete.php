<?php
	...
	if(!empty($_POST)){
		...
		$delete_ejes_gobierno = "DELETE FROM ejes_gobierno  WHERE  id='{$id}' ";
		$delete_ejes_gobierno=$conexion->query($delete_ejes_gobierno);
		$num=$conexion->affected_rows;
		if(!$delete_ejes_gobierno || $num=0){
			$success=false;
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'ejes_gobierno',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
