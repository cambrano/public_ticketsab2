	...
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_ciudadanos_encuestas-tabla').DataTable( {
				...
				"ajax":{
					url :"encuestasDistritosFederales/secciones_ine_ciudadanos_encuestas.php", // json datasource
			...
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="encuestasDistritosFederales/index.php?cot="+ids_string;   
		...
		function edit(valor){
			link="encuestasDistritosFederales/update.php?id="+valor; 
			var link2="encuestasDistritosFederales/update.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function encuestasSeccionesDistritoFederal(valor){
			link="encuestasSecciones/distrito_federal/index.php?id=<?= $id_encuesta ?>&id_distrito_federal="+valor; 
