<?php
	...
	$filename = 'Ciudadanos_Encuetas_Municipales_-_'.date("Ymd-His").'-'.$gen_id3.$mk_id.'.xlsx';
	...
	$id_municipio=$_POST['excel'][0]['id_municipio'];
	if( $id_municipio!="" ){
		$sqlexcel.=" AND sice.id_municipio IN ($id_municipio) ";
	}
	...
	$keywords = array('Ciudadanos Encuestas Municipales','Data','Información');
	$writer->setTitle('Ciudadanos Encuestas Municipales');
	...
	while($row=$result->fetch_assoc()){
		//$row['Fecha Hora'] = date('Y-m-d H:i:s', strtotime($row['Fecha Hora']));
		if($numero == 300001){
			$numero = 1;
		}
		if($numero==1){ 
