<?php
	...
	if(!empty($_POST)){
		...
		$sql="
			...
			( SELECT s.fecha_hora FROM secciones_ine_ciudadanos_encuestas s WHERE s.id_municipio = m.id AND s.id_encuesta ='{$id_encuesta}' AND s.id_municipio = m.id ORDER BY s.fecha_hora DESC LIMIT 1) ultima_encuesta,
