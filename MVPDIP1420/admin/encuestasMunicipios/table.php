	...
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_ciudadanos_encuestas-tabla').DataTable( {
				...
				"ajax":{
					url :"encuestasMunicipios/secciones_ine_ciudadanos_encuestas.php", // json datasource
			...
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="encuestasMunicipios/index.php?cot="+ids_string;   
		...
		function edit(valor){
			link="encuestasMunicipios/update.php?id="+valor; 
			var link2="encuestasMunicipios/update.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function encuestasSeccionesMunicipio(valor){
			link="encuestasSecciones/municipio/index.php?id=<?= $id_encuesta ?>&id_municipio="+valor; 
