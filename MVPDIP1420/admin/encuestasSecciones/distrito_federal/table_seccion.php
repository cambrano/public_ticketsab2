	...
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_ciudadanos_encuestas-tabla').DataTable( {
				...
				"ajax":{
					url :"encuestasSecciones/distrito_federal/secciones_ine_ciudadanos_encuestas_seccion.php", // json datasource
			...
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="encuestasSecciones/distrito_federal/index.php?cot="+ids_string;   
		...
		function edit(valor){
			link="encuestasSecciones/distrito_federal/update_seccion.php?id="+valor; 
			var link2="encuestasSecciones/distrito_federal/update_seccion.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function downloadExcel(){
			...
			form.method = 'POST';
			form.action = 'encuestasDistritosFederales/excel/index.php?cot=<?=$_COOKIE['pageService']?>'; // URL de destino
		...
		function encuestasSeccionesDistritoFederal(valor){
			link="encuestasSecciones/distrito_federal/index.php?id="+valor; 
			var link2="encuestasSecciones/distrito_federal/index.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
		...
		function encuestasSeccion(valor){
			link="encuestasSecciones/distrito_federal/seccion.php?id="+valor; 
			var link2="encuestasSecciones/distrito_federal/seccion.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
