<?php
	...
	if($id_seccion_ine !=''){
		$sql_seccion = " AND id_seccion_ine ='{$id_seccion_ine}' ";
	}

	$sql = "
		SELECT 
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos WHERE id_distrito_federal ='{$id_distrito_federal}' {$sql_seccion} ) totales_ciudadanos,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas  WHERE id_encuesta = 1 AND id_distrito_federal ='{$id_distrito_federal}' {$sql_seccion} ) totales_encuestados,
		...
		WHERE sice.id_encuesta = '{$id_encuesta}' AND sice.id_distrito_federal ='{$id_distrito_federal}' ";

	if($id_seccion_ine !=''){
		$sql.= " AND sice.id_seccion_ine ='{$id_seccion_ine}' ";
	}
	...
	if($id_seccion_ine!=''){
		$sql_seccion.= " AND sicer.id_seccion_ine = {$id_seccion_ine} ";
	}
	...
	$sql="
		SELECT 
			...
			cr.id_cuestionario,
			cr.respuesta,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas_respuestas sicer WHERE sicer.id_cuestionario_respuesta = cr.id AND sicer.id_distrito_federal='{$id_distrito_federal}' {$sql_seccion} ) respuestas
	...
	//echo "<pre>";
	//print_r($datos_distritos_federales_encuestados);
	//echo "</pre>";
	if($id_seccion_ine==''){
		$sql="
			SELECT 
