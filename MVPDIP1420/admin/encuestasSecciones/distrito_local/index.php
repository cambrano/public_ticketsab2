<?php
	...
	if($_GET['refresh']==1){
		...
		if($id_encuesta!=""){
			...
			$id_distrito_local=$_GET['id_distrito_local'];
			setcookie("paguinaId_2",encrypt_ab_check($id_distrito_local), array('expires' => time() + (60*60*24), 'path' => '/', 'secure' => true, 'httponly' => true, 'samesite' => 'Strict'));
		}else{
			$id_encuesta = decrypt_ab_checkFinal($_COOKIE['paguinaId_1']);
			$id_distrito_local = decrypt_ab_checkFinal($_COOKIE['paguinaId_2']);
		}
		?>
