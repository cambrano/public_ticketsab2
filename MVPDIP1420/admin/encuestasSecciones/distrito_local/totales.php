<?php
	...
	$sql = "
		SELECT 
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos WHERE id_distrito_local ='{$id_distrito_local}' {$sql_seccion} ) totales_ciudadanos,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas  WHERE id_encuesta = 1 AND id_distrito_local ='{$id_distrito_local}' {$sql_seccion} ) totales_encuestados,
		...
		WHERE sice.id_encuesta = '{$id_encuesta}' AND sice.id_distrito_local ='{$id_distrito_local}' ";
	...
	if($id_seccion_ine==''){
		...
		if($id_distrito_local!=''){
			$sql.= " AND m.id ='{$id_distrito_local}'  ";
		}
	...
	$sql="
		SELECT 
			...
			cr.id_cuestionario,
			cr.respuesta,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas_respuestas sicer WHERE sicer.id_cuestionario_respuesta = cr.id AND sicer.id_distrito_local='{$id_distrito_local}' {$sql_seccion} ) respuestas
