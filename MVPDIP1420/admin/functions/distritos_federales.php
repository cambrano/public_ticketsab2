<?php
	...
	function distritos_federalesMunicipio($id=null,$id_municipio=null,$sin_seleccione=null) {
		include 'db.php'; 
		$id;
		$ids = explode(",", $id);
		if($sin_seleccione==""){
		...
		echo $sql="
			SELECT * FROM distritos_federales dl 
			WHERE EXISTS (
				SELECT * FROM secciones_ine s WHERE s.id_municipio = '{$id_municipio}' AND dl.id = s.id_distrito_federal
			)
		";
