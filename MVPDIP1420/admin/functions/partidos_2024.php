<?php
		function partidos_2024($id=null,$tipo=null,$coalicion=null,$sin_seleccione=null) {
			include 'db.php'; 
			$id;
		...
		function partidos_2024Nombres($id=null,$tipo=null,$coalicion=null,$sin_seleccione=null) {
			include 'db.php'; 
			$id;
			$select[$id]='selected="selected"';
			if($sin_seleccione==""){
			...
			while($row=$result->fetch_assoc()){
				$sel=$row['id'];
				$return .="<option ".$select[$sel]." value='".$row['nombre_corto']."' >".str_replace("_"," - ",$row['nombre_corto'])."</option> ";
			} 
			$conexion->close();
			return $return;
		}
		...
		function partidos_2024CompletaDatos($id=null,$nombre_corto=null){
			include 'db.php';
			$sql=("SELECT * FROM partidos_2024 WHERE 1 ");
			if($id!=''){
				$sql.= " AND id = '{$id}' ";
			}
			if($nombre_corto!=''){
		...
		function partido_2024PrincipalTipoEleccionDatos($tipo=null){
			include 'db.php';
			$sql=("SELECT * FROM partidos_2024 WHERE principal=1 ");
			if($tipo!=""){
				$sql.=" AND tipo={$tipo} ";
