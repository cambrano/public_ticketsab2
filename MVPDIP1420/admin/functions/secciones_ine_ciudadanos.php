<?php
	...
	function seccion_ine_ciudadanoFamilia($id_seccion_ine_ciudadano = null) {
		include 'db.php'; // Si no se proporciona una conexión, se usa la conexión desde db.php
	
		$datos = array(); // Inicializamos un array para almacenar los datos
	
		// Verificamos si se proporciona un ID de sección, si no se proporciona, devolvemos un array vacío
		if ($id_seccion_ine_ciudadano === null) {
			return $datos;
		}
	
		// Consultamos la base de datos para obtener los datos del ciudadano
		$sql = "SELECT * , (SELECT tc.nombre FROM tipos_ciudadanos tc WHERE tc.id = sic.id_tipo_ciudadano) tipo_ciudadano
				FROM secciones_ine_ciudadanos sic WHERE sic.id = '{$id_seccion_ine_ciudadano}'";
		$resultado = $conexion->query($sql);
		...
		if ($resultado->num_rows > 0) {
			...
			$row = $resultado->fetch_assoc();
			$datos[] = $row;
			// Si hay un ID de sección de ciudadano relacionado, llamamos recursivamente a la función
			if ($row['id_seccion_ine_ciudadano_compartido'] != '') {
				$id_seccion_ine_ciudadano_relacionado = $row['id_seccion_ine_ciudadano_compartido'];
				$datos_adicionales = seccion_ine_ciudadanoFamilia($id_seccion_ine_ciudadano_relacionado);
				// Concatenamos los datos adicionales obtenidos de la recursión
				$datos = array_merge($datos, $datos_adicionales);
			}
		}
	
		// Devolvemos todos los datos recolectados
		return $datos;
	}
