	...
	if(!empty($_POST)){
		...
		foreach ($data as $key => $value) {
			...
			if($tabla=="administradores_sistema" || $tabla=="empleados"){
				...
				$empleados['telefono']=$value['telefono'];
				$empleados['clave']=$value['clave'];
				$empleados['status']=$value['status'];
				$empleados['id_empleado_grupo']=$value['id_empleado_grupo'];
			...
			if($value['id']!=""){  
				if($status_comparacion){
					if($tabla=="empleados"){
						...
						unset($registrosUsuarios['id_empleado_grupo']);
						...
						$insertArray[$numarray]['tabla']='empleados';
