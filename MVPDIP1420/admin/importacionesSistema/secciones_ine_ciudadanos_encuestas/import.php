<?php
	...
	$file_doc = "import/34_ciudadanos_encuestas_".$xxx_numero.".csv";
	...
	$columnData = array(
			...
			'observaciones'=>array('alfanumerico','requerido'),
	);
	...
	$usuario=usuarios(); 
	$tipo=1;
	$tabla="secciones_ine_ciudadanos_encuestas";
	$numarray=0;
	...
	$primaria="id_seccion_ine_ciudadano_encuesta";
	foreach ($data as $key => $value) {
		...
		$value['id_seccion_ine_ciudadano'] = $claveIdDatos['id'];
		...
		$value['edad'] = $claveIdDatos['edad'];
		if($claveIdDatos['sexo']=="Hombre"){
			$value['sexo'] = 1;
		}else{
			$value['sexo'] = 2;
		}
	...
	if($numarray>0){
		...
		if($successcon!=1){
			echo "No se inserto ningun registro1";
		}
	}
?>