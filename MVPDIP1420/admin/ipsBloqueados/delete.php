<?php
	...
	$tablasRelacionadas = tablasRelacionadas('ips_bloqueados',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "ipsBloqueados/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("mensaje").classList.remove("mensajeError"); 
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar IP Bloqueada</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">IP Bloqueada</label><br>
			<label class="descripcionForm">
				<strong><?= $ip_bloqueadoDatos['ip']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
