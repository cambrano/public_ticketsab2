<?php
	...
	$sql = "SELECT count(*) total FROM ips_bloqueados WHERE 1 = 1   "; 
	...
	////////////////////////////
	////////////////////////////
	$sql = "SELECT id,ip,observaciones FROM ips_bloqueados WHERE 1 = 1  "; 
	// getting records as per search parameters
	$ip=$search_database['ip'];
	if( $ip!="" ){   //name
		$post_search=true;
		$sql.=" AND ip LIKE '%{$ip}%' ";
		$sqlContador .= " AND ip LIKE '%{$ip}%' ";
	}
	...
	while($row=$result->fetch_assoc()){
		$nestedData=array(); 
		$nestedData[] = str_pad($row["id"],6,0,STR_PAD_LEFT); 
		$nestedData[] = $row["ip"];
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM ips_bloqueados WHERE 1 = 1   "; 
