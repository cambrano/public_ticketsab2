<?php
	...
	if(!empty($_POST)){
		...
		include '../functions/log_usuarios_tracking_secciones.php';
		...
		$log_usuarios_tracking_seccionesDatosArray=log_usuarios_tracking_seccionesDatosArray($_POST['searchTable'][0],$orderby,$limit);
		...
		$casillas_votos_2024Datos = casillas_votos_2024DatosMapa('',$_POST['searchTable'][0]['id_seccion_ine'],$id_municipio,$id_distrito_local,$id_distrito_federal,'','','id_seccion_ine' );

		if($_POST['searchTable'][0]['id_seccion_ine']!=""){
			foreach ($casillas_votos_2024Datos as $key => $value) {
...
?>
	...
	<script type="text/javascript">
		function myMap(){
			...
			var marcadores = [
				<?php
					foreach ($log_usuarios_tracking_seccionesDatosArray as $key => $value) {
			...
			<?php
				foreach ($casillas_votos_2024Datos as $key => $value) {
					?>
					color_100 = '#ffff00'; //25% saturado
					color_30 = '#ecec53'; //25% lighter
					color_10 = '#b8b814'; //25% darker
					const pos_casilla_<?= $value['id'] ?> = {
						lat: <?= $value['latitud'] ?>,
						lng: <?= $value['longitud'] ?>,
					};
					map.setCenter(pos_casilla_<?= $value['id'] ?>);
					const circleCasilla100mts_<?= $value['id'] ?> = new google.maps.Circle({
						...
						fillOpacity: 0.35,
						map: map,
						center: pos_casilla_<?= $value['id'] ?>,
						radius: 100, // Radio del círculo en metros (puedes ajustarlo según tus necesidades)
					});
					const circleCasilla30mts_<?= $value['id'] ?> = new google.maps.Circle({
					...
					const circle10Casillamts_<?= $value['id'] ?> = new google.maps.Circle({
			...
			<?php
				foreach ($log_usuarios_tracking_seccionesDatosArray as $key => $value) {
					if($value['alerta_seccion'] == 1){
						?>
        				var puntoA = { lat: <?= $value['latitud_script'] ?>, lng: <?= $value['longitud_script'] ?> }; // Coordenadas del punto A
						var puntoB = { lat: <?= $value['punto_latitud'] ?>, lng: <?= $value['punto_longitud'] ?> }; // Coordenadas del punto B
						var line_<?= $value['id'] ?> = new google.maps.Polyline({
							path: [puntoA, puntoB],
							geodesic: true,
							strokeColor: '#070BED',
							strokeOpacity: 1.0,
