<?php
	...
	$dataPost["militante_partido"][0]['id_seccion_ine_ciudadano'] = seccion_ine_ciudadanoClaveElectorVerificacion($dataPost["militante_partido"][0]['clave_elector']);
	unset($dataPost["militante_partido"][0]['clave_elector']);
	...
	$militante_partidoClaveVerificacion=militante_partidoClaveVerificacion($dataPost["militante_partido"][0]["clave"],$dataPost["militante_partido"][0]['id'],1);
	...
	$imagen_img=1;
	if( registrosCompara("militantes_partidos",$dataPost["militante_partido"][0],1) || !empty($imagen_img) ){
		if(!empty($dataPost)){ 
			////! validamos si tiene imagen
			$militante_partidoDatos = militante_partidoDatos($dataPost["militante_partido"][0]['id']);
			if($imagen!=""){
					...
					if($_FILES['imagen']['error']==UPLOAD_ERR_OK) {
						if(is_uploaded_file($_FILES['imagen']['tmp_name'])){
							if($_FILES['imagen']['type'] == "image/jpg" || $_FILES['imagen']['type'] == "image/jpeg" || $_FILES["imagen"]["type"] == "image/png" ){
								...
								$name_old = $militante_partidoDatos['name'];
								if($name_old==""){
									include "../functions/genid.php";
									$name_old = $dataPost["militante_partido"][0]['name']='mp_image-'.$mk_id.'_'.$cod32.'.'.$extension['extension'];
								}
								$filepath_old = $militante_partidoDatos['file'];
								$name_solo = explode(".", $name_old);
								$name_new = $dataPost["militante_partido"][0]['name']=$name_solo[0].'.'.$extension['extension'];
								$rutaEfs = rutaEfs();
								$filepath = $rutaEfs.$dataPost["militante_partido"][0]['name'];
				...
				$dataPost["militante_partido"][0]['name']=$militante_partidoDatos['name'];
				$filepath = '../ftpFiles/files/'.$dataPost["militante_partido"][0]['name'];
				$dataPost["militante_partido"][0]['file']=$filepath;
				$dataPost["militante_partido"][0]['type']=$militante_partidoDatos['type'];
				$dataPost["militante_partido"][0]['file_size']=$militante_partidoDatos['file_size'];
			}

			$dataPost["militante_partido"][0]["fechaR"]=$fechaH;
			$dataPost["militante_partido"][0]["codigo_plataforma"]=$codigo_plataforma;
			$success=true;
			foreach($dataPost["militante_partido"] as $keyPrincipal => $atributos) {
			...
			unset($dataPost["militante_partido"][0]['id']); 
			$id_militante_partido=$dataPost["militante_partido"][0]["id_militante_partido"]=$id;
