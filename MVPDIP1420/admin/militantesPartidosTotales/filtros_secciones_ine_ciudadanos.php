...
?>
	<script type="text/javascript">
		...
		function searchTable(){
			...
			nombre = nombre.replace(espacios_invalidos, '');
			if(nombre != ""){
				estatus = 1;
			}
			var apellido_paterno = document.getElementById("lista_apellido_paterno").value;
			apellido_paterno = apellido_paterno.replace(espacios_invalidos, '');
			if(apellido_paterno != ""){
				estatus = 1;
			}
			var apellido_materno = document.getElementById("lista_apellido_materno").value;
			apellido_materno = apellido_materno.replace(espacios_invalidos, '');
			if(apellido_materno != ""){
				estatus = 1;
			}
			if(estatus == 0){
			...
			$.ajax({
				type: "POST",
				url: "militantesPartidosTotales/table_secciones_ine_ciudadanos.php",
