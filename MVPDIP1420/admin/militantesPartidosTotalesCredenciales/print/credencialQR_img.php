<?php
	...
	$img_logo='<img src="../../images/logos_partidos/'.$logo.'" height="40px" >';
	...
	$enlace_actual = 'https://' . $_SERVER['HTTP_HOST'].'/credencialDigital/militante.php?cot='.$id_seccion_ine_ciudadano.'&ck='.substr($codigo_seccion_ine_ciudadano, 0, 6).'&hex='.$id_militante_partido.'&tipo=dg';
	$enlace_actual = 'https://' . $_SERVER['HTTP_HOST'].'/cd/mp.php?cot='.$codigo_seccion_ine_ciudadano.'&sic='.$id_seccion_ine_ciudadano.'&hex='.$id_militante_partido.'&tipo=dg';
	$enlace_actual = 'https://' . $_SERVER['HTTP_HOST'].'/cd/mp.php?cot='.$id_seccion_ine_ciudadano.'&ck='.substr($codigo_seccion_ine_ciudadano, 0, 6).'&hex='.$id_militante_partido.'&tipo=dg';
	...
	imagecopyresampled($new_image, $party_image, 0, 0, 0, 0, 80, 80, imagesx($party_image), imagesy($party_image));
	...
	imagecopy($qr_image = imagecreatefromstring($qr_image_data), $new_image, $x, $y, 0, 0, 80, 80);
	...
	$img_qr = '<img style="width: 131px;" src="data:image/png;base64,' . $image_base64 . '" alt="QR Code with Image">';
	...
	// Inicializar mPDF
	$mpdf = new mPDF('L', array(85.6, 54)); // Tamaño de la tarjeta PVC en milímetros
