<?php
	...
	if(!empty($_POST)){
		...
		$sql="
			...
			GROUP BY cvp.id_municipio,cvp.id_partido_2024
		";
		...
		$sql="
			SELECT
				...
				si.latitud,
				si.longitud,
				(SELECT COUNT(cv.id) FROM casillas_votos_2024 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') casillas,
				(SELECT COUNT(cv.id) FROM secciones_ine cv WHERE cv.id_municipio = si.id ) secciones_ine,

				(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2024 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}' ) lista_nominal,
				(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2024 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_nulos,
				(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2024 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_can_nreg,
				(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2024 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_validos
