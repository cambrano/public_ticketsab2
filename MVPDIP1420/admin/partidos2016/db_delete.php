<?php
	...
	if(!empty($_POST)){
		...
		$delete_partidos_2016 = "DELETE FROM partidos_2016  WHERE  id='{$id}' ";
		$delete_partidos_2016=$conexion->query($delete_partidos_2016);
		$num=$conexion->affected_rows;
		if(!$delete_partidos_2016 || $num=0){
			$success=false;
			echo "ERROR delete partido 2016"; 
			echo "<br>";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'partidos_2016',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
