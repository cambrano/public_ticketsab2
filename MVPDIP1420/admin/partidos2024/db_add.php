<?php
	...
	if(!empty($_POST)){
		...
		$partido_2024ClaveVerificacion=partido_2024ClaveVerificacion($_POST["partido_2024"][0]['clave'],'',1);
		...
		$inset_partidos_2024= "INSERT INTO partidos_2024 ($fields_pdo) VALUES ($values_pdo);";

		$inset_partidos_2024=$conexion->query($inset_partidos_2024);
		$num=$conexion->affected_rows;
		if(!$inset_partidos_2024 || $num=0){
			$success=false;
			echo "ERROR inset_partidos_2024"; 
			var_dump($conexion->error);
		}

		$id=$_POST['partido_2024'][0]['id_partido_2024']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($_POST['partido_2024'][0]))."`";
		$values_pdo = "'".implode("','", $_POST['partido_2024'][0])."'";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'partidos_2024',$id,'Insert','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
