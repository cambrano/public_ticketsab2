<?php
	...
	if(!empty($_POST)){
		...
		$delete_partidos_2024 = "DELETE FROM partidos_2024  WHERE  id='{$id}' ";
		$delete_partidos_2024=$conexion->query($delete_partidos_2024);
		$num=$conexion->affected_rows;
		if(!$delete_partidos_2024 || $num=0){
			$success=false;
			echo "ERROR delete partido 2024"; 
			echo "<br>";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'partidos_2024',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
