<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'partidos_2024','partidos2024','index');
	if($redirectSecurity!=""){
		die;
	}
	$partido_2024Datos=partido_2024Datos($id);
	$id_imagen=$partido_2024Datos['id_imagen'];
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','partidos_2024',$_COOKIE["id_usuario"]);
	$tablasRelacionadas = tablasRelacionadas('partidos_2024',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "partidos2024/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("mensaje").classList.remove("mensajeError"); 
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?= $partido_2024Datos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
