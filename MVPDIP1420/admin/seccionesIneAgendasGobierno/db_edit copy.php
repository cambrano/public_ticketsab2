<?php
	...
	include __DIR__."/../functions/secciones_ine_agendas_gobierno_puntos.php";
	...
	if($seccion_ine_agenda_gobiernoClaveVerificacion){
		$claveF= clave("secciones_ine_agendas_gobierno");
	...
	if( registrosCompara("secciones_ine_agendas_gobierno",$_POST['seccion_ine_agenda_gobierno'][0],1) ){
		if(!empty($_POST)){
			...
			//$_POST["seccion_ine_agenda_gobierno"][0]['fecha_hora']=$_POST["seccion_ine_agenda_gobierno"][0]['fecha']." ".$_POST["seccion_ine_agenda_gobierno"][0]['hora'];
			$_POST["seccion_ine_agenda_gobierno"][0]['fecha_hora'] = $_POST["seccion_ine_agenda_gobierno"][0]['fecha'].' '.$_POST["seccion_ine_agenda_gobierno"][0]['hora'];
			...
			$success=true;
			foreach($_POST['seccion_ine_agenda_gobierno'] as $keyPrincipal => $atributos) {
			...
			$update_secciones_ine_agendas_gobierno = "UPDATE secciones_ine_agendas_gobierno SET ". join(",",$valueSets) . " WHERE id=".$id;
			$conexion->autocommit(FALSE);
			$update_secciones_ine_agendas_gobierno=$conexion->query($update_secciones_ine_agendas_gobierno);
			$num=$conexion->affected_rows;
			if(!$update_secciones_ine_agendas_gobierno || $num=0){
				$success=false;
				echo "<br>";
				echo "ERROR update_secciones_ine_agendas_gobierno"; 
				var_dump($conexion->error);
			}

			unset($_POST["seccion_ine_agenda_gobierno"][0]['id']); 
			$id_seccion_ine_agenda_gobierno=$_POST['seccion_ine_agenda_gobierno'][0]['id_seccion_ine_agenda_gobierno']=$id;
	...
	foreach ($add_dependencias as $key => $value) {
		$dep['id_seccion_ine_agenda_gobierno'] = $id_seccion_ine_agenda_gobierno;
	...
	foreach ($delete_dependencias as $key => $value) {
		$id = $value;
		$delete_secciones_ine_agendas_gobierno_puntos = "DELETE FROM secciones_ine_agendas_gobierno_dependencias  WHERE  id='$id' ";
		$conexion->autocommit(FALSE);
		$delete_secciones_ine_agendas_gobierno_puntos=$conexion->query($delete_secciones_ine_agendas_gobierno_puntos);
		$num=$conexion->affected_rows;
		if(!$delete_secciones_ine_agendas_gobierno_puntos || $num=0){
			$success=false;
			echo "ERROR delete_secciones_ine_agendas_gobierno_puntos."; 
			echo "<br>";
    ...
    foreach ($delete_dependencias_generales as $key => $value) {
        $id = $value;
        $delete_secciones_ine_agendas_gobierno_puntos = "DELETE FROM secciones_ine_agendas_gobierno_dependencias_generales  WHERE  id='$id' ";
        $conexion->autocommit(FALSE);
        $delete_secciones_ine_agendas_gobierno_puntos=$conexion->query($delete_secciones_ine_agendas_gobierno_puntos);
        $num=$conexion->affected_rows;
        if(!$delete_secciones_ine_agendas_gobierno_puntos || $num=0){
	...
	$secciones_ine_agendas_gobierno_puntosDatos = secciones_ine_agendas_gobierno_puntosDatos('',$id_seccion_ine_agenda_gobierno);

	foreach ($secciones_ine_agendas_gobierno_puntosDatos as $key => $value) {
	...
	foreach ($_POST['puntos_registrados'] as $key => $value) {
		if( registrosCompara("secciones_ine_agendas_gobierno",$_POST['seccion_ine_agenda_gobierno'][0],1) ){
			$entra = true;
			$value['id_seccion_ine_agenda_gobierno'] = $_POST['seccion_ine_agenda_gobierno'][0]['id'];
			$value['id_seccion_ine'] = $_POST["seccion_ine_agenda_gobierno"][0]['id_seccion_ine'];
			$value['id_municipio'] = $_POST["seccion_ine_agenda_gobierno"][0]['id_municipio'];
			$value['id_localidad'] = $_POST["seccion_ine_agenda_gobierno"][0]['id_localidad'];
			$value['fechaR']  = $fechaH;
			...
			foreach($value as $keyPrincipal => $atributos) {
				if($keyPrincipal !='id'){
					...
					$id_seccion_ine_agenda_gobierno_punto=$atributos;
				}
			}
			echo "<pre>";
			echo $update_secciones_ine_agendas_gobierno_punto = "UPDATE secciones_ine_agendas_gobierno_puntos SET ". join(",",$valueSets) . " WHERE id=".$id_seccion_ine_agenda_gobierno_punto;
			echo "</pre>";die;
			$update_secciones_ine_agendas_gobierno_punto=$conexion->query($update_secciones_ine_agendas_gobierno_punto);
			$num=$conexion->affected_rows;
			if(!$update_secciones_ine_agendas_gobierno_punto || $num=0){
				$success=false;
				echo "<br>";
				echo "ERROR update_secciones_ine_agendas_gobierno_punto"; 
				var_dump($conexion->error);
			}

			unset($value['id']);
			$value['id_seccion_ine_agenda_gobierno_punto'] = $id_seccion_ine_agenda_gobierno_punto;
			$fields_pdo = "`".implode('`,`', array_keys($value))."`";
			$values_pdo = "'".implode("','", $value)."'";
			$inset_secciones_ine_agendas_gobierno_historicos= "INSERT INTO secciones_ine_agendas_gobierno_puntos_historicos ($fields_pdo) VALUES ($values_pdo);";
			$inset_secciones_ine_agendas_gobierno_historicos=$conexion->query($inset_secciones_ine_agendas_gobierno_historicos);
			$num=$conexion->affected_rows;
			if(!$inset_secciones_ine_agendas_gobierno_historicos || $num=0){
				$success=false;
	...
	foreach ($_POST['puntos_nuevos'] as $key => $value){ 
		...
		$fields_pdo = "`".implode('`,`', array_keys($value))."`";
		$values_pdo = "'".implode("','", $value)."'";
		$inset_secciones_ine_agendas_gobierno_puntos= "INSERT INTO secciones_ine_agendas_gobierno_puntos ($fields_pdo) VALUES ($values_pdo);";
		...
		if(!$inset_secciones_ine_agendas_gobierno_puntos || $num=0){
			$success=false;
			echo "ERROR inset_secciones_ine_agendas_gobierno_puntos"; 
			var_dump($conexion->error);
		}
		$value['id_seccion_ine_agenda_gobierno_punto']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($value))."`";
		$values_pdo = "'".implode("','", $value)."'";
		$inset_secciones_ine_agendas_gobierno_puntos_historicos= "INSERT INTO secciones_ine_agendas_gobierno_puntos_historicos ($fields_pdo) VALUES ($values_pdo);";
		...
		if(!$inset_secciones_ine_agendas_gobierno_puntos_historicos || $num=0){
			$success=false;
			echo "ERROR inset_secciones_ine_agendas_gobierno_puntos_historicos"; 
			var_dump($conexion->error);
		}
	}
	foreach ($delete_puntos as $key => $value) {
		...
		$success=true;
		$id = $value;
		$delete_secciones_ine_agendas_gobierno_puntos = "DELETE FROM secciones_ine_agendas_gobierno_puntos  WHERE  id='$id' ";
		$conexion->autocommit(FALSE);
		$delete_secciones_ine_agendas_gobierno_puntos=$conexion->query($delete_secciones_ine_agendas_gobierno_puntos);
		$num=$conexion->affected_rows;
		if(!$delete_secciones_ine_agendas_gobierno_puntos || $num=0){
