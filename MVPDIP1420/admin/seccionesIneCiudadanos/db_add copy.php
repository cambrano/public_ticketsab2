<?php
	...
	foreach($_POST["seccion_ine_ciudadano"][0] as $keyPrincipal => $atributo) {
		$_POST["seccion_ine_ciudadano"][0][$keyPrincipal]= rtrim(ltrim(($atributo)));
	}

	//metemos los valores para que se no tengamos error
	foreach($_POST["usuarios"][0] as $keyPrincipal => $atributo) {
		$_POST["usuarios"][0][$keyPrincipal]= rtrim(ltrim(($atributo)));
	}
	...
	$usuario = mysqli_real_escape_string($conexion,$_POST["usuarios"][0]['usuario']);
	$usuarioValidadorSistema=usuarioValidadorSistema($usuario,'','');
	if($usuarioValidadorSistema){
		echo "Favor de Ingresar un Usuario Válido o que no exista en sistema.";
		die;
	}
	$clave = mysqli_real_escape_string($conexion,$_POST["seccion_ine_ciudadano"][0]['clave']);
	$seccion_ine_ciudadanoClaveVerificacion=seccion_ine_ciudadanoClaveVerificacion($clave,'',1);
	...
	$clave_elector = mysqli_real_escape_string($conexion,$_POST["seccion_ine_ciudadano"][0]['clave_elector']);
	$seccion_ine_ciudadanoClaveElectorVerificacion=seccion_ine_ciudadanoClaveElectorVerificacion($clave_elector,'',1);
	...
	if(!empty($_POST)){
		...
		$id_seccion_ine = mysqli_real_escape_string($conexion,$_POST["seccion_ine_ciudadano"][0]['id_seccion_ine']);
		...
		$lista_nominalId = lista_nominalId($clave_elector);
		if($lista_nominalId != ''){
		...
		$data = $_POST["seccion_ine_ciudadano"][0];
		// Preparar la sentencia SQL con marcadores de posición
		$fields = implode('`,`', array_keys($data));
		$values = implode(',', array_fill(0, count($data), '?'));
		$insert_secciones_ine_ciudadanos = "INSERT INTO secciones_ine_ciudadanos (`$fields`) VALUES ($values)";
		// Preparar la sentencia SQL
		$stmt = $conexion->prepare($insert_secciones_ine_ciudadanos);
		if ($stmt) {
			// Vincular parámetros
			$types = ''; // String que representa los tipos de datos de los valores (s para cadena, i para entero, etc.)
			$params = array();
			...
			// Agregar el string de tipos al principio del array de parámetros
			array_unshift($params, $types);
		
			// Llamar a la función bind_param para vincular los parámetros
		...
		$fields = implode('`,`', array_keys($data));
		$values = implode(',', array_fill(0, count($data), '?'));
		$insert_secciones_ine_ciudadanos = "INSERT INTO secciones_ine_ciudadanos_historicos (`$fields`) VALUES ($values)";
		// Preparar la sentencia SQL
		$stmt = $conexion->prepare($insert_secciones_ine_ciudadanos);
		if ($stmt) {
			// Vincular parámetros
		...
		$data = $_POST["usuarios"][0];
		// Preparar la sentencia SQL con marcadores de posición
		$fields = implode('`,`', array_keys($data));
		$values = implode(',', array_fill(0, count($data), '?'));
		$insert_usuarios = "INSERT INTO usuarios (`$fields`) VALUES ($values)";
		// Preparar la sentencia SQL
		$stmt = $conexion->prepare($insert_usuarios);
		if ($stmt) {
			// Vincular parámetros
			$types = ''; // String que representa los tipos de datos de los valores (s para cadena, i para entero, etc.)
			$params = array();
		...
		$fields = implode('`,`', array_keys($data));
		$values = implode(',', array_fill(0, count($data), '?'));
		$insert_usuario = "INSERT INTO usuarios_historicos (`$fields`) VALUES ($values)";
		// Preparar la sentencia SQL
