<?php
	...
	if(strlen($_POST["seccion_ine_ciudadano"][0]['clave_elector'])  != 18){
		echo "Clave de elector, no cumple con las condiciones.";
		die;
	}

	if(strlen($_POST["seccion_ine_ciudadano"][0]['folio'])  != 4){
		//echo "folio, debe ser de 4 caracteres.";
		//die;
	}

	if(strlen($_POST["seccion_ine_ciudadano"][0]['curp'])  != 18){
		echo "CURP, no cumple con las condiciones.";
		die;
	}
	...
	$seccion_ine_ciudadanoClaveElectorVerificacion;
	if($seccion_ine_ciudadanoClaveElectorVerificacion>0){
		$seccion_ine_ciudadanoDatos = seccion_ine_ciudadanoDatos($seccion_ine_ciudadanoClaveElectorVerificacion);
		echo "Esta clave de elector ya esta en el sistema con Clave: ".$seccion_ine_ciudadanoDatos['clave']." y Folio: ".$seccion_ine_ciudadanoDatos['folio'];
		$datos = seccion_ine_ciudadanoFamilia($seccion_ine_ciudadanoDatos['id_seccion_ine_ciudadano_compartido']);
		$familia_line = "";
		if(!empty($datos)){
			$num = 0;
			...
			$familia_line .= "<tr>";
				$familia_line .= "<td style='padding:5px;background-color: #FDFD96;color:black'>".$seccion_ine_ciudadanoDatos['clave']."<td>";
				$familia_line .= "<td style='padding:5px;background-color: #FDFD96;color:black'>".$seccion_ine_ciudadanoDatos['folio']."<td>";
				$familia_line .= "<td style='padding:5px;background-color: #FDFD96;color:black'>".$seccion_ine_ciudadanoDatos['tipo_ciudadano']."<td>";
				$familia_line .= "<td style='padding:5px;background-color: #FDFD96;color:black'>".$seccion_ine_ciudadanoDatos['nombre_completo']."<td>";
				$familia_line .= "</tr>";
			$familia_line .= "</table>";
		}
		echo $familia_line;
	...
	if(!empty($_POST)){
		...
		$_POST["seccion_ine_ciudadano"][0]['medio_registro'] = 2;
		$_POST["seccion_ine_ciudadano"][0]['distancia_alert'] = 0;
		...
		$insert_secciones_ine_ciudadanos= "INSERT INTO secciones_ine_ciudadanos ($fields_pdo) VALUES ($values_pdo);";
		$conexion->autocommit(FALSE);
		$insert_secciones_ine_ciudadanos=$conexion->query($insert_secciones_ine_ciudadanos);
		$num=$conexion->affected_rows;
		if(!$insert_secciones_ine_ciudadanos || $num=0){
			$success=false;
			//echo "ERROR insert_secciones_ine_ciudadanos"; 
			if (strpos($conexion->error, "clave_elector_UNIQUE") !== false) {
				echo "Error: Clave del elector duplicada.";
			} else {
				echo "Otro tipo de error: " . $conexion->error;
			}
			die;
		}
		$id_seccion_ine_ciudadano = $id=$_POST["seccion_ine_ciudadano"][0]['id_seccion_ine_ciudadano']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($_POST["seccion_ine_ciudadano"][0]))."`";
		$values_pdo = "'".implode("','", $_POST["seccion_ine_ciudadano"][0])."'";
		$insert_secciones_ine_ciudadanos_historicos= "INSERT INTO secciones_ine_ciudadanos_historicos ($fields_pdo) VALUES ($values_pdo);";
		$insert_secciones_ine_ciudadanos_historicos=$conexion->query($insert_secciones_ine_ciudadanos_historicos);
		$num=$conexion->affected_rows;
		if(!$insert_secciones_ine_ciudadanos_historicos || $num=0){
			$success=false;
			echo "ERROR insert_secciones_ine_ciudadanos_historicos"; 
			echo "<br>";
		...
		$insert_usuarios= "INSERT INTO usuarios ($fields_pdo) VALUES ($values_pdo);";
		$conexion->autocommit(FALSE);

		$insert_usuarios=$conexion->query($insert_usuarios);
		$num=$conexion->affected_rows;
		if(!$insert_usuarios || $num=0){
			$success=false;
			echo "ERROR insert_usuarios"; 
			echo "<br>";
			var_dump($conexion->error);
			echo "<br>";echo "<br>";
		}
		...
		if($api_whatsappDatos['status'] == 1){
			...
			$resultCamSMS = $conexion->query($sqlCamSMS);  
			while($rowCampSMS=$resultCamSMS->fetch_assoc()){
				$id_campana_whatsapp = $rowCampSMS['id'];
				$tipo = $rowCampSMS['tipo'];

				$campana_whatsapp_cartografiaDatos=campana_whatsapp_cartografiaDatos('',$id_campana_whatsapp);
				$campanas_whatsapp_tipos_ciudadanosIdDatos=campanas_whatsapp_tipos_ciudadanosIdDatos('',$id_campana_whatsapp); 
				$campana_whatsapp_cuerpoDatos=campana_whatsapp_cuerpoDatos('',$id_campana_whatsapp);
				// verificar tipo de ciudadano
				// tipo de cartografia

				//var_dump($campana_whatsapp_cartografiaDatos);
				$tipo_cartografia = true;
				if($campana_whatsapp_cartografiaDatos['tipo_cartografia']=='municipios'){
					if($campana_whatsapp_cartografiaDatos['id_tipo_cartografia']!=$id_municipio){
						$tipo_cartografia = false;
					}
				}elseif($campana_whatsapp_cartografiaDatos['tipo_cartografia']=='secciones_ine'){
					if($campana_whatsapp_cartografiaDatos['id_tipo_cartografia']!=$id_seccion_ine){
						$tipo_cartografia = false;
					}
				}elseif($campana_whatsapp_cartografiaDatos['tipo_cartografia']=='distritos_locales'){
					if($campana_whatsapp_cartografiaDatos['id_tipo_cartografia']!=$id_distrito_local){
						$tipo_cartografia = false;
					}
				}
				elseif($campana_whatsapp_cartografiaDatos['tipo_cartografia']=='distritos_federales'){
					if($campana_whatsapp_cartografiaDatos['id_tipo_cartografia']!=$id_distrito_federal){
						$tipo_cartografia = false;
					}
				}else{
					$tipo_cartografia = true;
				...
				if(!empty($campanas_whatsapp_tipos_ciudadanosIdDatos)){
					if(empty($campanas_whatsapp_tipos_ciudadanosIdDatos[$id_tipo_ciudadano])){
						$tipo_ciudadano = false;
					}
				}
				...
				if($tipo_cartografia && $tipo_ciudadano){
					//envia 
					unset($campana_whatsapp);
					$campana_whatsapp['id_seccion_ine_ciudadano'] = $id;
					$campana_whatsapp['id_seccion_ine'] = $id_seccion_ine;
					$campana_whatsapp['id_distrito_local'] = $id_distrito_local;
					$campana_whatsapp['id_distrito_federal'] = $id_distrito_federal;
					$campana_whatsapp['id_estado'] = $id_estado;
					$campana_whatsapp['id_municipio'] = $id_municipio;
					$campana_whatsapp['id_campana_whatsapp'] = $id_campana_whatsapp;
					$campana_whatsapp['id_campana_whatsapp_cuerpo'] = $campana_whatsapp_cuerpoDatos['id'];
					$campana_whatsapp['status'] = 0 ;
					$campana_whatsapp['fechaR'] = $fechaH;
					$campana_whatsapp['codigo_plataforma'] = $codigo_plataforma;
					$campana_whatsapp['codigo_seccion_ine_ciudadano'] = $codigo_seccion_ine_ciudadano;
					$campana_whatsapp['identificador'] = 1;
					$campana_whatsapp['fecha_registro'] = $fechaSF;
					$campana_whatsapp['hora_registro'] = $fechaSH;
					$campana_whatsapp['fecha_hora_registro'] = $fechaH;
					$campana_whatsapp['tipo'] = $tipo;

					$fields_pdo = "`".implode('`,`', array_keys($campana_whatsapp))."`";
					$values_pdo = "'".implode("','", $campana_whatsapp)."'";
					$insert_secciones_ine_ciudadanos_campanas_whatsapp_programadas= "INSERT INTO secciones_ine_ciudadanos_campanas_whatsapp_programadas ($fields_pdo) VALUES ($values_pdo);";
					$conexion->autocommit(FALSE);
					$insert_secciones_ine_ciudadanos_campanas_whatsapp_programadas=$conexion->query($insert_secciones_ine_ciudadanos_campanas_whatsapp_programadas);
					$num=$conexion->affected_rows;
					if(!$insert_secciones_ine_ciudadanos_campanas_whatsapp_programadas || $num=0){
						$success=false;
						echo "ERROR secciones_ine_ciudadanos_campanas_whatsapp_programadas"; 
						var_dump($conexion->error);
					}
				}
			}
