	...
	<script type="text/javascript">
		function myMap(coordenadas=null,zoomCoordenada=null) {
			...
			mover_pointer = document.getElementById("mover").value;
			if(mover_pointer ==1 ){
			...
			google.maps.event.addListener(marker, "dragend", function() { 
							getCoords(marker); 
							document.getElementById("mover").value=1; 
			});
			...
			foreach ($manzanas_ine_parametrosDatosMapa1 as $key => $value) {
				...
				*/
				...
				const centerManzana<?= $key ?> = { lat: <?= $manzanas_ineDatosMapa[$key]['latitud'] ?>, lng: <?= $manzanas_ineDatosMapa[$key]['longitud'] ?> };
				labelManzana<?= $key ?>.setMap(map);
				labelManzana<?= $key ?>.setPosition(centerManzana<?= $key ?>);
				<?php
			}
			?>
			<?php
