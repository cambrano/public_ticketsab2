    ...
    if(!empty($_POST['mapa'][0])){
		...
		$punto[0] = $_POST['mapa'][0]['location']['lat'];
		$punto[1] = $_POST['mapa'][0]['location']['lng'];
		foreach ($secciones_ine_parametrosDatosMapa as $key => $value) {
			$poligono = $value[0];
			if (puntoPoligono($punto, $poligono)){
				$id_seccion_ine = $key;
				break;
			}
		}
	...
	<script type="text/javascript">
		function myMapManzana(coordenadas=null,zoomCoordenada=null) {
			...
			<?php
			foreach ($secciones_ine_parametrosDatosMapa1 as $key => $value) {
			...
			?>
			<?php
			foreach ($manzanas_ine_parametrosDatosMapa1 as $key => $value) {
			...
			foreach ($manzanas_ineDatosMapa as $key => $value) {
				...
				$div = '<div class="divMapaSecciones">
							...
							<div class="datos_seccion">
								<p>
									Sección: <b>'.$value['clave_seccion_ine'].'</b><br>
									Manzana: <b>'.$value['numero'].'</b><br>
									<button class="btn btn-primary" onclick="manzanaSelect('.$value['id'].')">Asignar</button>
								</p>
								<p>
				...
				infoWindowSeccion = new google.maps.InfoWindow();
				manzanas_areaSeccion<?= $key ?>.addListener("click", (function(event){
					myLatlngSeccion = new google.maps.LatLng("<?= $value['latitud'] ?>","<?= $value['longitud'] ?>"); 
					infoWindowSeccion.setContent('<?= $div ?>');
					infoWindowSeccion.setPosition(myLatlngSeccion);
					infoWindowSeccion.open(mapSeccion);
				}));
			...
			if($punto[0]!=''){
				...
				const center = { lat: <?= $punto[0] ?>, lng: <?= $punto[1] ?> };
	...
	<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBBrai7GSb0T1XxAG4yOFZwnWOcHXCzNaI&callback=myMapManzana"></script>
	<script type="text/javascript">
		$(".loader").fadeOut(2000);
	</script>