<?php
	if($reload_mapa == ""){
		...
		if(!empty($_POST)){
			...
			include '../functions/tipos_ciudadanos.php';
			...
			include '../functions/secciones_ine_ciudadanos_avance_semaforo.php';
			include '../functions/secciones_ine_ciudadanos_secciones_avance_semaforo.php';
			include '../functions/tipos_ciudadanos_secciones_avance_semaforo.php';
			$tipos_ciudadanos_secciones_avance_semaforoDatos = tipos_ciudadanos_secciones_avance_semaforoDatos();
			...
			$id_tipo_ciudadanox = explode(",", $_POST['searchTable'][0]['id_tipo_ciudadano']);
			$id_tipo_ciudadano = $id_tipo_ciudadanox[0];
			...
			$tipos_ciudadanos_secciones_ineDatos = tipos_ciudadanos_secciones_ineDatos('',1,'');
			$tipos_ciudadanosDatos = tipos_ciudadanosDatos('',1);
			...
			if($posicion_encontrada == 0 && !empty($id_secciones_ine) ){
				foreach ($id_secciones_ine as $valor) {
					// Verificar si el valor existe como clave en $secciones_ineDatosMapa
					if (array_key_exists($valor, $secciones_ineDatosMapa)) {
						// Si existe, acceder a los datos de esa sección
						$datos_seccion = $secciones_ineDatosMapa[$valor];
						$latitud = $datos_seccion['latitud'];
						$longitud = $datos_seccion['longitud'];
					}
				}
			}
					...
					function myMap(){
						...
						<?php
							foreach ($secciones_ineDatosMapa as $key => $value) {
								...
								$div_tipos_ciudadadanos = "";
								foreach ($tipos_ciudadanosDatos as $keyTC => $valueTC) {
									$div_tipos_ciudadadanos .= "<tr>";
									if($valueTC['id']==$id_tipo_ciudadano){
										$semaforo_tipo = $valueTC['nombre'];
										$variable_encontrar = $tipos_ciudadanos_secciones_ineDatos[$value['id']][$valueTC['id']]['total'];
									}
									$div_tipos_ciudadadanos .= '<td style="border-bottom:1px solid black" >'.$valueTC['nombre']."</td>";
									$div_tipos_ciudadadanos .= '<td style="border-bottom:1px solid black" >'.number_format($tipos_ciudadanos_secciones_ineDatos[$value['id']][$valueTC['id']]['total'],0,'.',',')."</td>";
								...
								if($tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['status']==1){
									$semaforo_activo = true;
									$rango_verde_inicial = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['verde_rango_inicial'];
									$rango_verde_final = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['verde_rango_final'];

									$rango_amarillo_inicial = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['amarillo_rango_inicial'];
									$rango_amarillo_final = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['amarillo_rango_final'];

									$rango_rojo_inicial = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['rojo_rango_inicial'];
									$rango_rojo_final = $tipos_ciudadanos_secciones_avance_semaforoDatos[$key][$id_tipo_ciudadano]['rojo_rango_final'];

									if (
										intval($variable_encontrar) == intval($rango_verde_inicial) || 
										(
											intval($variable_encontrar) >= intval($rango_verde_inicial) && 
											(
												intval($variable_encontrar) <= floatval($rango_verde_final) || 
												floatval($rango_verde_final) <= intval($variable_encontrar)
											)
										)
									) {
										$strokeColor='#008000';
								...
								if($semaforo_activo){
									...
									<div style="text-align: center;">
										<b>Semáforo<br>'.$semaforo_tipo.':'.$variable_encontrar.'</b>
									</div>
								...
								$div = '<div class="divMapaSecciones">
											...
											<div class="datos_seccion">
												<p>
													...
													<br>
													Manzanas: <b>'.number_format($value['manzanas'],0,'.',',').'</b><br>
													<table style="width:90%;text-align: left;">
													<tr>
														<td colspan=2 style="border-bottom:1px solid black;text-align: center;"><b>Tipos Ciudadanos</b></td>
													</tr>
													'.$div_tipos_ciudadadanos.'
													</table>
												</p>
