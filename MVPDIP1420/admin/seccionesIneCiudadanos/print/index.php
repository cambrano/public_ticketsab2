<?php
	...
	foreach ($documentos_tipo as $key => $value) {
		...
		foreach ($value['files'] as $tipo => $name) {
			$mostrarImagenBase64 = mostrarImagenBase64($name['name']);
			$image = "data:image/png;base64,".$mostrarImagenBase64;
			if($name['show']==1){
				if($value['datos']['tipo']=='ine'){
					$tabla_documentos_oficiales .= '<img style="border-radius: 8px;padding: 5px;" src="'.$image.'" width="220">';
				}else{
					$tabla_documentos_oficiales .= '<img style="border-radius: 8px;padding: 5px;" src="'.$image.'" width="560">';
				}
			}
		}
		$tabla_documentos_oficiales .= "</center>";
	...
	$documentos_oficiales = array(
		"[__Tabla_Documentos_Oficiales__]" => $tabla_documentos_oficiales1,
	);
