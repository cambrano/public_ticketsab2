...
?>
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_ciudadanos_check_2024-tabla').DataTable( {
				"destroy": true,
				...
				"ajax":{
					url :"seccionesIneCiudadanosCheck2024/secciones_ine_ciudadanos_check_2024.php", // json datasource
					type: "post",  // method  , by default get
					data: {
						postData: <?php echo $postData; ?>
					},
					error: function(){  // error handling
						$(".secciones_ine_ciudadanos_check_2024-tabla-error").html("");
						$("#secciones_ine_ciudadanos_check_2024-tabla").append('<tbody class="secciones_ine_ciudadanos_check_2024-tabla-error"><tr><th colspan="8">Registros no encontrados</th></tr></tbody>');
						$("#secciones_ine_ciudadanos_check_2024-tabla_processing").css("display","none");
						
					}
				}
			});

			$('#secciones_ine_ciudadanos_check_2024-tabla').on( 'order.dt', function () {
				var table = $('#secciones_ine_ciudadanos_check_2024-tabla').dataTable();
				var api = table.api();
				...
				*/
				...
				$.ajax({
					type: "POST",
					url: "seccionesIneCiudadanosCheck2024/mapa.php",
			...
			$('#secciones_ine_ciudadanos_check_2024-tabla').css( 'display', 'table' );
			$('#secciones_ine_ciudadanos_check_2024-tabla').resize();
			$('#secciones_ine_ciudadanos_check_2024-tabla').DataTable().columns.adjust().responsive.recalc();
			$("#secciones_ine_ciudadanos_check_2024-tabla_filter").css("display","none");  // hiding global search box
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="seccionesIneCiudadanosCheck2024/index.php?cot="+ids_string;   
					var link2="seccionesIneCiudadanosCheck2024/index.php";
			...
			$('#secciones_ine_ciudadanos_check_2024-tabla').on( 'page.dt', function () {
		...
		function entrega(value){
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanosCheck2024/entrega_ciudadano.php",
		...
		function recibe(value){
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanosCheck2024/recibe_ciudadano.php",
				data: {casilla_voto: casilla_voto},
				async: true,
				success: function(data) {
					var res = data.substr(0,2);
		...
		function downloadExcel(){
			...
			*/
			...
			form.method = 'POST';
			form.action = 'seccionesIneCiudadanosCheck2024/excel/index.php?cot=<?=$_COOKIE['pageService']?>'; // URL de destino
	...
	<table id="secciones_ine_ciudadanos_check_2024-tabla" class="table table-striped table-bordered  cell-border compact stripe" style="width:100%">
		<thead>
			<tr>
				<th>Clave</th>
				<th>Sección</th>
