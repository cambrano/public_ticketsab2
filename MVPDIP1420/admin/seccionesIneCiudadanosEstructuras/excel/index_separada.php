	...
	<script>
		...
		var data = { 
			...
			foreach ($_POST as $key => $value) {
				if($value !="[object HTMLSelectElement]" && $value != ''){
					$miStringSinPrefijo = str_replace("form_excel_", "", $key);
					?>
					<?= $miStringSinPrefijo ?> : '<?= $value ?>',
					<?php
				}
			}
			?>
		...
		$.ajax({
			type: "POST",
			url: "excel_separada.php",
			data: {excel: excel,pageService : '<?= $_COOKIE['pageService'] ?>' },
