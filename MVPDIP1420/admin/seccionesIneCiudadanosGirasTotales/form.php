...
?>
	<script type="text/javascript">
		...
		function buscar_clave_electoral(){
			...
			var dataString = 'clave_elector='+clave_elector+'&id_seccion_ine_gira='+id_seccion_ine_gira;
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanosGirasTotales/search_clave_elector.php",
				data: dataString,
				dataType: 'json',
				success: function(data) {
					console.log(data);
					...
					if(data.contador==0){
						if(data.status_verificacion==false){
							...
							$("#mensaje_ine_disponible").html("Este ciudadano no esta en la gira.");
							document.getElementById("mensaje_ine_disponible").classList.add("mensajeSucces");
						}
					}else{
						document.getElementById("mensaje_ine_disponible").classList.add("mensajeError");
						$("#mensaje_ine_disponible").html("Este ciudadano ya esta registrado en esta gira.");
						document.getElementById("mensaje_ine_disponible").classList.add("mensajeError");
					}
	...
	<div style=" width: 100%; display:inline-block; text-align: left;">
		...
		<div class="sucFormTitulo">
			<label class="labelForm" id="labeltemaname">Datos Ciudadanos</label>
		</div>
		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Clave Electoral<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" type="text"  name="clave_elector" autocomplete="off"  id="clave_elector" value="<?= $seccion_ine_ciudadano_giraDatos['clave_elector'] ?>" placeholder="" onblur="aMays(event, this)" size="40" maxlength="18"/><br>
		</div>
		...
		<div class="sucForm">
			...
			<input class="inputlogin" <?= $claveF['input'] ?> type="text" style="width: 100%" name="clave" autocomplete="off"  id="clave" value="<?= $seccion_ine_ciudadano_giraDatos['clave'] ?>" placeholder="Clave" onkeyup="clave(this.value)" /><br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Folio<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" type="text" name="folio" autocomplete="off"  id="folio" value="<?= $seccion_ine_ciudadano_giraDatos['folio'] ?>" onkeyup="aMays(event, this)" /><br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Fecha<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" type="text" name="fecha" autocomplete="off"  id="fecha" value="<?= $seccion_ine_ciudadano_giraDatos['fecha'] ?>" placeholder="" /><br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Hora<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" type="text" name="hora" autocomplete="off"  id="hora" value="<?= $seccion_ine_ciudadano_giraDatos['hora'] ?>" placeholder="" /><br>
		</div>

		<div class="sucForm" style="width: 100%">
			<label class="labelForm" id="labeltemaname">Observaciones</label><br>
			<textarea id="observaciones" style="width: 99%;height: 150px"><?= $seccion_ine_ciudadano_giraDatos['observaciones'] ?></textarea> <br>
		</div>
