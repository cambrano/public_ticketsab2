<?php
	...
	if(!empty($_POST)){
		...
		@session_start();
		$clave_elector = $_POST['search_clave_elector'];
		...
		$sqlClave = "SELECT count(*) contador FROM secciones_ine_ciudadanos_grupos WHERE id_seccion_ine_ciudadano = '{$id_seccion_ine_ciudadano}' ";
		$resultClave = $conexion->query($sqlClave);
		$rowClave=$resultClave->fetch_assoc();
		
		$myObj->disponible = $rowClave['contador'];
