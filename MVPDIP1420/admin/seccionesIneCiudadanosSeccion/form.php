<?php
	...
	if(!isMobileDevice()){
		...
		#$manzanas_ineDatosMapa = manzanas_ineDatosMapaForm($origen);
		#$manzanas_ine_parametrosDatosMapa = manzanas_ine_parametrosDatosMapa('','','',$id_municipio,$id_distrito_local,$id_distrito_federal,'','');
	}else {
		// dispositivo mobil
		if ($tipo_uso_plataforma != 'all'){
			...
			// si es dispositivomobil y tiene all o lo mostrara
			///$manzanas_ineDatosMapa = manzanas_ineDatosMapaForm($origen);
			///$manzanas_ine_parametrosDatosMapa = manzanas_ine_parametrosDatosMapa('','','',$id_municipio,$id_distrito_local,$id_distrito_federal,'','');
		}
	}
?>
	...
	<script type="text/javascript">
		...
		async function bucleFamilia(clave_elector) {
			...
			document.getElementById("valid_ine_btn").disabled = true;
		...
		function crearTablaConDatos(dataArray, primera_data) {
			...
			document.getElementById("mensaje_ine_tabla").classList.add("mensajeError");
			document.getElementById("mensaje_ine_tabla").style.setProperty("padding", "5px", "important");
			document.getElementById("mensaje_ine_disponible").classList.add("mensajeError");
			document.getElementById("mensaje_ine_disponible").style.setProperty("padding", "5px", "important");
			
			$("#mensaje_ine_tabla").html(tableHTML);
			$("#mensaje_ine_disponible").html("No Disponible, Este ciudadano está en el sistema. Con Clave: " +primera_data.clave + ' y Folio: ' + primera_data.folio);
		}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(data.disponible==true){
						...
						document.getElementById("mensaje_ine_disponible").classList.add("mensajeError");
						$("#mensaje_ine_disponible").html("No Disponible, Este ciudadano esta en el sistema. con Clave: "+data.clave+' y Folio: '+data.folio+'<br>'+data.familia);
					}
				}
			});
		}
	...
	<div style=" width: 100%; display:inline-block; text-align: left;">
		...
		<div id="divDatosCiudadano" >
			...
			<div class="sucForm">
				...
				<input class="inputlogin" type="text" name="clave" autocomplete="off" <?= $claveF['input'] ?> id="clave" value="<?= $seccion_ine_ciudadanoDatos['clave'] ?>" onkeyup="clave(this.value)" /><br>
			</div>
			<div class="sucForm">
				<label class="labelForm" id="labeltemaname">Folio<font color="#FF0004">*</font></label><br>
				<input class="inputlogin" type="text" name="folio" autocomplete="off"  id="folio" value="<?= $seccion_ine_ciudadanoDatos['folio'] ?>" onkeyup="aMays(event, this)" /><br>
			</div>
			<div class="sucForm" style="width: 100%"></div>
			<div class="sucForm">
				<label class="labelForm" id="labeltemaname">Clave Electoral<font color="#FF0004">*</font></label><br>
				<input class="inputlogin" type="text"  name="clave_elector" autocomplete="off"  id="clave_elector" value="<?= $seccion_ine_ciudadanoDatos['clave_elector'] ?>" placeholder="" onblur="aMays(event, this)" size="40" maxlength="19"/><br>
			</div>
			<div class="sucForm" style="width: 100%">
				<input type="button" id=valid_ine_btn onclick="bucleFamilia()" value="Buscar Datos INE"><input type="hidden" id="valid_ine">
			</div> 
			<div class="sucForm">
			...
			<div class="sucForm" style="width: 100%; background-color: #e3edfc;padding: 20px;">
				...
				<div id="busqueda_clave_electoral">
					...
					<div class="data_interior">
						...
						No. Exterior: <b id="ine_no_exterior"><?= $row['num_ext'] =='' ? 'No Encontrado' : $row['num_ext'] ?></b><br>
						No. Interior: <b id="ine_no_interior"><?= $row['num_int'] =='' ? 'No Encontrado' : $row['num_int'] ?></b><br>
		...
		<div id="divDatosDireccionActual" style="display:none">
			...
			<div class="sucForm" style="width: 100%">
				...
				<input type="hidden" name="mover" id="mover" value="" placeholder="">
