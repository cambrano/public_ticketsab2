<?php
	...
	echo $redirectSecurity=redirectSecurity($seccion_ine_ciudadano_seccion_avance_semaforoDatos['id'],'secciones_ine_ciudadanos_secciones_avance_semaforo','seccionesIneCiudadanosSeccionesAvanceSemaforo','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('secciones_ine_ciudadanos_secciones_avance_semaforo',$seccion_ine_ciudadano_seccion_avance_semaforoDatos['id']);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			//var dataString = 'id=<?= $seccion_ine_ciudadano_seccion_avance_semaforoDatos['id']?>';
			var dataString = 'id=<?= $seccion_ine_ciudadano_seccion_avance_semaforoDatos['id']?>';
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanosSeccionesAvanceSemaforo/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("mensaje").classList.remove("mensajeError"); 
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Semáforo en sección : <?= $seccion_ineDatos['numero'] ?></font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Seccion semáforo en sección</label><br>
			<label class="descripcionForm">
				<strong><?= $tipo_casillaDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
