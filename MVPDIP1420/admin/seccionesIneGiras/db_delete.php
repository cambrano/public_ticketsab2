<?php
	...
	if(!empty($_POST)){
		...
		if(!$delete_secciones_ine_giras_puntos || $num=0){
			$success=false;
			echo "ERROR delete secciones_ine_giras_puntos"; 
			echo "<br>";
			echo("Errorcode: " . mysqli_errno($conexion));
			echo "<br>";
		}

		$delete_secciones_ine_giras_dependencias = "DELETE FROM secciones_ine_giras_dependencias  WHERE  id_seccion_ine_gira='$id' AND id<>0 ";
		$delete_secciones_ine_giras_dependencias=$conexion->query($delete_secciones_ine_giras_dependencias);
		$num=$conexion->affected_rows;
		if(!$delete_secciones_ine_giras_dependencias || $num=0){
			$success=false;
			echo "ERROR delete secciones_ine_giras_dependencias"; 
			echo "<br>";
			echo("Errorcode: " . mysqli_errno($conexion));
			echo "<br>";
		}

		$delete_secciones_ine_giras_dependencias_generales = "DELETE FROM secciones_ine_giras_dependencias_generales  WHERE  id_seccion_ine_gira='$id' AND id<>0 ";
		$delete_secciones_ine_giras_dependencias_generales=$conexion->query($delete_secciones_ine_giras_dependencias_generales);
		$num=$conexion->affected_rows;
		if(!$delete_secciones_ine_giras_dependencias_generales || $num=0){
			$success=false;
			echo "ERROR delete secciones_ine_giras_dependencias_generales"; 
			echo "<br>";
			echo("Errorcode: " . mysqli_errno($conexion));
			echo "<br>";
		}
