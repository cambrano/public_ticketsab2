<?php
	...
	$id_partido_2016 = $configuracion_matriz_rentabilidad_secciones_ine_2016Datos['id_partido_2016_distrito_federal'];// Partidos 2016 PRI
	//$id_partido_2016 = $configuracion_matriz['id_partido_2016'] = '1';// Partidos 2016
	$id_partido_legado = $configuracion_matriz_rentabilidad_secciones_ine_2016Datos['id_partido_legado'];// Partidos Legados
	#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2016Datos['tipo_eleccion'] = '0';// 0 - Ayuntamiento | 1 - Municipio | 2 - Distrito Federal
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2016 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2016 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2016 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2016 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) total_lista_nominal,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2016 WHERE id_distrito_federal={$id_distrito_federal} AND tipo =  '{$tipo}' )t
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_distrito_federal_2016_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_distrito_federal_2016_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
