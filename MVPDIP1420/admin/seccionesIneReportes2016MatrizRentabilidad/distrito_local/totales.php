<?php
	...
	$id_partido_2016 = $configuracion_matriz_rentabilidad_secciones_ine_2016Datos['id_partido_2016_distrito_local'];// Partidos 2016 PRI
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2016 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2016 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2016 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2016 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) total_lista_nominal,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2016 WHERE id_distrito_local={$id_distrito_local} AND tipo =  '{$tipo}' )t
