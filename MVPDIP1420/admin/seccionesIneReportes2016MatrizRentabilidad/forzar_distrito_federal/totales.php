<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2016 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2016 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_municipio={$id_municipio} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2016 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2016 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) total_lista_nominal,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2016 WHERE id_municipio={$id_municipio} AND tipo =  '{$tipo}' )t
	";
	//echo "<pre>".$sql."</pre>";
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_forzar_distrito_federal_2016_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_forzar_distrito_federal_2016_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
