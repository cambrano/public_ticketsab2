	...
	if(!empty($_POST)){
		...
		*/
		...
		if($id_distrito_local!=''){
			$sqlDisitritoLocal = " AND cvp.id_distrito_local IN ({$id_distrito_local}) ";
		}
		$sqlDisitrito=$sqlDisitritoLocal.$sqlDisitritoFederal;
		$sql="
			SELECT
				p.id,
				p.clave,
			...
			WHERE cvp.id_distrito_federal='{$id_distrito_federal}' AND cvp.tipo = '{$tipo}' {$sqlPartidos} {$sqlMunicipio} {$sqlDisitrito}
