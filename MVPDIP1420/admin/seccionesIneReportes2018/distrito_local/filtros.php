...
?>
	<script type="text/javascript">
		function searchTable(value){
			...
			if(tipo_seccion==""){
				$('#secciones_reportes-tabla').DataTable().column(12).search(tipo_seccion).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+tipo_seccion+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(12).search(tipo_seccion_array_table.join('|'), true, false).draw();
			}
			...
			if(id_distrito_local==""){
				$('#secciones_reportes-tabla').DataTable().column(4).search(id_distrito_local).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_seccion_ine+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(4).search(id_distrito_local_array_table.join('|'), true, false).draw();
			}
			...
			if(id_distrito_federal==""){
				$('#secciones_reportes-tabla').DataTable().column(5).search(id_distrito_federal).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_seccion_ine+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(5).search(id_distrito_federal_array_table.join('|'), true, false).draw();
			}
			...
			*/
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2018/distrito_local/mapa.php",
				data: {searchTable: searchTable,mapa:mapa},
				async: true,
				success: function(data) {
					$("#mapaLoad").html(data);
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2018/distrito_local/totales.php",
				data: {searchTable: searchTable,mapa:mapa},
				async: true,
				success: function(data) {
					$("#totalesLoad").html(data);
				}
			});
		}
	</script>
	...
	<div style="width: 100%;">
		<button class="btn btn-info"  onClick="verMasDistritoLocalMatrizRentabilidad2018()">
			<span>Matriz Rentabilidad</span>
		</button>
	</div>
	...
	<div class="sucForm" style="display:1">
