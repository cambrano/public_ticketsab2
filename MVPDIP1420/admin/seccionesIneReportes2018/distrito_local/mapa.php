	...
	if(!empty($_POST)){
		...
		*/
		...
		if($id_distrito_federal!=''){
			$sqlDisitritoFederal = " AND cvp.id_distrito_federal IN ({$id_distrito_federal}) ";
		}
		...
		$sql="
			...
			WHERE cvp.id_distrito_local='{$id_distrito_local}' AND cvp.tipo = '{$tipo}' {$sqlPartidos} {$sqlMunicipio} {$sqlDisitrito}
