<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) total_lista_nominal,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_local={$id_distrito_local} {$sql_secciones_ine_ciudadanos} ) ciudadanos_totales,

				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_local={$id_distrito_local} {$sql_programas_apoyos} ) apoyos_programas,
				(SELECT COUNT(*) FROM secciones_ine_actividades sia LEFT JOIN secciones_ine si ON sia.id_seccion_ine = si.id WHERE si.id_distrito_local={$id_distrito_local} {$sql_actividades} ) acciones_obras,
				(SELECT COUNT(*) FROM secciones_ine_grupos sig LEFT JOIN secciones_ine si ON sig.id_seccion_ine = si.id WHERE si.id_distrito_local={$id_distrito_local} {$sql_grupos} ) grupos_interes,
				...
				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_local = {$id_distrito_local} AND sig.tipo = 'junta' {$sql_giras}  ) juntas,
				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_local = {$id_distrito_local} AND sig.tipo = 'visita' {$sql_giras}  ) visitas,
				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_local = {$id_distrito_local} AND sig.tipo = 'caminata' {$sql_giras}  ) caminatas

			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo =  '{$tipo}' )t
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_distrito_local_2018_'.$id_distrito_local.'-'.$_COOKIE["id_usuario"].'.json';
	...
	$sql = "SELECT 
			...
			prm.id,
			(SELECT SUM(cvrm.votos_nulos) FROM casillas_votos_2022_revocacion_mandato cvrm WHERE cvrm.id_distrito_local='{$id_distrito_local}' ) votos_nulos,
			(SELECT SUM(cvrm.votos) FROM casillas_preguntas_2022_revocacion_mandato cvrm WHERE cvrm.id_pregunta_2022_revocacion_mandato = prm.id AND cvrm.id_distrito_local='{$id_distrito_local}' ) votos
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_distrito_local_2018_'.$id_distrito_local.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
