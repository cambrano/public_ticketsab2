<?php
	...
	$perido_fecha_inicial = $_SESSION['matriz_rentabilidad_periodo'][$tipo][$ano]['fecha_inicial'];
	$perido_fecha_final = $_SESSION['matriz_rentabilidad_periodo'][$tipo][$ano]['fecha_final'];
	...
	if(!empty($_POST)){
		...
		$id_partido_2018 = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['id_partido_2018_distrito_federal'];// Partidos 2018 PRI
		//$id_partido_2018 = $configuracion_matriz['id_partido_2018'] = '1';// Partidos 2018
		$id_partido_legado = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['id_partido_legado'];// Partidos Legados
		#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['tipo_eleccion'] = '1';// 0 - Ayuntamiento | 1 - Distrito Federal | 2 - Distrito Federal
		...
		*/
		...
		$sql="
			SELECT
				...
				main.latitud,
				main.longitud
			FROM distritos_federales main
			WHERE 1 
		";
		if($id_distrito_federal !=''){
			#$sql .= " AND main.id = {$id_distrito_federal} ";
		}
		...
		$sql="
			SELECT
				...
				p.principal,
				cvp.id_distrito_federal,
				cvp.id_seccion_ine
			FROM  casillas_votos_partidos_2018 cvp
			LEFT JOIN partidos_2018 p
			ON p.id = cvp.id_partido_2018
			WHERE cvp.id_distrito_federal='{$id_distrito_federal}' AND cvp.tipo = '{$tipo}' {$sqlPartidos} {$sqlMunicipio}
			GROUP BY cvp.id_seccion_ine,cvp.id_partido_2018
		";
		$result = $conexion->query($sql);
		...
		$sql="SELECT 
			...
			WHERE cprm.id_distrito_federal = '{$id_distrito_federal}'  {$sqlPartidosRM2019} {$sqlMunicipioRM2019}
		...
		$sql="
			SELECT
				...
				(SELECT GROUP_CONCAT(DISTINCT ls.localidad SEPARATOR ' - ' ) FROM localidades_secciones_ine ls WHERE ls.seccion = si.clave AND ls.id_estado='{$id_estado}' ) seccion_localidades,
				(SELECT COUNT(cv.id) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id AND cv.tipo='{$tipo}') casillas,
				(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id AND cv.tipo='{$tipo}' ) lista_nominal,
				(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id AND cv.tipo='{$tipo}') votos_nulos,
				(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id AND cv.tipo='{$tipo}') votos_can_nreg,
				(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2018 cv WHERE cv.id_seccion_ine = si.id AND cv.tipo='{$tipo}') votos_validos,

				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic WHERE sic.id_distrito_federal={$id_distrito_federal} AND sic.id_seccion_ine = si.id  {$sqlCiudadanosRegistrados} ) ciudadanos_registrados,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} AND sic.id_seccion_ine = si.id  {$sqlApoyosProgramas} ) apoyos_programas,
				...
				(SELECT COUNT(*) FROM militantes_partidos mp LEFT JOIN secciones_ine_ciudadanos sic ON mp.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} AND mp.id_partido_legado = '{$id_partido_legado}' AND sic.id_seccion_ine = si.id {$sqlMilitantes} ) militantes,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_categorias sicc WHERE sicc.id_distrito_federal={$id_distrito_federal} AND sicc.id_tipo_categoria_ciudadano = '{$id_tipo_categoria_ciudadano}' AND sicc.id_seccion_ine = si.id  {$sqlFuncionarios} ) funcionarios,
		...
		while($row=$result->fetch_assoc()){
			...
			$datos_distritos_federales[$id_distrito_federal]['latitud'] = $row['latitud'];
			$datos_distritos_federales[$id_distrito_federal]['longitud'] = $row['longitud']; 
		...
		*/
		...
		$sql="
			...
			LEFT JOIN partidos_2018 p
			ON p.id = cvp.id_partido_2018
			WHERE cvp.id_distrito_federal='{$id_distrito_federal}' AND cvp.tipo = '{$tipo}'
			GROUP BY cvp.id_seccion_ine,cvp.id_partido_2018
		";
		...
		$sql="SELECT 
			...
			LEFT JOIN preguntas_2022_revocacion_mandato prm
			ON cprm.id_pregunta_2022_revocacion_mandato = prm.id
			WHERE cprm.id_distrito_federal = '{$id_distrito_federal}'
			GROUP BY cprm.id_seccion_ine,id_pregunta_2022_revocacion_mandato
			ORDER BY cprm.id_seccion_ine,SUM(cprm.votos) DESC
			";
		$result = $conexion->query($sql);
		...
		$_SESSION['reporte_Sistema']['competitividad_rentabilidad_individual'] = $competitividad_rentabilidad_individual;
		$_SESSION['reporte_Sistema']['competitividad_rentabilidad_totales'] = $competitividad_rentabilidad_totales;
		//$_SESSION['reporte_Sistema']['competitividad_rentabilidad'] = $datos_secciones_ine;
	}

	$orden = 1;
	$competitividad_rentabilidad_individual = $_SESSION['reporte_Sistema']['competitividad_rentabilidad_individual'];
	...
	if(empty($_POST)){
		$_SESSION['reporte_Sistema']['data'] = $datos_secciones_ine;
	}
	//$_SESSION['reporte_Sistema']['pdf']['secciones'] = $datos_secciones_ine;
	...
	$columnas_titulos = array(
		...
		1 => array('row' => 'numero' ,'nombre' => 'Sección' ,'tipo' => 'integer', 'fill' =>'#397CB5'),
		2 => array('row' => 'tipo' ,'nombre' => 'Tipo Sección' ,'tipo' => 'string', 'fill' =>'#397CB5'),

		3 => array('row' => 'primera_fuerza' ,'nombre' => 'Primera Fuerza' ,'tipo' => 'string', 'fill' =>'#213415'),
		4 => array('row' => 'votos_primera_fuerza' ,'nombre' => 'Votos Primera Fuerza' ,'tipo' => 'integer', 'fill' =>'#213415'),

		5 => array('row' => 'segunda_fuerza' ,'nombre' => 'Segunda Fuerza' ,'tipo' => 'string', 'fill' =>'#355b7d'),
		6 => array('row' => 'votos_segunda_fuerza' ,'nombre' => 'Votos Segunda Fuerza' ,'tipo' => 'integer', 'fill' =>'#355b7d'),

		7 => array('row' => 'sistema' ,'nombre' => 'Sistema' ,'tipo' => 'string', 'fill' =>'#8B8000'),
		8 => array('row' => 'votos_sistema' ,'nombre' => 'Votos Sistema' ,'tipo' => 'integer', 'fill' =>'#8B8000'),
		//? partidps
		9 => array('row' => 'partidos' ,'tipo' => 'integer'),
		///
		10 => array('row' => 'votos_can_nreg' ,'nombre' => 'Votos Can Nrg' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		11 => array('row' => 'votos_nulos' ,'nombre' => 'Votos Nulos' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		12 => array('row' => 'votos_totales' ,'nombre' => 'Votos Totales' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		13 => array('row' => 'lista_nominal' ,'nombre' => 'Lista Nominal' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		14 => array('row' => 'rentabilidad' ,'nombre' => 'Rentabilidad Votos Validos - '.$searh_partido ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		15 => array('row' => 'rentabilidad_posicion' ,'nombre' => 'Rentabilidad' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		16 => array('row' => 'competividad' ,'nombre' => 'Competividad Primera Fuerza - Segunda Fuerza' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		17 => array('row' => 'competividad_posicion' ,'nombre' => 'Competividad' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		18 => array('row' => 'semaforo' ,'nombre' => 'Semáforo' ,'tipo' => 'string', 'fill' =>'#397CB5' ),
		19 => array('row' => 'ciudadanos_registrados' ,'nombre' => 'Ciudadanos Registrados' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		20 => array('row' => 'acciones_obras' ,'nombre' => 'Programas de Inversión' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		21 => array('row' => 'apoyos_programas' ,'nombre' => 'Programas de Gobierno' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		22 => array('row' => 'funcionarios' ,'nombre' => 'Funcionarios' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		23 => array('row' => 'militantes' ,'nombre' => 'Militantes' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		24 => array('row' => 'juntas' ,'nombre' => 'Juntas' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		25 => array('row' => 'visitas' ,'nombre' => 'Visitas' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
		26 => array('row' => 'caminatas' ,'nombre' => 'Caminatas' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
	);

	//unset($_SESSION['reporte_Sistema']['columnas_titulos']['excel']);
	unset($_SESSION['reporte_Sistema']['excel']['columnas_titulos']);
	foreach ($columnas_titulos as $key => $value) {
		if($value['row']=='partidos'){
			foreach ($partidos as $key => $value) {
				$_SESSION['reporte_Sistema']['excel']['columnas_titulos'][]= array('row' => $key , 'nombre' => $key ,'tipo' => 'integer', 'fill' =>'#'.$value['color_background'] );
			}
		}else{
			$_SESSION['reporte_Sistema']['excel']['columnas_titulos'][]=$value;
		}
		///$_SESSION['reporte_Sistema']['data'];
	}

	//echo "<pre>";
	//print_r($datos_secciones_ine);
	//echo "</pre>";
	$_SESSION['reporte_Sistema']['excel']['columnas_datos'] = NULL;
	$_SESSION['reporte_Sistema']['pdf']['secciones'] = NULL;
	foreach ($datos_secciones_ine as $id_seccion_ine => $datos) {
		...
		if($colum['municipio']!=''){
			//echo "<pre>";
			//print_r($colum);
			$_SESSION['reporte_Sistema']['excel']['columnas_datos'][] = $colum;
			$_SESSION['reporte_Sistema']['pdf']['secciones'][] = $datos;
			//echo "</pre>";
		}
	}
	
	//echo "<textarea>";
	//echo json_encode($_SESSION['reporte_Sistema']);
	//echo "</textarea>";
	//die;
	//Esta session se utiliza en grafica para generar las graficas
	//$_SESSION['reporte_Sistema']['data']
	...
	<script type="text/javascript">
		function myMap(){
			...
			foreach ($datos_distritos_federales as $key => $value) {
				...
				if($value['partido_ganador_background']=="" || $key != $id_distrito_federal ){
				...
				if(!empty($value['orden_votos_individual']['semaforo']['color'])){
					...
					$div = '<div class="divMapa">
								<div class="info_content">
									...
									<h5>Localidad(es): '.$value['seccion_localidades'].'</h5>
