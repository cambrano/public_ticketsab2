<?php
	...
	if(!empty($_POST['searchTable'][0])){
		...
		$sqlDistritos = $sqlMunicipio.$sqlDistritoLocal;
		$sqlDistritosCVP = $sqlMunicipioCVP.$sqlDistritoLocalCVP;
	}
	$tipo = 2;
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} {$sqlSeccionesIneSimple} {$sqlDistritos} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) total_lista_nominal,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_federal={$id_distrito_federal} {$sqlSeccionesIne} {$sqlDistritos} ) ciudadanos_totales,
				(SELECT GROUP_CONCAT(DISTINCT id_distrito_local) AS id_distrito_local FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} {$sqlSeccionesIneSimple} {$sqlDistritos} ) distritos_locales,
				(SELECT GROUP_CONCAT(DISTINCT id_distrito_federal) AS id_distrito_federal FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} {$sqlSeccionesIneSimple} {$sqlDistritos} ) distritos_federales
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2021 WHERE id_distrito_federal={$id_distrito_federal} AND tipo =  '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} )t
	...
	$sql="
		...
		WHERE cvp.id_distrito_federal={$id_distrito_federal} AND cvp.tipo = '{$tipo}' {$sqlSeccionesIneCVP}  {$sqlDistritosCVP}
