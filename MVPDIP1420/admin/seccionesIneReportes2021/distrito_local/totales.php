<?php
	...
	if(!empty($_POST['searchTable'][0])){
		...
		if(!empty($id_municipio)){
			$sqlMunicipio = " AND id_municipio IN ($id_municipio) ";
			$sqlMunicipioCVP = " AND cvp.id_municipio IN ($id_municipio) ";
			
		}
		if(!empty($id_distrito_federal)){
			$sqlDistritoFederal = " AND id_distrito_federal IN ($id_distrito_federal) ";
			$sqlDistritoLocalCVP = " AND cvp.id_distrito_federal IN ($id_distrito_federal) ";
			
		}
		$sqlDistritos = $sqlMunicipio.$sqlDistritoFederal;
		$sqlDistritosCVP = $sqlMunicipioCVP.$sqlDistritoLocalCVP;
	}
	$tipo = 1;
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} {$sqlSeccionesIneSimple} {$sqlDistritos} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} ) total_lista_nominal,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_local={$id_distrito_local} {$sqlSeccionesIne} {$sqlDistritos} ) ciudadanos_totales,
				(SELECT GROUP_CONCAT(DISTINCT id_distrito_local) AS id_distrito_local FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} {$sqlSeccionesIneSimple} {$sqlDistritos} ) distritos_locales,
				(SELECT GROUP_CONCAT(DISTINCT id_distrito_federal) AS id_distrito_federal FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} {$sqlSeccionesIneSimple} {$sqlDistritos} ) distritos_federales
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2021 WHERE id_distrito_local={$id_distrito_local} AND tipo =  '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} )t
	...
	$sql="
		...
		WHERE cvp.id_distrito_local={$id_distrito_local} AND cvp.tipo = '{$tipo}' {$sqlSeccionesIneCVP}  {$sqlDistritosCVP}
