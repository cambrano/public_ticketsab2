<?php
	...
	#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2021Datos['tipo_eleccion'] = '0';// 0 - Ayuntamiento | 1 - Municipio | 2 - Distrito Federal
	...
	$sql = "SELECT 
				...
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_federal={$id_distrito_federal} {$sql_secciones_ine_ciudadanos} ) ciudadanos_totales,
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_distrito_federal_2021_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_distrito_federal_2021_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
