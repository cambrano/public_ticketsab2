...
?>
		...
		function searchTable(value){
			...
			for (var i = 0; i < id_seccion_ine_input.length; i++) {
				if (id_seccion_ine_input.options[i].selected){
					...
					id_seccion_ine_array_table.push("(^a" + id_seccion_ine_input.options[i].value + "z)");
				}
			}
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2022RevocacionMandato/table.php",
			...
			*/
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2022RevocacionMandato/distrito_federal/mapa.php",
				data: {searchTable: searchTable,mapa:mapa},
				async: true,
				success: function(data) {
					$("#mapaLoad").html(data);
	...
	<div class="sucForm">
		<label class="labelForm" id="labeltemaname">Preguntas</label><br>
		<select class="selectpicker" multiple data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="partido_ganador_id" onchange="searchTable();">
			<?php
			echo preguntas_2022_revocacion_mandato('',0,'SIN');
			?>
			<option value="x"> NO DATA</option>
		</select><br>
	</div>
	<div class="sucForm">
