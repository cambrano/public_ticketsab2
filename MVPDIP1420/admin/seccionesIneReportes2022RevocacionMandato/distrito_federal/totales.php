<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 ) votos_nulos_2018,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 ) votos_can_nreg_2018,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 ) casillas_2018,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 ) total_lista_nominal_2018,

				( SELECT id_pregunta_2022_revocacion_mandato votos FROM casillas_preguntas_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 GROUP BY id_pregunta_2022_revocacion_mandato ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2018
			FROM
				(SELECT SUM(votos) votos_validos_2018 FROM  casillas_preguntas_2022_revocacion_mandato WHERE id_distrito_federal={$id_distrito_federal} AND tipo=0 )t
	...
	if($extranjeros_mode){
		$distritos_federales = $row['distritos_federales']-1;
		$extranjero = 1;
	}else{
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2018.votos) FROM casillas_preguntas_2022_revocacion_mandato cpv2018 WHERE cpv2018.id_pregunta_2022_revocacion_mandato = p2018.id AND cpv2018.id_distrito_federal={$id_distrito_federal} AND cpv2018.tipo=0 ) votos_totales_2018
			FROM preguntas_2022_revocacion_mandato p2018
	...
	$sql="
		...
		FROM preguntas_2022_revocacion_mandato p2018
		LEFT JOIN casillas_preguntas_2022_revocacion_mandato cvp2018
		ON p2018.id = cvp2018.id_pregunta_2022_revocacion_mandato
		WHERE cvp2018.id_distrito_federal={$id_distrito_federal} AND p2018.tipo=0 AND cvp2018.tipo=0
		GROUP BY cvp2018.id_pregunta_2022_revocacion_mandato
	";
	$sql;
...
<div class="totales">
	<div style="width: 100%;display: table;padding: 5px 5px 5px 0px;background-color: white">
		...
		<div style="background-color: white;padding: 5px;display: table;">
			...
			$partido_ganador_nombre_2018 = str_replace("_"," - ",$partido_ganador['nombre']);
			?>
			<div class="div50Reporte">
