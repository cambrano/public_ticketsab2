<?php
	...
	if(!empty($_POST['searchTable'][0])){
		...
		if($tipo_seccion == "0" || $tipo_seccion == "1" ){
			$sqlSeccionesIneCVTipo = " AND EXISTS (SELECT * FROM secciones_ine  WHERE secciones_ine.id = casillas_votos_2024.id_seccion_ine AND secciones_ine.tipo = '{$tipo_seccion}' )  ";
			...
			$sqlSeccionesIneCVPTipo = " AND EXISTS (SELECT * FROM secciones_ine  WHERE secciones_ine.id = casillas_votos_partidos_2024.id_seccion_ine AND secciones_ine.tipo = '{$tipo_seccion}' )  ";
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2024 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} {$sqlSeccionesIneCVTipo} ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2024 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} {$sqlSeccionesIneCVTipo} ) votos_can_nreg,
				...
				(SELECT COUNT(id) FROM casillas_votos_2024 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} {$sqlSeccionesIneCVTipo} ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2024 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} {$sqlSeccionesIneCVTipo} ) total_lista_nominal,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2024 WHERE id_municipio={$id_municipio} AND tipo =  '{$tipo}' {$sqlSeccionesIne} {$sqlDistritos} {$sqlSeccionesIneCVPTipo} )t
