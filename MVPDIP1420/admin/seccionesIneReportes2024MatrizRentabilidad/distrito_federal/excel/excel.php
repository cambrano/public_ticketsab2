<?php
	...
	$rutaEfs = rutaEfs();
	$archivo_json = $rutaEfs . 'titulos_distrito_federal_2024_-'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	...
	$archivo_json = $rutaEfs . 'distrito_federal_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	if (file_exists($archivo_json)) {
		// Lee el contenido del archivo JSON
		$json_data = file_get_contents($archivo_json);
	...
	$filename = 'MatrizRentabilidadDistritoFederal_'.$datos_generales['territorio_nombre'].'_-_'.date("Ymd-His").'-'.$gen_id3.$mk_id.'.xlsx';
	...
	*/
	...
	$keywords = array('Matriz de Rentabilidad Distrito Federal','Data','Información');
	$writer->setTitle('Matriz de Rentabilidad Distrito Federal');
