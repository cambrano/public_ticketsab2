<?php
	...
	if(!empty($_POST)){
		...
		$id_distrito_federal = $_POST['mapa'][0]['id_distrito_federal'];
	}
	//var_dump($_POST);
	...
	*/
	$sql = "
		SELECT 
			...
			(SELECT SUM(cvp2024.votos) FROM casillas_votos_partidos_2024 cvp2024 WHERE cvp2024.id_distrito_federal = {$id_distrito_federal}  AND cvp2024.tipo={$tipo} ) suma_votos_validos
		FROM casillas_votos_2024 cv2024
		WHERE cv2024.id_distrito_federal = {$id_distrito_federal} AND cv2024.tipo={$tipo};
	";
	...
	$sql = "
		...
		WHERE cvp2024.id_distrito_federal = {$id_distrito_federal} AND p2024.tipo={$tipo} AND cvp2024.tipo = {$tipo};
	";
	$result = $conexion->query($sql); 
	while ($row = $result->fetch_assoc()) {
		if ($row['principal'] == 1) {
	...
	*/
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'distrito_federal_2024_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'titulos_distrito_federal_2024_-'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
			...
			<?php
				foreach ($datos_distritos_federales as $key => $value) {
					...
					foreach ($distritos_federales_parametrosDatosMapa[$value['id']] as $keyT => $valueT) {
						$path = "distritos_federales_polygon".$key."_".$keyT;
						echo "var " . $path . " = [";
					...
					if (empty($value['partido_ganador_background']) || $key != $id_distrito_federal) {
						$value['partido_ganador_border'] = "000000";
						$value['partido_ganador_background'] = "000000";
					}
				
					?>
					var distrito_federal_area<?= $key ?> = new google.maps.Polygon({
					...
					distrito_federal_area<?= $key ?>.setMap(map);
					<?php
				}
