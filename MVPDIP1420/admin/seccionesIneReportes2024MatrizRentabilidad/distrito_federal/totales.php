<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2024 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2024 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2024 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2024 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) total_lista_nominal,
				...
				(SELECT COUNT(*) FROM secciones_ine_agendas_gobierno_locaciones siagl LEFT JOIN secciones_ine s ON siagl.id_seccion_ine = s.id WHERE s.id_distrito_federal = {$id_distrito_federal} ) eventos_agenda_gobierno
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2024 WHERE id_distrito_federal={$id_distrito_federal} AND tipo =  '{$tipo}' )t
	";
	//echo "<pre>".$sql."</pre>";
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_distrito_federal_2024_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_distrito_federal_2024_'.$id_distrito_federal.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
				...
				<table style="table-layout: fixed; width: 100%" cellspacing="0" cellpadding="0">
					...
					<tbody>
						...
						<tr>
							<td
								style="text-align: left; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.6); border-bottom: 1px solid white">
								<font class="fontLabelReporte">Eventos Agenda Gobierno:</font>
							</td>
							<td
								style="text-align: right; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.3); border-bottom: 1px solid white">
								<font class="fontDataReporte" style="font-size: 12px">
									<?=number_format($datos_generales['eventos_agenda_gobierno'], 0, '.', ','); ?>
								</font>
							</td>
						</tr>
					</tbody>
