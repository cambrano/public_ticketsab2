...
?>
	<script type="text/javascript">
		...
		function searchTable(value){
			...
			var id_secciones_ine_colonia_array = [];
			var id_secciones_ine_colonias_array_table = [];
			for (var i = 0; i < id_secciones_ine_colonia_input.length; i++) {
				if (id_secciones_ine_colonia_input.options[i].selected){
					
					var array_colonias = id_secciones_ine_colonia_input.options[i].value.split(',');
					array_colonias.forEach(function(element) {
						id_seccion_ine_array.push(element);
						id_seccion_ine_array_table.push("(^" + element + "$)");
					});
					//id_secciones_ine_colonia_array.push(id_secciones_ine_colonia_input.options[i].value);
					//id_secciones_ine_colonias_array_table.push("(^" + id_secciones_ine_colonia_input.options[i].value + "$)");
				}
			}

			var id_secciones_ine_localidad_input = document.getElementById("id_secciones_ine_localidad");
			var id_secciones_ine_localidad_array = [];
			var id_secciones_ine_localidades_array_table = [];
			for (var i = 0; i < id_secciones_ine_localidad_input.length; i++) {
				if (id_secciones_ine_localidad_input.options[i].selected){
					
					var array_localidades = id_secciones_ine_localidad_input.options[i].value.split(',');
					array_localidades.forEach(function(element) {
						id_seccion_ine_array.push(element);
						id_seccion_ine_array_table.push("(^" + element + "$)");
					});
					//id_secciones_ine_localidad_array.push(id_secciones_ine_localidad_input.options[i].value);
					//id_secciones_ine_localidades_array_table.push("(^" + id_secciones_ine_localidad_input.options[i].value + "$)");
				}
			}

			//id_seccion_ine_1 = id_secciones_ine_colonia_array.join(","); 
			//console.log(id_secciones_ine_colonias_array_table);
			const unique_array_secciones_ine = [...new Set(id_seccion_ine_array)];
			id_seccion_ine_array = [];
			id_seccion_ine_array = unique_array_secciones_ine;

			const unique_array_secciones_ine_table = [...new Set(id_seccion_ine_array_table)];
			id_seccion_ine_array_table = [];
			id_seccion_ine_array_table = unique_array_secciones_ine_table;
			...
			if(tipo_seccion==""){
				$('#secciones_reportes-tabla').DataTable().column(19).search(tipo_seccion).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+tipo_seccion+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(19).search(tipo_seccion_array_table.join('|'), true, false).draw();
				$('id_secciones_ine').children().remove();
			}
			...
			if(semaforo==""){
				$('#secciones_reportes-tabla').DataTable().column(14).search(semaforo).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+semaforo+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(14).search(semaforo_array_table.join('|'), true, false).draw();
			}
			...
			if(id_distrito_local==""){
				$('#secciones_reportes-tabla').DataTable().column(5).search(id_distrito_local).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_seccion_ine+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(5).search(id_distrito_local_array_table.join('|'), true, false).draw();
			}
			...
			if(id_distrito_federal==""){
				$('#secciones_reportes-tabla').DataTable().column(6).search(id_distrito_federal).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_seccion_ine+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(6).search(id_distrito_federal_array_table.join('|'), true, false).draw();
			}
			...
			var data = {   
					...
					'id_municipio' : id_municipio,
					'semaforo' : semaforo,
					'secciones_ine_giras' : secciones_ine_giras,
			...
			if(partido_ganador_id==""){
				$('#secciones_reportes-tabla').DataTable().column(2).search(partido_ganador_id).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(1).search("(^"+partido_ganador_id+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(2).search(partido_ganador_id_array_table.join('|'), true, false).draw();
			}

			if(id_municipio==""){
				$('#secciones_reportes-tabla').DataTable().column(3).search(id_municipio).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_municipio+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(3).search(id_municipio_array_table.join('|'), true, false).draw();
			}
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2024/table.php",
			...
			*/
			...
			if(value != 'pagina'){
				...
				$.ajax({
					type: "POST",
					url: "seccionesIneReportes2024MatrizRentabilidad/distrito_federal/mapa.php",
					data: {searchTable: searchTable,mapa:mapa},
					async: true,
					success: function(data) {
						$("#mapaLoad").html(data);
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2024MatrizRentabilidad/distrito_federal/graficas.php",
	...
	<div class="sucForm">
		...
		<select class="selectpicker" data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_municipio" onchange="searchTable();">
			<?php
			echo seccion_ineMunicipios('',$id_estado,'','',$id_distrito_federal);
			?>
		</select><br>
	</div>
	<div class="sucForm">
		...
		<select class="selectpicker" multiple data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_distrito_local" onchange="searchTable();">
			<?php
			echo seccion_ineDistritosLocales('','','',$id_distrito_federal,'SIN');
			?>
		</select><br>
	</div>
	<div class="sucForm" style="display:none">
		...
		<select class="selectpicker" multiple data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_distrito_federal" onchange="searchTable();">
			<?php
			echo distritos_federales($id_distrito_federal,'SIN');
			?>
		</select><br>
	</div>
	<div class="sucForm">
		...
		<select class="selectpicker" multiple data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_secciones_ine_colonia" onchange="searchTable();">
			<?php
			echo secciones_ine_colonias('','','','',$id_distrito_federal,'SIN');
			?>
		</select><br>
	</div>
	<div class="sucForm" >
		...
		<select class="selectpicker" data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_secciones_ine_localidad" onchange="searchTable();">
			<?php
			echo localidades_secciones_ineIdSeccionesSelect('',$id_estado,'','',$id_distrito_federal,'','');
			?>
		</select><br>
	</div>
	<div class="sucForm"">
	...
	<div class="sucForm">
		...
		<select class="selectpicker" multiple data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="partido_ganador_id" onchange="searchTable();">
			...
			<?php
			echo partidos_2024('',$tipo,'sin_coalicion','SIN');
			?>
		</select><br>
	</div>
	<div class="sucForm">
