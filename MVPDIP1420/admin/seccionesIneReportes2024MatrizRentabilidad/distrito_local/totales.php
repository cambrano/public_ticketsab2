<?php
	...
	$id_partido_2024 = $configuracion_matriz_rentabilidad_secciones_ine_2024Datos['id_partido_2024_distrito_local'];// Partidos 2024 PRI
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2024 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2024 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2024 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2024 WHERE id_distrito_local={$id_distrito_local} AND tipo = '{$tipo}' ) total_lista_nominal,
				...
				(SELECT COUNT(*) FROM secciones_ine_agendas_gobierno_locaciones siagl LEFT JOIN secciones_ine s ON siagl.id_seccion_ine = s.id WHERE s.id_distrito_local = {$id_distrito_local} ) eventos_agenda_gobierno
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2024 WHERE id_distrito_local={$id_distrito_local} AND tipo =  '{$tipo}' )t
	";
	//echo "<pre>".$sql."</pre>";
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'datos_generales_distrito_local_2024_'.$id_distrito_local.'-'.$_COOKIE["id_usuario"].'.json';
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'datos_partidos_distrito_local_2024_'.$id_distrito_local.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
