<?php
	...
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','cartografias_distritos_federales_2024',$_COOKIE["id_usuario"]);
	...
	$rutaEfs = rutaEfs();
	$archivo_json = $rutaEfs . 'titulos_forzar_distrito_federal_2024_-'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	...
	$archivo_json = $rutaEfs . 'forzar_distrito_federal_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	if (file_exists($archivo_json)) {
		// Lee el contenido del archivo JSON
		$json_data = file_get_contents($archivo_json);
		...
		//echo "El archivo JSON no existe en la ruta especificada";
		echo "Error Información";
		die;
	}
	

	$datos_generales['territorio_nombre'];
	$filename = 'MatrizRentabilidadF_Distrito_FederalMunicipio_'.str_replace(" ", "_", $datos_generales['territorio_nombre']).'_-_'.date("Ymd-His").'-'.$gen_id3.$mk_id.'.xlsx';
	...
	*/
	...
	$keywords = array('Matriz de Rentabilidad F_Distrito_Federal Municipio','Data','Información');
	$writer->setTitle('Matriz de Rentabilidad F_Distrito_Federal Municipio');
	...
	foreach ($titulos as $key => $value) {
		...
		$columnas[] = $value['row'];
		$style[] = array('auto_filter' => true, 'fill' => $value['fill'], 'color' => $value['color'], 'font-style' => 'bold', 'border' => 'left,right,top,bottom');
	}
	...
	foreach ($columnas_datos as $key => $data) {
		...
		foreach ($titulos as $keyT => $value) {
			if($value['row'] == 'seccion_colonias' || $value['row'] == 'seccion_localidades'  ){
				$columna_valor[]  = "_".str_replace('*_*', "\n_", $data[$value['row']]);
			}else{
				$columna_valor[] = $data[$value['row']];
			}
			
			$columna_row[] = $value['row'];
		}

		foreach ($columna_row as $keyT => $value) {
			$marco = $color_reg % 2;
			if ($value == 'seccion_colonias' || $value == 'seccion_localidades'  ) {
				//$styleRow[] = array('wrap_text' => true, 'valign' => 'top');
                if($marco == 0){
					$styleRow[$keyT] = array('fill' => '#e9e9e9','color'=>'#000000','border'=>'left,right,top,bottom','wrap_text' => true, 'valign' => 'center');
				}else{
					$styleRow[$keyT] = array('color'=>'#000000','border'=>'left,right,top,bottom','wrap_text' => true, 'valign' => 'center');
				}
			}else{
				if($marco == 0){
					$styleRow[$keyT] = array('fill' => '#e9e9e9','color'=>'#000000','border'=>'left,right,top,bottom', 'valign' => 'center');
				}else{
					$styleRow[$keyT] = array('color'=>'#000000','border'=>'left,right,top,bottom', 'valign' => 'center');
				}
			}
		}
