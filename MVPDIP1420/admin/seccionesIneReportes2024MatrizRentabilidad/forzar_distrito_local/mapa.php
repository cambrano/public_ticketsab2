<?php
	...
	if(!empty($_POST)){
		...
		$id_municipio = $_POST['mapa'][0]['id_municipio'];
	}
	//var_dump($_POST);
	...
	*/
	...
	//$latitud_sugerido = $datos_municipios[$id_municipio]['latitud'];
	//$longitud_sugerido = $datos_municipios[$id_municipio]['longitud'];
			...
			<?php
				foreach ($datos_municipios as $key => $value) {
					...
					foreach ($municipios_parametrosDatosMapa[$value['id']] as $keyT => $valueT) {
						$path = "municipios_polygon".$key."_".$keyT;
						echo "var " . $path . " = [";
					...
					if (empty($value['partido_ganador_background']) || $key != $id_municipio) {
						$value['partido_ganador_border'] = "000000";
						$value['partido_ganador_background'] = "000000";
					}
				
					?>
					var municipio_area<?= $key ?> = new google.maps.Polygon({
