<?php
	...
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','cartografias_gobernador_2024',$_COOKIE["id_usuario"]);
	...
	$rutaEfs = rutaEfs();
	$archivo_json = $rutaEfs . 'titulos_gobernador_2024_-'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	...
	$archivo_json = $rutaEfs . 'gobernador_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
	if (file_exists($archivo_json)) {
		// Lee el contenido del archivo JSON
		$json_data = file_get_contents($archivo_json);
	...
	$filename = 'MatrizRentabilidadGobernadorMunicipio_'.str_replace(" ", "_", $datos_generales['territorio_nombre']).'_-_'.date("Ymd-His").'-'.$gen_id3.$mk_id.'.xlsx';
	...
	*/
	...
	$keywords = array('Matriz de Rentabilidad Gobernador Municipio','Data','Información');
	$writer->setTitle('Matriz de Rentabilidad Gobernador Municipio');
