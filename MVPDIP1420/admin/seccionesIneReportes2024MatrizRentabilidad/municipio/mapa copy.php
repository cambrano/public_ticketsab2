<?php
		...
		*/
		$sql = "
			SELECT 
				...
				(SELECT SUM(cvp2024.votos) FROM casillas_votos_partidos_2024 cvp2024 WHERE cvp2024.id_municipio = {$id_municipio}  AND cvp2024.tipo={$tipo} ) suma_votos_validos
			FROM casillas_votos_2024 cv2024
			WHERE cv2024.id_municipio = {$id_municipio} AND cv2024.tipo={$tipo};
		";
		...
		$sql = "
			...
			WHERE cvp2024.id_municipio = {$id_municipio} AND p2024.tipo={$tipo} AND cvp2024.tipo = {$tipo};
		";
		$result = $conexion->query($sql); 
		while ($row = $result->fetch_assoc()) {
			if ($row['principal'] == 1) {
		...
		while($row=$result->fetch_assoc()){
			...
			if($row['tipo']==1){
				$row['tipo_nombre'] = 'Urbano';
			}else{
				$row['tipo_nombre'] = 'Rural';
			}
			//!votos totales
			$row['votos_totales'] = $row['votos_nulos'] + $row['votos_can_nreg'] + $row['votos_validos'];
				...
				if(!empty($coalicion_principal)){
					...
					foreach ($partidos_2024_individual[$row['id']] as $key => $value) {
						if($key == $principal_clave){
							...
							$votos_totales_individual = $value['votos'];
						}
					}
					...
					$row['principal_votos_totales_individual'] = $votos_totales_individual;
					$votos_totales_individual = 0;
					foreach ($partidos_principal_individuales as $key => $value) {
						$votos_totales_individual = $votos_totales_individual + $value['votos'];
					}
					$row['principal_votos_totales_individuales_coaliciones'] = $votos_totales_individual;
					
					$row['principal_partidos_individuales'] = $partidos_principal_individuales;
					$row['partidos_principal_coalicion'] = $partidos_principal_coalicion;
	...
	foreach ($seccion_ine_datos as $key => $value) {
		$seccion_ine_datos[$key]['rentabilidad_participacion_votos__partido_individual_porcentaje_orden'] = $rentabilidad_individual_orden[$key];
		$seccion_ine_datos[$key]['rentabilidad_participacion_votos__partido_coalicion_porcentaje_orden'] = $rentabilidad_coalicion_orden[$key];
	}
	...
	foreach ($seccion_ine_datos as $seccion => $data) {
		...
		foreach ($data as $column => $dato) {
			/*
			if($column=='partidos_principal_individuales'){
				echo "<td>";
				echo "<pre>";
				//var_dump($dato);
				echo "</pre>";
	...
	//$json_data = json_encode($seccion_ine_datos);//Json donde se guarda la informacion para evitar las variables de sesion
	...
	if ($json_data === false) {
		...
		$archivo_json = $rutaEfs.'municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
