<?php
	...
	if(!empty($_POST)){
		...
		$id_partido_2024 = $configuracion_matriz_rentabilidad_secciones_ine_2024Datos['id_partido_2024_ayuntamiento'];// Partidos 2024 PRI
		//$id_partido_2024 = $configuracion_matriz['id_partido_2024'] = '1';// Partidos 2024
		$id_partido_legado = $configuracion_matriz_rentabilidad_secciones_ine_2024Datos['id_partido_legado'] = '1';// Partidos Legados
		...
		*/
		...
		while($row=$result->fetch_assoc()){
			...
			$partidos_nombre[$row['clave']] = $partidos[$row['clave']] = $row['nombre_corto'];
		...
		*/
		...
		if ($json_data === false) {
			...
			$archivo_json = $rutaEfs.'competitividad_rentabilidad_individual_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
		...
		if ($json_data === false) {
			...
			// Guarda el JSON en un archivo
			$archivo_json = $rutaEfs.'competitividad_rentabilidad_totales_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
			if (file_put_contents($archivo_json, $json_data)) {
				//echo "Los datos se han guardado en $archivo_json";
			} else {
				//echo "Error al guardar los datos en $archivo_json";
	...
	if(empty($_POST)){
		...
		if ($json_data === false) {
			...
			// Guarda el JSON en un archivo
			$archivo_json = $rutaEfs.'datos_secciones_ine_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
			if (file_put_contents($archivo_json, $json_data)) {
				//echo "Los datos se han guardado en $archivo_json";
			} else {
				//echo "Error al guardar los datos en $archivo_json";
	...
	$columnas_titulos = array(
		...
		28 => array('row' => 'eventos_agenda_gobierno' ,'nombre' => 'Eventos Agenda Gobierno' ,'tipo' => 'integer', 'fill' =>'#397CB5' ),
	...
	foreach ($columnas_titulos as $key => $value) {
		if($value['row']=='partidos'){
			foreach ($partidos as $key => $value) {
				$columnas_titulos_partidos[]= array('row' => $key , 'nombre' => $value['nombre_corto'] ,'tipo' => 'integer', 'fill' =>'#'.$value['color_background'] );
			}
		}else{
	...
	if ($json_data === false) {
		...
		// Guarda el JSON en un archivo
		$archivo_json = $rutaEfs.'columnas_titulos_partidos_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
		if (file_put_contents($archivo_json, $json_data)) {
			//echo "Los datos se han guardado en $archivo_json";
		} else {
			//echo "Error al guardar los datos en $archivo_json";
	...
	$partidos_n = $partidos;
	if(empty($partidos)){
		foreach ($partidos_nombre as $key => $value) {
			$partidos[$key]['nombre_corto'] = $value;
		}
	}else{
		unset($partidos);
		foreach ($partidos_n as $key => $value) {
			$partidos[$key]['nombre_corto'] = $value['nombre_corto'];
		}
	}
	...
	foreach ($datos_secciones_ine as $id_seccion_ine => $datos) {
		foreach ($columnas_titulos_partidos  as $key => $value) {
			if($value['row'] == 'rentabilidad'){
				...
				$colum[ $value['row'] ] = $partidos[$datos['orden_votos_individual']['primera_fuerza']]['nombre_corto'];
				...
				$colum[ $value['row'] ] = $partidos[$datos['orden_votos_individual']['segunda_fuerza']]['nombre_corto'];
				...
				if($datos['orden_votos_individual']['sistema']==''){
					$colum[ $value['row'] ] = $partidos[$searh_partido]['nombre_corto'];
					...
					//$colum[ $value['row'] ] = $partidos[$datos['orden_votos_individual']['sistema']];
	...
	if(!empty($columnas_datos)){
		...
		if ($json_data === false) {
			...
			// Guarda el JSON en un archivo
			$archivo_json = $rutaEfs.'columnas_datos_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
			if (file_put_contents($archivo_json, $json_data)) {
				//echo "Los datos se han guardado en $archivo_json";
			} else {
				//echo "Error al guardar los datos en $archivo_json";
		...
		if ($json_data === false) {
			...
			// Guarda el JSON en un archivo
			$archivo_json = $rutaEfs.'secciones_municipio_2024_'.$id_municipio.'-'.$_COOKIE["id_usuario"].'.json';
			if (file_put_contents($archivo_json, $json_data)) {
				//echo "Los datos se han guardado en $archivo_json";
			} else {
				//echo "Error al guardar los datos en $archivo_json";
	...
	if(!empty($id_secciones_ine_validos)){
		if(!empty($_POST['mapa'][0]['secciones_ine_agendas_gobierno']) || !empty($_POST['mapa'][0]['secciones_ine_actividades'])  ){
		...
		if(!empty($_POST['mapa'][0]['secciones_ine_agendas_gobierno'])){
			...
			foreach ($secciones_ine_agendas_gobiernoDatosArray as $key => $value) {
				...
				$id_seccion_ine_agendas_gobierno[] = $value['id'];
			}
			/*
			if(!empty($id_seccion_ine_agendas_gobierno)){
