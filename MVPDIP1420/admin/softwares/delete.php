<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'softwares','softwares','index');
	if($redirectSecurity!=""){
		die;
	}
	$softwareDatos=softwareDatos($id);
	$moduloAccionPermisos = moduloAccionPermisos('operatividad','softwares',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('softwares',$id);
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Software</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?= $softwareDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
