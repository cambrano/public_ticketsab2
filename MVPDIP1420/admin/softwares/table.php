...
?>
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#softwares-tabla').DataTable( {
				"destroy": true,
				...
				"ajax":{
					url :"softwares/softwares.php", // json datasource
					type: "post",  // method  , by default get
					data: {
						postData: <?php echo $postData; ?>
					},
					error: function(){  // error handling
						$(".softwares-tabla-error").html("");
						$("#softwares-tabla").append('<tbody class="softwares-tabla-error"><tr><th colspan="8">Registros no encontrados</th></tr></tbody>');
						$("#softwares-tabla_processing").css("display","none");
			...
			$('#softwares-tabla').resize();
			$('#softwares-tabla').DataTable().columns.adjust().responsive.recalc();
			$("#softwares-tabla_filter").css("display","none");  // hiding global search box
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="softwares/index.php?cot="+ids_string;   
					var link2="softwares/index.php";
		...
		function edit(valor){
			link="softwares/update.php?id="+valor; 
			var link2="softwares/update.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function add(){
			////ajax
			link="softwares/create.php";
			var link2="softwares/create.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function borrar(valor){
			link="softwares/delete.php?id="+valor; 
			var link2="softwares/delete.php";
			dataString = 'urlink='+link2; 
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
