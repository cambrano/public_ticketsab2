<?php
	...
	$sql = "SELECT count(*) total FROM sub_dependencias WHERE 1 = 1   "; 
	...
	$sql = "SELECT  sicpa.id,
					...
					sicpa.nombre,
					sicpa.nombre_corto
			FROM sub_dependencias sicpa WHERE 1 = 1";
	$sql.= " AND sicpa.id_dependencia ='{$id_dependencia}' ";
	...
	if( $nombre!="" ){   //name
		$post_search=true;
		$sql.=" AND sicpa.nombre LIKE '%{$nombre}%' ";
		$sqlContador .= " AND sicpa.nombre LIKE '%{$nombre}%' ";
	} 

	$nombre_corto=$search_database['nombre_corto'];
	if( $nombre_corto!="" ){   //name
		$post_search=true;
		$sql.=" AND sicpa.nombre_corto  = '{$nombre_corto}' ";
		$sqlContador .= " AND sicpa.nombre_corto = '{$nombre_corto}' ";
	}
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM sub_dependencias sicpa WHERE 1 = 1 AND sicpa.id_dependencia ='{$id_dependencia}'  "; 
