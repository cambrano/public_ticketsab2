<?php
	...
	$columns = array( 
		...
		0 =>'clave',
		1 =>'nombre',
		2 =>'nombre_tipo_estructura_ciudadano',
	...
	$sql = "SELECT count(*) total FROM tipos_ciudadanos tc WHERE 1 = 1   "; 
	...
	$sql = "SELECT 
				tc.id,
				tc.clave,
				tc.nombre,tec.nombre AS nombre_tipo_estructura_ciudadano
			FROM tipos_ciudadanos tc
			LEFT JOIN tipos_estructuras_ciudadanos tec
			ON tc.id_tipo_estructura_ciudadano = tec.id
			WHERE 1 = 1 
	"; 
	...
	if( $clave!="" ){   //name
		$post_search=true;
		$sql.=" AND tc.clave LIKE '%{$clave}%' ";
		$sqlContador .= " AND tc.clave LIKE '%{$clave}%' ";
	} 

	$nombre=$search_database['nombre'];
	if( $nombre!="" ){   //name
		$post_search=true;
		$sql.=" AND tc.nombre LIKE '%{$nombre}%' ";
		$sqlContador .= " AND tc.nombre LIKE '%{$nombre}%' ";
	}
	$id_tipo_estructura_ciudadano=$search_database['id_tipo_estructura_ciudadano'];
	if( $id_tipo_estructura_ciudadano!="" ){   //name
		$post_search=true;
		$sql.=" AND tc.id_tipo_estructura_ciudadano = '{$id_tipo_estructura_ciudadano}' ";
		$sqlContador .= " AND tc.id_tipo_estructura_ciudadano LIKE '{$id_tipo_estructura_ciudadano}' ";
	}
	...
	while($row=$result->fetch_assoc()){
		...
		$nestedData[] = "<div style='text-transform: none;'>".$row["nombre_tipo_estructura_ciudadano"]."<div>"; 
		if($option_delete){
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM tipos_ciudadanos tc WHERE 1 = 1   "; 
