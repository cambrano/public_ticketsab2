<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'tipos_gastos','tiposGastos','index');
	if($redirectSecurity!=""){
		die;
	}
	$tipo_gastoDatos=tipo_gastoDatos($id);
	$moduloAccionPermisos = moduloAccionPermisos('operatividad','tipos_gastos',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('tipos_gastos',$id);
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Tipo de Gasto</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Tipo de Gasto</label><br>
			<label class="descripcionForm">
				<strong><?= $tipo_gastoDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
