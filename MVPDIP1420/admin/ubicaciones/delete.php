<?php
	...
	echo $redirectSecurity=redirectSecurity($id,'ubicaciones','ubicaciones','index');
	if($redirectSecurity!=""){
		die;
	}
	$ubicacionDatos=ubicacionDatos($id);
	$moduloAccionPermisos = moduloAccionPermisos('operatividad','ubicaciones',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('ubicaciones',$id);
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Ubicación</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Ubicación</label><br>
			<label class="descripcionForm">
				<strong><?= $ubicacionDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
