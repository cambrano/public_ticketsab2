<?php
	...
	$sql = "SELECT count(*) total FROM empleados_dependencias ed WHERE 1 = 1   "; 
	if($id_empleado!=''){
		$sql.=" AND ed.id_empleado = '{$id_empleado}' ";
	}
	...
	$sql   = "SELECT 
		ed.id,
		ed.id_dependencia,
		(SELECT d.nombre FROM dependencias d WHERE d.id = ed.id_dependencia ) dependencia
		FROM empleados_dependencias ed WHERE ed.id_empleado='{$id_empleado}' ";
	// getting records as per search parameters
	if($validar_codigo_plataforma == false){
		$sql .= " AND ed.codigo_plataforma = '{$codigo_plataforma}' ";
	}
	$id_dependencia=$search_database['id_dependencia'];
	if( $id_dependencia!="" ){   //name
		$post_search=true;
		$sql.=" AND ed.id_dependencia = '{$id_dependencia}' ";
		$sqlContador.=" AND ed.id_dependencia = '{$id_dependencia}' ";
	} 
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM empleados_dependencias ed WHERE 1 = 1 "; 
		if($id_empleado!=''){
			$sqlContadorScript.=" AND ed.id_empleado = '{$id_empleado}' ";
		}
		if($validar_codigo_plataforma == false){
			$sqlContadorScript .= " AND ed.codigo_plataforma = '{$codigo_plataforma}' ";
		}
