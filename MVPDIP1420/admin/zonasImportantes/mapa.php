<?php
	if($reload_mapa == ""){
		...
		if(!empty($_POST)){
			...
			include '../functions/zonas_importantes.php';
			$punto=false;
			...
			$zonas_importantesDatosArray=zonas_importantesDatosArray($_POST['searchTable'][0],$orderby,$limit);
			...
			if($punto==false){
				$zoom = 14;
				if($zonas_importantesDatosArray[0]['latitud'] != '' ){
			...
			$secciones_ineDatosMapa = secciones_ineDatosForm($search_map);
			$zonas_importantesDatosArray = zonas_importantesDatosArray('','',$id_municipio,$id_distrito_local,$id_distrito_federal,'','');
		}
