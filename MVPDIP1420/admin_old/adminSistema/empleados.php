<?php
	...
	$sql   = "SELECT count(*) total FROM empleados e WHERE EXISTS (SELECT * FROM usuarios u WHERE u.id_perfil_usuario=2 AND e.id = u.id_empleado   ) "; 
	...
	$sql   = "
		...
		FROM empleados e WHERE EXISTS (SELECT * FROM usuarios u WHERE u.id_perfil_usuario=2 AND e.id = u.id_empleado   ) "; 
	...
	$nombre_usuario=$_SESSION['searchTable']['nombre_usuario'];
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total
		FROM empleados e WHERE EXISTS (SELECT * FROM usuarios u WHERE u.id_perfil_usuario=2 AND e.id = u.id_empleado   ) ";
		if($validar_codigo_plataforma == false){
