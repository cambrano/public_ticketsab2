<?php
	...
	$sql = "SELECT count(*) total FROM api_whatsapp_mensajes WHERE 1 = 1   "; 
	...
	////////////////////////////
	$sql = "SELECT 
			(SELECT awm2.fechaR FROM api_whatsapp_mensajes awm2 WHERE awm2.whatsapp= awm.whatsapp ORDER BY fechaR DESC LIMIT 1  ) fechaR,
			awm.whatsapp,
			count(*) mensajes
			FROM api_whatsapp_mensajes awm
			WHERE awm.id_seccion_ine_ciudadano is null"; 
	// getting records as per search parameters
	$whatsapp=$_SESSION['searchTable']['whatsapp'];
	if( $whatsapp!="" ){   //name
		$post_search=true;
		$sql.=" AND whatsapp LIKE '%{$whatsapp}%' ";
		$sqlContador .= " AND whatsapp LIKE '%{$whatsapp}%' ";
	} 

 

	$sql .=' GROUP BY whatsapp ';
	...
	while($row=$result->fetch_assoc()){
		...
		*/
		if($option_edit){
			$edit='<button class="btn btn-info bt_responsive"  onClick="edit('.$row["whatsapp"].');" >
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM api_whatsapp_mensajes WHERE 1 = 1   "; 
