<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="campanasMailing/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'campanas_mailing','campanasMailing','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('campanas_mailing',$id);
	$registros_titutlo = 'Este registro esta ligado a : <br>';
	foreach ($tablasRelacionadas['tablas'] as $key => $value) {
		if($value['registros']>0 && $value['tabla']!='campanas_mailing_cuerpos' && $value['tabla']!='campanas_mailing_programadas' && $value['tabla']!='campanas_mailing_cartografias' && $value['tabla']!='campanas_mailing_tipos_ciudadanos' && $value['tabla']!='campanas_mailing_tipos_categorias_ciudadanos' && $value['tabla']!='campanas_mailing_encuestas' ){
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "campanasMailing/db_delete.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Camapaña Mail</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?= $campana_mailing_cuerpoDatos['asunto']?></strong>
			</label><br>
			<label class="labelForm" id="labeltemaname">Cuerpo</label><br>
			<?=	$campana_mailing_cuerpoDatos['cuerpo'];	?>
