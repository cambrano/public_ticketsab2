<?php
	@session_start(); 
	$_SESSION['Paguinasub']="campanasMailing/update.php";  
	if(!empty($_GET)){
	...
	include __DIR__."/../functions/campanas_mailing_encuestas.php";
	...
	$redirectSecurity=redirectSecurity($id,'campanas_mailing','campanasMailing','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$campana_mailing_programadaDatos=campana_mailing_programadaDatos('',$id);
	$campana_mailing_encuestaDatos=campana_mailing_encuestaDatos('',$id);
	$campana_mailing_cartografiaDatos=campana_mailing_cartografiaDatos('',$id);
	$campanas_mailing_tipos_ciudadanosIdDatos=campanas_mailing_tipos_ciudadanosIdDatos('',$id); 
	$campanas_mailing_tipos_categorias_ciudadanosIdDatos=campanas_mailing_tipos_categorias_ciudadanosIdDatos('',$id); 
	...
	if($campana_mailing_cartografiaDatos['tipo_cartografia']=='municipios'){
		...
		$selectCartografia = municipios($campana_mailing_cartografiaDatos['id_tipo_cartografia']);
		$cartografia_texto = 'Municipio';
	}elseif($campana_mailing_cartografiaDatos['tipo_cartografia']=='secciones_ine'){
		include __DIR__."/../functions/secciones_ine.php";
		$selectCartografia = secciones_ine($campana_mailing_cartografiaDatos['id_tipo_cartografia']);
		...
		$selectCartografia = distritos_locales($campana_mailing_cartografiaDatos['id_tipo_cartografia']);
	...
	elseif($campana_mailing_cartografiaDatos['tipo_cartografia']=='distritos_federales'){
		...
		$selectCartografia = distritos_federales($campana_mailing_cartografiaDatos['id_tipo_cartografia']);
...
/*
	echo "<pre>";
	print_r($campanas_mailing_tipos_ciudadanosIdDatos);
	echo "</pre>";
...
*/
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			var id_cuerpo = '<?= $campana_mailing_cuerpoDatos['id'] ?>';
			if(id_cuerpo == ""){
				document.getElementById("sumbmit").disabled = false;
			...
			var id_programada = '<?= $campana_mailing_programadaDatos['id'] ?>';
			var id_cartografia = '<?= $campana_mailing_cartografiaDatos['id'] ?>';
			var id_campana_encuesta = '<?= $campana_mailing_encuestaDatos['id'] ?>';
			...
			<?php
				foreach ($tipos_ciudadanosDatos as $key => $value) {
					...
					var data = {
							'id' : '<?= $campanas_mailing_tipos_ciudadanosIdDatos[$value['id']] ['id'] ?>',
			...
			<?php
				foreach ($tipos_categorias_ciudadanosDatos as $key => $value) {
					...
					var data = {
							'id' : '<?= $campanas_mailing_tipos_categorias_ciudadanosIdDatos[$value['id']]['id'] ?>',
			...
			$.ajax({
				type: "POST",
				url: "campanasMailing/db_edit.php",
				...
				success: function(data) {
					...
					if(data=="SI"){
						...
						if(data==""){
							$("#homebody").load('campanasMailing/index.php');
						}else{
							document.getElementById("sumbmit").disabled = false;
							$("#mensaje").html(data);
							document.getElementById("mensaje").classList.add("mensajeError");
		...
		<div class="bodyform">
			<div class= "bodyheader">
				...
				<label class="descripcionForm">
					<font style="font-size: 13px;">Campos para modificar a campaña mailing.</font><br><br>
				</label><br>
