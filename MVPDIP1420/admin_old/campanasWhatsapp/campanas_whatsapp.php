<?php
	...
	$sql = "SELECT count(*) total FROM campanas_whatsapp WHERE 1 = 1   "; 
	...
	$sql = "SELECT 
			id,
			CASE
				WHEN tipo = 1 THEN 'bienvenida'
				WHEN tipo = 3 THEN 'encuesta'
				ELSE 'programada'
			END tipo,
			IF(status=1,'activo','no activo') status,
			nombre 
		FROM campanas_whatsapp WHERE 1 = 1  "; 
	...
	while($row=$result->fetch_assoc()){
		...
		$cancelar_correos='<button class="btn btn-primary bt_responsive"  onClick="cancelar_masivo('.$row["id"].');" >Cancelar</button>';
		$reenviar_correos_configurada='<button class="btn btn-primary bt_responsive"  onClick="reenviar_masivo_configurada('.$row["id"].');" >Configuración</button>';

		$reenviar_correos_todos='<button class="btn btn-primary bt_responsive"  onClick="reenviar_masivo_todos('.$row["id"].');" >Todos</button>';

		$reenviar_correos_segmentada='<button class="btn btn-primary bt_responsive"  onClick="reenviar_masivo_segmentada('.$row["id"].');" >Segmentada</button>';

		$nestedData[] =  "
			<div class='opciones_botones_1'>
				{$reenviar_correos_configurada} 
			</div>";
		$nestedData[] =  "
		<div class='opciones_botones_1'>
			{$reenviar_correos_segmentada} 
		</div>"; 
		$nestedData[] =  "
			<div class='opciones_botones_1'>
				{$cancelar_correos}
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM campanas_whatsapp WHERE 1 = 1   "; 
