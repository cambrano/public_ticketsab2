<?php
	...
	if(!empty($_POST)){
		...
		$id_campana_whatsapp = $_POST["campana_whatsapp"][0]['id'];

		$conexion->autocommit(FALSE);
		$success=true;

		$campana_whatsappDatos=campana_whatsappDatos($id_campana_whatsapp);
		...
		$id_encuesta = $_POST["campana_whatsapp_encuesta"][0]['id_encuesta'];
		...
		$tipo_cartografia = $_POST["campana_whatsapp_cartografia"][0]['tipo_cartografia'];
		$id_tipo_cartografia = $_POST["campana_whatsapp_cartografia"][0]['id_tipo_cartografia'];
		if(!empty($_POST["campana_whatsapp_cartografia"][0]['tipo_cartografia']) && !empty($_POST["campana_whatsapp_cartografia"][0]['id_tipo_cartografia']) ){
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],"secciones_ine_ciudadanos_campanas_whatsapp_programadas",$id_campana_whatsapp,'reenviar_masivos_segmentada','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
