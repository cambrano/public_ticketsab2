<?php
	...
	if(!empty($_GET)){
		$_SESSION['Paguinasub']="casillasVotos2021Status/delete.php";
	...
	$casilla_voto_2021_statusDatos=casilla_voto_2021_statusDatos($id);
	$id = $casilla_voto_2021_statusDatos['id'];

	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','casillas_votos_2021',$_COOKIE["id_usuario"]);

	$tablasRelacionadas = tablasRelacionadas('casillas_votos_2021_status',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2021Status/db_delete.php",
				data: dataString,
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?=$casilla_voto_2021_statusDatos['fechaR']; ?></strong>
			</label><br><br>
			<label class="labelForm" id="labeltemaname">Estatus</label><br>
			<label class="descripcionForm">
				<?php

				if($casilla_voto_2021_statusDatos['status'] == 1){
					$casilla_voto_2021_statusDatos['status'] = 'Abierto';
				}elseif ($casilla_voto_2021_statusDatos['status'] == 2) {
					$casilla_voto_2021_statusDatos['status'] = 'Cerrado Con Gente';
				}elseif ($casilla_voto_2021_statusDatos['status'] == 3) {
					$casilla_voto_2021_statusDatos['status'] = 'Cerrado';
				}else{
					$casilla_voto_2021_statusDatos['status'] = 'Inicio Con Conteo';
				}

				?>
				<strong><?=$casilla_voto_2021_statusDatos['status']; ?></strong>
			</label><br><br>
