<?php
	...
	if(!empty($_GET)){
		$_SESSION['Paguinasub']="casillasVotos2021StatusReportes/delete.php";
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	$id_casilla_voto_2021=$_SESSION['id_casilla_voto_2021']; 
	echo $redirectSecurity=redirectSecurity($id,'casillas_votos_2021_incidencias','casillasVotos2021StatusReportes','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','casillas_votos_2021_incidencias',$_COOKIE["id_usuario"]);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2021StatusReportes/db_delete.php",
				data: dataString,
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				...
				if($casilla_voto_2021_incidenciaDatos['status'] == 1){
					$casilla_voto_2021_incidenciaDatos['status'] = 'Verde';
				}elseif ($casilla_voto_2021_incidenciaDatos['status'] = 2) {
					$casilla_voto_2021_incidenciaDatos['status'] = 'Amarillo';
				}elseif ($casilla_voto_2021_incidenciaDatos['status'] = 3) {
					$casilla_voto_2021_incidenciaDatos['status'] = 'Rojo';
				}else{
					$casilla_voto_2021_incidenciaDatos['status'] = 'No tiene';
				}

				?>
				<strong><?=$casilla_voto_2021_incidenciaDatos['status']; ?></strong>
			</label><br><br>
