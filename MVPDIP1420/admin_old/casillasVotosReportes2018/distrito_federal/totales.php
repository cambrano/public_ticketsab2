<?php
	...
	$sql="
		...
		WHERE cvp.id_distrito_federal='{$id_distrito_federal}' AND cvp.tipo = '{$tipo}' AND cvp.id_seccion_ine = '{$id_seccion_ine}'
		GROUP BY cvp.id_distrito_federal,cvp.id_seccion_ine,cvp.id_casilla_voto_2018,cvp.id_partido_2018
	";
	$sql;
	...
	$sql="
		...
		WHERE si.id_distrito_federal = '$id_distrito_federal' AND si.id_seccion_ine = '{$id_seccion_ine}' AND si.tipo = '{$tipo}'
	";
	$result = $conexion->query($sql); 
	while($row=$result->fetch_assoc()){
