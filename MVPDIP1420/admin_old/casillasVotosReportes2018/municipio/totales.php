<?php
	...
	$sql="
		SELECT
			...
			cvp.id_municipio,
			cvp.id_seccion_ine,
			cvp.id_casilla_voto_2018
		FROM  casillas_votos_partidos_2018 cvp
		LEFT JOIN partidos_2018 p
		ON p.id = cvp.id_partido_2018
		WHERE cvp.id_municipio='{$id_municipio}' AND cvp.tipo = '{$tipo}' AND cvp.id_seccion_ine = '{$id_seccion_ine}'
		GROUP BY cvp.id_municipio,cvp.id_seccion_ine,cvp.id_casilla_voto_2018,cvp.id_partido_2018
	";
	$sql;
	...
	while($row=$result->fetch_assoc()){
		...
		if($row['clave_partidos_coaliciones'] != ''){
			$partidos_coaliciones[$row['id_casilla_voto_2018']][$row['nombre_corto']]=$row;
		}else{
			$partidos_sin_coaliciones[$row['id_casilla_voto_2018']][$row['nombre_corto']]=$row;
		} 
		$num=$num+1;
	}
	...
	$sql="
		SELECT
			...
			(SELECT COUNT(cv.id) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') casillas,
			(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}' ) lista_nominal,
			(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_nulos,
			(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2018 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_can_nreg,
			(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2018 cv WHERE cv.id_seccion_ine = si.id_seccion_ine AND cv.tipo='{$tipo}') votos_validos
		FROM casillas_votos_2018 si
		WHERE si.id_municipio = '$id_municipio' AND si.id_seccion_ine = '{$id_seccion_ine}' AND si.tipo = '{$tipo}'
	";
	$result = $conexion->query($sql); 
	while($row=$result->fetch_assoc()){
...
<style type="text/css">
	...
	/* For mobile phones: */
		...
		.totales,.div50Reporte,.div25Reporte,.div50ReporteSNF,.div33Reporte{
			width: 100%;
		}
		.div25Reporte,.div50Reporte,.div100Reporte{
			padding: 10px;
	...
	/* For mobile phones: */
		.totales,.div50Reporte,.div25Reporte,.div50ReporteSNF,.div25Reportepartidos,.div30Reportepartidos,.div33Reporte{
			width: 100%;
		}
		.div25Reporte,.div50Reporte,.div50ReporteSNF,.div100Reporte{
			padding: 10px;
