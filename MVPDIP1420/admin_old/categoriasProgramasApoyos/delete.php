<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="categoriasProgramasApoyos/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'categorias_programas_apoyos','categoriasProgramasApoyos','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('categorias_programas_apoyos',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "categoriasProgramasApoyos/db_delete.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Categoría Programas Apoyos</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Categoría Programas Apoyos</label><br>
			<label class="descripcionForm">
				<strong><?= $categoria_programa_apoyoDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
