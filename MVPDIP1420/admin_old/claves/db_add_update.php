<?php
	...
	if(!empty($id)){ 
		...
		$registroCompara= registrosCompara("claves",$_POST["claves"][0],1);
	...
	if($registroCompara){
		if(!empty($_POST["claves"][0])){
			...
			if(empty($id)){ 
				...
				$inset_claves= "INSERT INTO claves ($fields_pdo) VALUES ($values_pdo);";
				...
				$fields_pdo = "`".implode('`,`', array_keys($_POST["claves"][0]))."`";
				$values_pdo = "'".implode("','", $_POST["claves"][0])."'";
				$inset_claves_historicos= "INSERT INTO claves_historicos ($fields_pdo) VALUES ($values_pdo);";
				$conexion->autocommit(FALSE);
				...
				$conexion->autocommit(FALSE);
				$update_claves = "UPDATE claves SET ". join(",",$valueSets) . " WHERE id=".$id;
			...
			if($success){
				$log= logUsuario($_COOKIE["id_usuario"],'claves',$id,$tipo,'',$fechaH);
				if($log==true){
					echo "SI";
					$conexion->commit();
					$conexion->close();
