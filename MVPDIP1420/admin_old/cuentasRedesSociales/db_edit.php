<?php
	...
	$cuenta_red_socialClaveVerificacion=cuenta_red_socialClaveVerificacion($_POST["cuenta_red_social"][0]["clave"],$_POST["cuenta_red_social"][0]['id'],1);
	...
	if( registrosCompara("cuentas_redes_sociales",$_POST['cuenta_red_social'][0],1)){
		if(!empty($_POST)){
			...
			$cuenta_red_socialDatos=cuenta_red_socialDatos($_POST["cuenta_red_social"][0]['id']);
			$_POST["cuenta_red_social"][0]=array_merge($cuenta_red_socialDatos, $_POST["cuenta_red_social"][0]);


			if($cuenta_red_socialDatos['ip'] != $_POST["cuenta_red_social"][0]['ip']){
			...
			$success=true;
			foreach($_POST['cuenta_red_social'] as $keyPrincipal => $atributos) {
			...
			$update_cuentas_redes_sociales = "UPDATE cuentas_redes_sociales SET ". join(",",$valueSets) . " WHERE id=".$id;
			$conexion->autocommit(FALSE);
			$update_cuentas_redes_sociales=$conexion->query($update_cuentas_redes_sociales);
			$num=$conexion->affected_rows;
			if(!$update_cuentas_redes_sociales || $num=0){
				$success=false;
				echo "<br>";
				echo "ERROR update_cuentas_redes_sociales"; 
				var_dump($conexion->error);
			}

			unset($_POST["cuenta_red_social"][0]['id']); 
			$id_cuenta_red_social=$_POST['cuenta_red_social'][0]['id_cuenta_red_social']=$id;
			...
			if($success){
				$log= logUsuario($_COOKIE["id_usuario"],'cuentas_redes_sociales',$id_cuenta_red_social,'Update','',$fechaH);
				if($log==true){
					echo "SI";
					$conexion->commit();
					$conexion->close();
