<?php
	...
	$tipo_actividadClaveVerificacion=tipo_actividadClaveVerificacion($_POST["distrito_local"][0]["clave"],$_POST["distrito_local"][0]['id'],1);
	...
	if(!empty($_POST)){
		if( registrosCompara("distritos_locales",$_POST['distrito_local'][0],1) ){
			$distrito_localDatos=distrito_localDatos($_POST['distrito_local'][0]['id']);
			//$_POST['registro']=$fechaH;
			$_POST["distrito_local"][0]['fechaR']=$fechaH;
			$_POST["distrito_local"][0]['codigo_plataforma']=$codigo_plataforma;
			$id_distrito_local = $_POST['distrito_local'][0]['id'];


			$success=true;
			foreach($_POST['distrito_local'] as $keyPrincipal => $atributos) {
			...
			$update_distritos_locales = "UPDATE distritos_locales SET ". join(",",$valueSets) . " WHERE id=".$id;
			$conexion->autocommit(FALSE);
			$update_distritos_locales=$conexion->query($update_distritos_locales);
			$num=$conexion->affected_rows;
			if(!$update_distritos_locales || $num=0){
				$success=false;
				echo "<br>";
				echo "ERROR update_distritos_locales"; 
				var_dump($conexion->error);
			}

			unset($_POST["distrito_local"][0]['id']); 
			$id_distrito_local=$_POST['distrito_local'][0]['id_distrito_local']=$id;
			$fields_pdo = "`".implode('`,`', array_keys($_POST["distrito_local"][0]))."`";
			$values_pdo = "'".implode("','", $_POST['distrito_local'][0])."'";
			$insert_distritos_locales_historicos= "INSERT INTO distritos_locales_historicos ($fields_pdo) VALUES ($values_pdo);";
			$insert_distritos_locales_historicos=$conexion->query($insert_distritos_locales_historicos);
			$num=$conexion->affected_rows;
			if(!$insert_distritos_locales_historicos || $num=0){
				$success=false;
			...
			foreach ($_SESSION['limites'] as $key => $value) {
				...
				if($value['status']==1 && $value['id']==''){
					...
					unset($value['numero']);
					unset($value['status']);
					$value['id_distrito_local'] = $id_distrito_local;
					$value['fechaR'] = $fechaH;
			...
			if($success){
				$log= logUsuario($_COOKIE["id_usuario"],'distritos_locales',$id_distrito_local,'Update','',$fechaH);
				if($log==true){
					echo "SI";
					$conexion->commit();
					$conexion->close();
