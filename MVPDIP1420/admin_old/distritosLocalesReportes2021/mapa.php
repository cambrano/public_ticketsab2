<?php
	...
	if(!empty($_POST)){
		...
		if($id_distrito_local!=''){
			$sqlPartidos = " AND cvp.id_distrito_local IN ({$id_distrito_local}) ";
		}
		...
		$sql="
			...
			GROUP BY cvp.id_distrito_local,cvp.id_partido_2021
		";
		...
		while($row=$result->fetch_assoc()){
			...
			if($row['clave_partidos_coaliciones'] != ''){
				$partidos_coaliciones[$row['id_distrito_local']][$row['nombre_corto']]=$row;
			}else{
				$partidos_sin_coaliciones[$row['id_distrito_local']][$row['nombre_corto']]=$row;
			} 
			$num=$num+1;
		}
		$sql="
			SELECT
				...
				si.latitud,
				si.longitud,
				(SELECT COUNT(cv.id) FROM casillas_votos_2021 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') casillas,
				(SELECT COUNT(cv.id) FROM secciones_ine cv WHERE cv.id_distrito_local = si.id ) secciones_ine,

				(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2021 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}' ) lista_nominal,
				(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2021 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_nulos,
				(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2021 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_can_nreg,
				(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2021 cv WHERE cv.id_distrito_local = si.id AND cv.tipo='{$tipo}') votos_validos
			FROM distritos_locales si WHERE 1
		";
		if($id_distrito_local!=''){
			$sql.=" AND si.id IN ({$id_distrito_local}) ";
		}
		$result = $conexion->query($sql); 
		while($row=$result->fetch_assoc()){
			...
			foreach ($partidos_sin_coaliciones[$row['id']] as $clave => $array) {
				...
				foreach ($partidos_coaliciones[$row['id']] as $nombre_corto => $arraysc) {
					...
					$posicion_coalicion = explode('_', $nombre_corto);
					/*
					...
					if ($pos !== false ) {
					*/
					if (in_array($array['nombre_corto'], $posicion_coalicion)) {
				...
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['id'] = $array['id'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['clave'] = $clave;
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['nombre_corto'] = $array['nombre_corto'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['nombre'] = $array['nombre'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['principal'] = $array['principal'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['logo'] = $array['logo'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['color_border'] = $array['color_border'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['color_background'] = $array['color_background'];

				$datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_individual'] = $array['votos'];
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['coaliciones_sin_orden'] = $coaliciones;
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_coaliciones'] = $sum_coaliciones;
				...
				foreach ($coalicion_orden_individual as $votos => $partidos_array) {
					foreach ($partidos_array as $index => $partido) {
						$datos_distritos_locales[$row['id']]['partidos'][$clave]['coaliciones_orden_votos_individual'][$partido]=$votos;
						if($clave != $partido){
							$total_votos_individual = $total_votos_individual + $votos;
						}
					}
				}
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_coaliciones_individual'] = $total_votos_individual;
				$datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_totales'] = $datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_individual'] + $datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_coaliciones'] + $datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_coaliciones_individual'] ;


				$ordena_votos_individual[$row['id']][$array['votos']] [] = $clave ;
				$ordena_votos_totales[$row['id']][ $datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_totales'] ] [] = $clave ;

				#$partidos_orden_individual[ $datos_distritos_locales[$row['id']]['partidos'][$clave]['votos_individual'] ][$array['votos']][] = $array['nombre_corto']; 
			}
			//! Importante
			...
			foreach ($ordena_votos_individual[$row['id']] as $votos => $partidos_array) {
				foreach ($partidos_array as $index => $partido) {
					$datos_distritos_locales[$row['id']]['orden_votos_individual']['partidos'][$partido]=$votos;
					$validador = $validador + $votos;
					if(empty($datos_distritos_locales[$row['id']]['orden_votos_individual']['primera_fuerza'])){
						$datos_distritos_locales[$row['id']]['orden_votos_individual']['primera_fuerza'] = $partido;
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal']==1 ){
							$sistema = true;
						}
					}elseif (empty($datos_distritos_locales[$row['id']]['orden_votos_individual']['segunda_fuerza'])  ) {
						$datos_distritos_locales[$row['id']]['orden_votos_individual']['segunda_fuerza'] = $partido;
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal']==1 ){
							$sistema = true;
						}
					}else{
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal'] == 1 && $sistema == false){
							$datos_distritos_locales[$row['id']]['orden_votos_individual']['sistema'] = $partido;
						}
					}
				}
			}
			if($validador <= 0){
				$datos_distritos_locales[$row['id']]['orden_votos_individual']['segunda_fuerza'] = $datos_distritos_locales[$row['id']]['orden_votos_individual']['primera_fuerza'] ='NoData';
			}
			$validador = 0;
			foreach ($ordena_votos_totales[$row['id']] as $votos => $partidos_array) {
				foreach ($partidos_array as $index => $partido) {
					$datos_distritos_locales[$row['id']]['orden_votos_totales']['partidos'][$partido]=$votos;
					$validador = $validador + $votos;
					if(empty($datos_distritos_locales[$row['id']]['orden_votos_totales']['primera_fuerza'])){
						$datos_distritos_locales[$row['id']]['orden_votos_totales']['primera_fuerza'] = $partido;
						$primera_fuerza = $partido;
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal']==1 ){
							$sistema = true;
						}
					}elseif (empty($datos_distritos_locales[$row['id']]['orden_votos_totales']['segunda_fuerza']) && empty($datos_distritos_locales[$row['id']]['partidos'][$partido]['coaliciones_orden_votos_individual'][$primera_fuerza]  )  ) {
						$datos_distritos_locales[$row['id']]['orden_votos_totales']['segunda_fuerza'] = $partido;
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal']==1 ){
							$sistema = true;
						}
					}else{
						if($datos_distritos_locales[$row['id']]['partidos'][$partido]['principal'] == 1 && $sistema == false){
							$datos_distritos_locales[$row['id']]['orden_votos_totales']['sistema'] = $partido;
						}
					}
				}
			}
			if($validador <= 0){
				$datos_distritos_locales[$row['id']]['orden_votos_totales']['segunda_fuerza'] = $datos_distritos_locales[$row['id']]['orden_votos_totales']['primera_fuerza'] ='NoData';
			}

			if($partido_ganador_id !=''){
				$partidos_search = explode(',', $partido_ganador_id);

				$primera_fuerza = $datos_distritos_locales[$row['id']]['orden_votos_individual']['primera_fuerza'];
				if($primera_fuerza == 'NoData'){
					$datos_primera_fuerza['id'] = 0;
				}else{
					$datos_primera_fuerza = $datos_distritos_locales[$row['id']]['partidos'][$primera_fuerza];
				}

				if(in_array($datos_primera_fuerza['id'], $partidos_search) == false ){
					unset($datos_distritos_locales[$row['id']]);
				}
			}
	...
	#echo "<pre>";
	#print_r($datos_distritos_locales);
	#echo "</pre>";
	...
	<script type="text/javascript">
		function myMap(){
			...
			foreach ($distritos_locales_parametrosDatosMapa as $key => $value) {
				...
				$primera_fuerza = $datos_distritos_locales[$key]['orden_votos_individual']['primera_fuerza'];
				$color_background = $datos_distritos_locales[$key]['partidos'][$primera_fuerza]['color_background'] ;
				$color_border = $datos_distritos_locales[$key]['partidos'][$primera_fuerza]['color_border'] ;
				if($primera_fuerza == "" ){
			...
			foreach ($datos_distritos_locales as $key => $value) {
				$primera_fuerza = $datos_distritos_locales[$key]['orden_votos_individual']['primera_fuerza'];
				$logo = $datos_distritos_locales[$key]['partidos'][$primera_fuerza]['logo'] ;
			...
			var infoWindowContent = [
					...
					foreach ($datos_distritos_locales as $key => $value){
						...
						$primera_fuerza = $datos_distritos_locales[$key]['orden_votos_individual']['primera_fuerza'];
						$datos_primera_fuerza = $datos_distritos_locales[$key]['partidos'][$primera_fuerza];
						foreach ($datos_primera_fuerza['coaliciones_orden_votos_individual'] as $partido => $votos) {
							$coali_primera_fuerza[] = $partido.': '.$votos;
						}
						$segunda_fuerza = $datos_distritos_locales[$key]['orden_votos_individual']['segunda_fuerza'];
						$datos_segunda_fuerza = $datos_distritos_locales[$key]['partidos'][$segunda_fuerza];
						foreach ($datos_segunda_fuerza['coaliciones_orden_votos_individual'] as $partido => $votos) {
							$coali_segunda_fuerza[] = $partido.': '.$votos;
						}
						...
						$div = '<div class="divMapa">
									...
									<div class="datos_votos">
										...
										<p style="padding:0px;text-align:left;">
											...
											Votos Coalición Ind: <b>'.number_format($datos_primera_fuerza['coaliciones_orden_votos_individual'], 0, '.', ',').'</b><br>
									...
									<div class="datos_partido">
										<p style="padding:0px;text-align:left;">
											...
											Votos Coalición Ind: <b>'.number_format($datos_segunda_fuerza['coaliciones_orden_votos_individual'], 0, '.', ',').'</b><br>
