	...
	if(!empty($_POST)){
		...
		$sql="
			SELECT 
				...
				( SELECT s.fecha_hora FROM secciones_ine_ciudadanos_encuestas s WHERE s.id_distrito_federal = m.id AND s.id_encuesta ='{$id_encuesta}' AND s.id_distrito_federal = m.id ORDER BY s.fecha_hora DESC LIMIT 1) ultima_encuesta,
			...
			WHERE 1
		";
		if($_POST['searchTable'][0]['id_distrito_federal']!=""){
			$sql.= " AND m.id IN ({$_POST['searchTable'][0]['id_distrito_federal']}) ";
		}
