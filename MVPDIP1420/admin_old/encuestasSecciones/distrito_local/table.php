	...
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_ciudadanos_encuestas-tabla').DataTable( {
				...
				"ajax":{
					url :"encuestasSecciones/distrito_local/secciones_ine_ciudadanos_encuestas.php", // json datasource
			...
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="encuestasSecciones/distrito_local/index.php?cot="+ids_string;   
		...
		function edit(valor){
			link="encuestasSecciones/distrito_local/update.php?id="+valor; 
			var link2="encuestasSecciones/distrito_local/update.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function downloadExcel(){
			...
			?>
			var link="encuestasDistritosLocales/excel/index.php?cot=<?=$_COOKIE['pageService']?>"; 
			//window.open(link);
		...
		function encuestasSeccionesDistritoLocal(valor){
			link="encuestasSecciones/distrito_local/index.php?id="+valor; 
			var link2="encuestasSecciones/distrito_local/index.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
		...
		function encuestasSeccion(valor){
			link="encuestasSecciones/distrito_local/seccion.php?id="+valor; 
			var link2="encuestasSecciones/distrito_local/seccion.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
