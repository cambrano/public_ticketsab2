<?php
	...
	$sql = "
		SELECT 
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos WHERE id_municipio ='{$id_municipio}' {$sql_seccion} ) totales_ciudadanos,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas  WHERE id_encuesta = 1 AND id_municipio ='{$id_municipio}' {$sql_seccion} ) totales_encuestados,
		...
		WHERE sice.id_encuesta = '{$id_encuesta}' AND sice.id_municipio ='{$id_municipio}' ";
	...
	$sql="
		SELECT 
			...
			cr.id_cuestionario,
			cr.respuesta,
			(SELECT COUNT(*) FROM secciones_ine_ciudadanos_encuestas_respuestas sicer WHERE sicer.id_cuestionario_respuesta = cr.id AND sicer.id_municipio ='{$id_municipio}' {$sql_seccion} ) respuestas
	...
	//echo "<pre>";
	//print_r($datos_municipios_encuestados);
	//echo "</pre>";
	if($id_seccion_ine==''){
		$sql="
			SELECT 
