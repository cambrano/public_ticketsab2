<?php
	...
	$sql = "SELECT 
			...
			IF(
				(SELECT ce.id FROM correos_electronicos ce WHERE ce.id_identidad = e.id ) IS NULL,
				'no',
				'si'
			) correos_electronicos,

			IF(
				(SELECT crs.id FROM cuentas_redes_sociales crs WHERE crs.id_identidad = e.id ) IS NULL,
				'no',
				'si'
			) cuentas_redes_sociales,
			IF(
				(SELECT do.id FROM documentos_oficiales do WHERE do.id_identidad = e.id ) IS NULL,
				'no',
				'si'
	...
	$id_estado=$_SESSION['searchTable']['id_estado'];
	if( $id_estado!="" ){     //name
		$post_search=true;
	...
	$correos_electronicos=$_SESSION['searchTable']['correos_electronicos'];
	...
	$cuentas_redes_sociales=$_SESSION['searchTable']['cuentas_redes_sociales'];
