...
<script type="text/javascript">
	function guardarImportacion(){
		...
		var ruta = "importacionesSistema/casillas_votos_2022_revocacion_mandato/casillas_votos_2022_revocacion_mandato_db_add.php";
...
<?php
	...
	if($tabla=="casillas_votos_2022_revocacion_mandato"){
		$columnData = array(
		...
		while(($line = fgetcsv($csvFile)) !== FALSE){
			...
			if($countdraw > 1){
				...
				foreach ($columnData as $key => $value) {
					...
					if(in_array("requerido_opcional", $value) && $line_valor=="" ){
						if($color[$num][$key]!=""){
							$color[$num][$key]='';
							$unique_db_error_no_encontrado[]="";
						}
					}

					if($key=='municipio'){
						$municipioId=municipioId($line_valor,$id_estado);
