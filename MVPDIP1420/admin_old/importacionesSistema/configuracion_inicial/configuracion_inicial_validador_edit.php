...
<script type="text/javascript">
	function guardarImportacion(){
		...
		var ruta = "importacionesSistema/configuracion_inicial/configuracion_inicial_db_edit.php";
...
<?php
	...
	if($tabla=="configuracion"){
		$columnData = array(
					...
					'url_base'=>array('alfanumerico',''),
					'nombre_completo_representante'=>array('slogan',''),
		...
		while(($line = fgetcsv($csvFile)) !== FALSE){
			...
			if($countdraw > 1){
				...
				foreach ($columnData as $key => $value) {
					...
					$value;
					//validaciones
					if(in_array("numerico", $value)){
						if(!validarNumerico($line_valor)){
