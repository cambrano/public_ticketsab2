<?php
	...
	include "../../functions/correos_electronicos.php";
	...
	if(!empty($_POST)){
		...
		foreach ($data as $key => $value) {
			...
			if($value['id']!=""){
				$primaria="id_correo_electronico";
				//cambios de columnas
				//$value['id_identidad']=claveId($value['clave_identidades'],'identidades');
				//unset($value['clave_identidades']);
				//$value['id_servidor_correo']=claveId($value['clave_servidores_correos'],'servidores_correos');
				//unset($value['clave_servidores_correos']);
				...
				if($array_servidores_correos[$value['clave_servidores_correos']] ==""){
					$value['id_servidor_correo'] = claveId($value['clave_servidores_correos'],'servidores_correos');
					$array_servidores_correos[$value['clave_servidores_correos']] = $value['id_servidor_correo'];
					unset($value['clave_servidores_correos']);
				}else{
					$value['id_servidor_correo'] = $array_servidores_correos[$value['clave_servidores_correos']];
					unset($value['clave_servidores_correos']);
				}
