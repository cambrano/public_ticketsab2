<?php
	...
	if(!empty($_POST)){
		...
		foreach ($data as $key => $value) {
			...
			if($value['id']!=""){
				$primaria="id_cuenta_red_social_actividad";
				//cambios de columnas
				//$value['id_identidad']=claveId($value['clave_identidades'],'identidades');
				//unset($value['clave_identidades']);

				//$value['id_cuenta_red_social']=claveId($value['clave_cuentas_redes_sociales'],'cuentas_redes_sociales');
				//unset($value['clave_cuentas_redes_sociales']);

				//$value['id_red_social']=claveId($value['clave_redes_sociales'],'redes_sociales');
				//unset($value['clave_redes_sociales']);

				//$value['id_tipo_actividad']=claveId($value['clave_tipos_actividades'],'tipos_actividades');
				//unset($value['clave_tipos_actividades']);

				if($array_identidades[$value['clave_identidades']] ==""){
					$value['id_identidad'] = claveId($value['clave_identidades'],'identidades');
					$array_identidades[$value['clave_identidades']] = $value['id_identidad'];
					unset($value['clave_identidades']);
				}else{
					$value['id_identidad'] = $array_identidades[$value['clave_identidades']];
					unset($value['clave_identidades']);
				}

				if($array_cuentas_redes_sociales[$value['clave_cuentas_redes_sociales']] ==""){
					$value['id_cuenta_red_social'] = claveId($value['clave_cuentas_redes_sociales'],'cuentas_redes_sociales');
					$array_cuentas_redes_sociales[$value['clave_cuentas_redes_sociales']] = $value['id_cuenta_red_social'];
					unset($value['clave_cuentas_redes_sociales']);
				}else{
					$value['id_cuenta_red_social'] = $array_cuentas_redes_sociales[$value['clave_cuentas_redes_sociales']];
					unset($value['clave_cuentas_redes_sociales']);
				}

				if($array_redes_sociales[$value['clave_redes_sociales']] ==""){
					$value['id_red_social'] = claveId($value['clave_redes_sociales'],'redes_sociales');
					$array_redes_sociales[$value['clave_redes_sociales']] = $value['id_red_social'];
					unset($value['clave_redes_sociales']);
				}else{
					$value['id_red_social'] = $array_redes_sociales[$value['clave_redes_sociales']];
					unset($value['clave_redes_sociales']);
				}

				if($array_tipos_actividades[$value['clave_tipos_actividades']] ==""){
					$value['id_tipo_actividad'] = claveId($value['clave_tipos_actividades'],'tipos_actividades');
					$array_tipos_actividades[$value['clave_tipos_actividades']] = $value['id_tipo_actividad'];
					unset($value['clave_tipos_actividades']);
				}else{
					$value['id_tipo_actividad'] = $array_tipos_actividades[$value['clave_tipos_actividades']];
					unset($value['clave_tipos_actividades']);
				}

				$value['fecha_hora_emision'] = $value['fecha_emision']." ".$value['hora_emision'];
