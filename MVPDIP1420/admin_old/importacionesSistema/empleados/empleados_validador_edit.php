...
<script type="text/javascript">
	function guardarImportacion(){
		...
		var ruta = "importacionesSistema/empleados/empleados_db_edit.php";
...
<?php
	...
	if($tabla=="empleados"){
		$columnData = array(
					...
					'status'=>array('status',''),
					'id_empleado_grupo'=>array('alfanumerico',''),
				);
	}
		...
		while(($line = fgetcsv($csvFile)) !== FALSE){
			...
			if($countdraw > 1){
				...
				foreach ($columnData as $key => $value) {
					...
					if($key=='clave'){
						$id=claveId($line_valor,'empleados');
						if(!claveValidadorSistema($line_valor,'empleados')){
