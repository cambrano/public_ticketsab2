<?php
	...
	if(!empty($_POST)){
		...
		foreach ($data as $key => $value) {
			...
			if($value['municipio']!=""){
				$value['id_municipio']=municipioIdClave($value['municipio'],$id_estado);
				$value['clave_municipio'] = str_pad($value['municipio'],  4, "0",STR_PAD_LEFT); 
				unset($value['municipio']);
			}

			if($array_distritos_locales[$value['clave_distritos_locales']] ==""){
				$value['id_distrito_local'] = claveId($value['clave_distritos_locales'],'distritos_locales');
				$value['clave_distrito_local'] = $value['clave_distritos_locales'];
				$array_distritos_locales[$value['clave_distritos_locales']] = $value['id_distrito_local'];
				unset($value['clave_distritos_locales']);
			}else{
				$value['id_distrito_local'] = $array_distritos_locales[$value['clave_distritos_locales']];
				$value['clave_distrito_local'] = $value['clave_distritos_locales'];
				unset($value['clave_distritos_locales']);
			}

			if($array_distritos_federales[$value['clave_distritos_federales']] ==""){
				$value['id_distrito_federal'] = claveId($value['clave_distritos_federales'],'distritos_federales');
				$value['clave_distrito_federal'] = $value['clave_distritos_federales'];
				$array_distritos_federales[$value['clave_distritos_federales']] = $value['id_distrito_federal'];
				unset($value['clave_distritos_federales']);
			}else{
				$value['id_distrito_federal'] = $array_distritos_federales[$value['clave_distritos_federales']];
				$value['clave_distrito_federal'] = $value['clave_distritos_federales'];
				unset($value['clave_distritos_federales']);
			}

			if($array_cuarteles[$value['clave_cuarteles']] ==""){
				$value['id_cuartel'] = claveId($value['clave_cuarteles'],'cuarteles');
				$array_cuarteles[$value['clave_cuarteles']] = $value['id_cuartel'];
				unset($value['clave_cuarteles']);
			}else{
				$value['id_cuartel'] = $array_cuarteles[$value['clave_cuarteles']];
				unset($value['clave_cuarteles']);
			}
			...
			if($value['id']!=""){
				if($tabla=="secciones_ine"){
					$primaria="id_seccion_ine";
				}
				//$value['fechaR']=$fechaH;
