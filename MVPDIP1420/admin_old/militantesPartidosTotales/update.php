<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="militantesPartidosTotales/update.php";  
	...
	$redirectSecurity=redirectSecurity($id,'militantes_partidos','militantesPartidosTotales','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	if($id_partido_legado!=""){
		...
		$nombre_completo = $partido_legadoDatos['nombre_completo'];
		$id_seccion_ine = $partido_legadoDatos['id_seccion_ine'];
	}else{
	...
	$militante_partidoDatos['clave_elector'] = $row['clave_elector'];
	$image = '../admin/ftpFiles/files/'.$militante_partidoDatos['name'];
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			var id_partido_legado = '<?= $id_partido_legado ?>'; 
			if(id_partido_legado == ""){
			...
			*/
			...
			formData.append('clave_elector' , clave_elector);
			...
			var ruta = "militantesPartidosTotales/db_edit.php";
			...
			$.ajax({
				...
				success: function(data){ 
					if(data=="SI"){
						...
						$("#homebody").load("militantesPartidosTotales/index.php?refresh=1");
					}else{
						if(data==""){
							document.getElementById("mensaje").classList.add("mensajeError");
							$("#homebody").load("militantesPartidosTotales/index.php?refresh=1");
