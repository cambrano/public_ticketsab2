<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE tipo = 0 ) votos_nulos_2021,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE tipo = 0 ) votos_can_nreg_2021,
				(SELECT COUNT(id) FROM secciones_ine ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE tipo = 0) casillas_2021,
				(SELECT COUNT(id) FROM distritos_locales ) distritos_locales_2021,
				(SELECT COUNT(id) FROM distritos_federales ) distritos_federales_2021,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE tipo = 0 ) total_lista_nominal_2021,
				(SELECT COUNT(id) FROM municipios WHERE id_estado='{$id_estado}') municipios,
				( SELECT id_partido_2021 votos FROM casillas_votos_partidos_2021 WHERE tipo = 0 GROUP BY id_partido_2021 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2021,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos ) ciudadanos_totales,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE check_in=1 AND check_out=1 ) ciudadanos_totales_doble_check,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE check_in=1 ) ciudadanos_totales_check_in,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE check_out=1 ) ciudadanos_totales_check_out,
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic LEFT JOIN secciones_ine_ciudadanos_check_2021 sicc2021 ON sic.id = sicc2021.id_seccion_ine_ciudadano WHERE IF(sicc2021.check_in IS NULL, 0,sicc2021.check_in ) = 0 AND IF(sicc2021.check_out IS NULL, 0,sicc2021.check_out ) = 0) ciudadanos_totales_sin_check

			FROM
			(SELECT SUM(votos) votos_validos_2021 FROM  casillas_votos_partidos_2021 WHERE tipo = 0 )t";
	...
	$votos_nulos_porcentaje_2021 = truncar($votos_nulos_porcentaje_2021,2);
	...
	if($extranjeros_mode){
		...
		$distritos_locales_2021 = $row['distritos_locales_2021']-1;
		$municipios = $row['municipios']-1;
		$extranjero = 1;
	}else{
		$distritos_federales_2021 = $row['distritos_federales_2021'];
		$distritos_locales_2021 = $row['distritos_locales_2021'];
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2021.votos) FROM casillas_votos_partidos_2021 cpv2021 WHERE cpv2021.id_partido_2021 = p2021.id AND cpv2021.tipo = 0 ) votos_totales_2021
			FROM partidos_2021 p2021
			WHERE p2021.principal=1 AND p2021.tipo = 0 ";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	...
	$sql="
		SELECT
			...
			p2021.principal,
			(SELECT SUM(cvp2021.votos) FROM casillas_votos_partidos_2021 cvp2021 WHERE cvp2021.id_partido_2021 = p2021.id AND cvp2021.tipo = 0) votos_2021
		FROM partidos_2021 p2021 WHERE p2021.tipo = 0
	";
	$sql;
...
<div class="totales">
	<div style="width: 100%;display: table;padding: 5px 5px 5px 0px;background-color: white">
		<div style="background-color: white;padding: 5px;display: table;">
			...
			<div class="div50Reporte" style="padding: 2px; margin: 0px;">
				<table style="table-layout: fixed; width: 100%" cellspacing="0" cellpadding="0">
					...
					<tbody>
						<tr>
							<td style="text-align: left; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.6); border-bottom: 1px solid white"><font class="fontLabelReporte">Extranjero:</font></td>
							<td style="text-align: right; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.3); border-bottom: 1px solid white">
								<font class="fontDataReporte" style="font-size: 12px">
									<?=number_format($extranjero, 0, '.', ','); ?>
								</font>
							</td>
						</tr>
						<tr>
							<td style="text-align: left; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.6); border-bottom: 1px solid white"><font class="fontLabelReporte">Municipios:</font></td>
							<td style="text-align: right; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.3); border-bottom: 1px solid white">
								<font class="fontDataReporte" style="font-size: 12px">
									<?=number_format($municipios, 0, '.', ','); ?>
								</font>
							</td>
						</tr>
						<tr>
		...
		<div style="background-color: white;padding: 0px;display: table;">
			<?php
			$votos_validos_2021;
