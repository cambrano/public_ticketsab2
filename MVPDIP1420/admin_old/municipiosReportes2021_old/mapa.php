<?php
	...
	if(!empty($_POST)){
		...
		$sql="
			...
			GROUP BY cvp.id_municipio,cvp.id_partido_2021
		";
		...
		while($row=$result->fetch_assoc()){
			...
			if($row['clave_partidos_coaliciones'] != ''){
				$partidos_coaliciones[$row['id_municipio']][$row['nombre_corto']]=$row;
			}else{
				$partidos_sin_coaliciones[$row['id_municipio']][$row['nombre_corto']]=$row;
			} 
			$num=$num+1;
		}
		$sql="
			SELECT
				...
				si.latitud,
				si.longitud,
				(SELECT COUNT(cv.id) FROM casillas_votos_2021 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') casillas,
				(SELECT COUNT(cv.id) FROM secciones_ine cv WHERE cv.id_municipio = si.id ) secciones_ine,

				(SELECT SUM(cv.lista_nominal) FROM casillas_votos_2021 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}' ) lista_nominal,
				(SELECT SUM(cv.votos_nulos) FROM casillas_votos_2021 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_nulos,
				(SELECT SUM(cv.votos_can_nreg) FROM casillas_votos_2021 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_can_nreg,
				(SELECT SUM(cv.votos) FROM casillas_votos_partidos_2021 cv WHERE cv.id_municipio = si.id AND cv.tipo='{$tipo}') votos_validos
		...
		while($row=$result->fetch_assoc()){
			...
			if($partido_ganador_id !=''){
				...
				$segunda_fuerza = $datos_municipios[$row['id']]['orden_votos_individual']['primera_fuerza'];
				if($primera_fuerza=='NoData'){
					...
					$datos_segunda_fuerza = $datos_municipios[$row['id']]['partidos'][$segunda_fuerza];
				}
	...
	<script type="text/javascript">
		function myMap(){
			...
			foreach ($municipios_parametrosDatosMapa as $key => $value) {
				...
				$color_background = $datos_municipios[$key]['partidos'][$primera_fuerza]['color_background'] ;
				$color_border = $datos_municipios[$key]['partidos'][$primera_fuerza]['color_border'] ;
				if($primera_fuerza == "" ){
