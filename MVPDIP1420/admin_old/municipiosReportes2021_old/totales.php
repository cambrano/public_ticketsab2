<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE tipo = '{$tipo}') votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE tipo = '{$tipo}') votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE tipo = '{$tipo}') casillas,
				(SELECT COUNT(id) FROM municipios WHERE id_estado ='{$id_estado}' ) municipios,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE tipo = '{$tipo}') total_lista_nominal
			FROM
			(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2021 WHERE tipo = '{$tipo}')t";
                ...
                <table style="table-layout: fixed; width: 100%" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                        <tr>
                            <td
                                style="text-align: left; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.6); border-bottom: 1px solid white">
                                <font class="fontLabelReporte">Municipios:</font>
                            </td>
                            <td
                                style="text-align: right; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.3); border-bottom: 1px solid white">
                                <font class="fontDataReporte" style="font-size: 12px">
