<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="partidos2018/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'partidos_2018','partidos2018','index');
	if($redirectSecurity!=""){
		die;
	}
	$partido_2018Datos=partido_2018Datos($id);
	$id_imagen=$partido_2018Datos['id_imagen'];
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','partidos_2018',$_COOKIE["id_usuario"]);
	$tablasRelacionadas = tablasRelacionadas('partidos_2018',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "partidos2018/db_delete.php",
				data: dataString,
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?= $partido_2018Datos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
