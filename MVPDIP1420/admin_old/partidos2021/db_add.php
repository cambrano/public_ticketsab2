<?php
	...
	if(!empty($_POST)){
		...
		$partido_2021ClaveVerificacion=partido_2021ClaveVerificacion($_POST["partido_2021"][0]['clave'],'',1);
		...
		$inset_partidos_2021= "INSERT INTO partidos_2021 ($fields_pdo) VALUES ($values_pdo);";

		$inset_partidos_2021=$conexion->query($inset_partidos_2021);
		$num=$conexion->affected_rows;
		if(!$inset_partidos_2021 || $num=0){
			$success=false;
			echo "ERROR inset_partidos_2021"; 
			var_dump($conexion->error);
		}

		$id=$_POST['partido_2021'][0]['id_partido_2021']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($_POST['partido_2021'][0]))."`";
		$values_pdo = "'".implode("','", $_POST['partido_2021'][0])."'";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'partidos_2021',$id,'Insert','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
