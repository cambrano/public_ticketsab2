<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="partidos/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'partidos_','partidos','index');
	if($redirectSecurity!=""){
		die;
	}
	$partido_Datos=partido_Datos($id);
	$id_imagen=$partido_Datos['id_imagen'];
	$moduloAccionPermisos = moduloAccionPermisos('sistema_unico_beneficiarios','partidos_',$_COOKIE["id_usuario"]);
	$tablasRelacionadas = tablasRelacionadas('partidos_',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		function cerrar(){
			$("#homebody").load('partidos/index.php');
		}
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			<label class="descripcionForm">
				<strong><?= $partido_Datos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
