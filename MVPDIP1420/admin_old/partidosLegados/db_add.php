<?php
	...
	if(!empty($_POST)){
		...
		$partido_legadoClaveVerificacion=partido_legadoClaveVerificacion($_POST["partido_legado"][0]['clave'],'',1);
		...
		$inset_partidos_legados= "INSERT INTO partidos_legados ($fields_pdo) VALUES ($values_pdo);";

		$inset_partidos_legados=$conexion->query($inset_partidos_legados);
		$num=$conexion->affected_rows;
		if(!$inset_partidos_legados || $num=0){
			$success=false;
			echo "ERROR inset_partidos_legados"; 
			var_dump($conexion->error);
		}

		$id=$_POST['partido_legado'][0]['id_partido_legado']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($_POST['partido_legado'][0]))."`";
		$values_pdo = "'".implode("','", $_POST['partido_legado'][0])."'";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'partidos_legados',$id,'Insert','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
