<?php
	...
	@session_start();
	$_SESSION['Paguinasub']="partidosLegados/index.php";
		...
		<div style="float: right; width: 100%; text-align: left;"> 
			<?php
				if( $moduloAccionPermisos['insert'] || $moduloAccionPermisos['all']){
					?>
					<input type="button" value="Nuevo Partido" onClick="add();"> 
					<?php
				}
				if( $moduloAccionPermisos['download'] || $moduloAccionPermisos['all']){
					?>
					<input type="button" value="Excel Partidos Legados General" onClick="downloadExcel();"> 
					<?php
				}
			?>
		</div>
