<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="programasApoyos/delete.php";
	if(!empty($_GET)){
	...
	$tablasRelacionadas = tablasRelacionadas('programas_apoyos',$id);
	$registros_titutlo = 'Este registro esta ligado a : <br>';
	foreach ($tablasRelacionadas['tablas'] as $key => $value) {
		if($value['registros']>0 && $value['tabla']!='programas_apoyos_categorias' && $value['tabla']!='programas_apoyos_territorios' && $value['tabla']!='programas_apoyos_dependencias'){
...
?>
	...
	<script language="javascript" type="text/javascript">
		function cerrar(){
			$("#homebody").load('programasApoyos/index.php');
		}
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "programasApoyos/db_delete.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Programa Apoyos</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Programa Apoyos</label><br>
