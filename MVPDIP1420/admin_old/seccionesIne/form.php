<?php
	...
	$zoom="8";
	if($seccion_ineDatos['longitud']=="" || $seccion_ineDatos['latitud']=="" ){
	}else{
	...
	if(
		moduloAccion('sistema_unico_beneficiarios','secciones_ine',$_COOKIE["id_usuario"],$permiso) ||
		moduloAccion('sistema_unico_beneficiarios','secciones_ine',$_COOKIE["id_usuario"],'All') ){
		$permiso_usuario_action = true;
	}else{
		if(moduloAccion('sistema_unico_beneficiarios','secciones_ine',$_COOKIE["id_usuario"],'View')){
...
?>
		...
		function myMap(coordenadas=null,zoomCoordenada=null) {
			...
			*/
			...
			foreach ($secciones_area as $key => $value) {
				...
				echo "];";
				?>
				bermudaTriangle<?= $key ?> = new google.maps.Polygon({
	...
	<div style=" width: 100%; display:inline-block; text-align: left;">
		...
		<div class="sucFormTitulo">
			<label class="labelForm" id="labeltemaname">Datos Sección</label>
		</div>
		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Clave<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" <?= $claveF['input'] ?> type="text" name="clave" autocomplete="off"  id="clave" value="<?= $seccion_ineDatos['clave'] ?>" placeholder="" maxlength="120" onkeyup="clave(this.value)"/><br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Número<font color="#FF0004">*</font></label><br>
			<input class="inputlogin" type="text" name="numero" autocomplete="off"  id="numero" value="<?= $seccion_ineDatos['numero'] ?>" placeholder="" onkeypress="return CheckNumeric()" /><br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Municipio<font color="#FF0004">*</font></label><br>
			<select id="id_municipio">
				<?php echo municipios($seccion_ineDatos['id_municipio'],$id_estado); ?>
			</select>
			<br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Distrito Federal<font color="#FF0004">*</font></label><br>
			<select id="id_distrito_federal">
				<?php echo distritos_federales($seccion_ineDatos['id_distrito_federal']); ?>
			</select>
			<br>
		</div>

		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Distrito Local<font color="#FF0004">*</font></label><br>
			<select id="id_distrito_local">
				<?php echo distritos_locales($seccion_ineDatos['id_distrito_local']); ?>
			</select>
			<br>
		</div>
		...
		<div class="sucForm">
			...
			<input type="text" name="latitud" id="latitud" value="<?= $seccion_ineDatos['latitud'] ?>" placeholder="latitud" onkeypress="return CheckNumeric()" ><br>
		</div>
		<div class="sucForm">
			<label class="labelForm" id="labeltemaname">Longitud<font color="#FF0004">*</font></label><br>
			<input type="text" name="longitud" id="longitud" value="<?= $seccion_ineDatos['longitud'] ?>" placeholder="longitud" onkeypress="return CheckNumeric()" >
		</div>
		<div id="mapa">
		...
		<script type="text/javascript">
			function generar_limite(){
				...
				$.ajax({
					type: "POST",
					url: "seccionesIne/form_limite.php",
			...
			function editarLimite(value){
				...
				var ruta = "seccionesIne/form_ajax.php";
				$.ajax({
					url: ruta,
					type: "POST",
					data: dataString, 
			...
			function eliminarLimite(value){
				...
				var ruta = "seccionesIne/form_limite.php";
				$.ajax({
					url: ruta,
					type: "POST",
					data: dataString, 
		...
		<div id="form_limite">
			...
			<div class="sucForm">
				<label class="labelForm" id="labeltemaname">Latitud<font color="#FF0004">*</font></label><br>
				<input type="text" name="latitud" id="latitud_limite" value="<?= $seccion_ineDatos['latitud'] ?>" placeholder="latitud" autocomplete="off" onkeypress="return CheckNumeric()" ><br>
			</div>
			<div class="sucForm">
				<label class="labelForm" id="labeltemaname">Longitud<font color="#FF0004">*</font></label><br>
				<input type="text" name="longitud" id="longitud_limite" value="<?= $seccion_ineDatos['longitud'] ?>" placeholder="longitud" autocomplete="off" onkeypress="return CheckNumeric()" >
			</div>
