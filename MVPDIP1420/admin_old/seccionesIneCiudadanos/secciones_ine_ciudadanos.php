<?php
	...
	$sql = "SELECT count(*) total FROM secciones_ine_ciudadanos e WHERE 1 = 1  ";
	...
	if($tipo_uso_plataforma=='municipio'){
		...
		//$_SESSION['searchTable']['distrito_federal'] = $distrito_federal;
	}
	...
	foreach ($_SESSION['reporte_Sistema']['database'] as $key => $value) {
		...
		$url = '../../expediente.php?cot='.$value["clave_elector"];
		$link = '<a href="#" onclick="window.open(\'' . $url . '\', \'miVentana\', \'width=600,height=400\');">'.$value["clave_elector"].'</a>';
		$nestedData[] = $link;
	...
	if($post_search){
		...
		if($validar_codigo_plataforma == false){
			$sqlContador .= " AND e.codigo_plataforma = '{$codigo_plataforma}' ";
		}
