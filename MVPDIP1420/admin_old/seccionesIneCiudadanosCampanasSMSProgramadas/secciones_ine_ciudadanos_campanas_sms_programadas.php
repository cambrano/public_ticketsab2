<?php
	...
	$sql = "SELECT count(*) total FROM secciones_ine_ciudadanos_campanas_sms_programadas WHERE 1 = 1   "; 
	...
	$_SESSION['reporte_Sistema']['sql'] = $sql_excel;
	...
	while($row=$result->fetch_assoc()){
		...
		$nestedData[] = "<div style='text-transform: none;'>".$row["celular"]."<div>"; 
		...
		if($row["status"]=='0'){
			...
			$status='<img class="bntImageSize" src="img/circulo_check_green.png">';
		}elseif($row["status"]=='2'){
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM secciones_ine_ciudadanos_campanas_sms_programadas siccmp WHERE 1 = 1   "; 
