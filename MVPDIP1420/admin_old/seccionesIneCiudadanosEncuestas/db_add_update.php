		...
		if($_POST["seccion_ine_ciudadano_encuesta"][0]['id']==""){
			...
			if($api_whatsappDatos['status'] == 1){
				...
				$sqlCamWhatsapp = '
					...
					LEFT JOIN campanas_whatsapp_encuestas cme
					ON cm.id = cme.id_campana_whatsapp
					WHERE cm.status = 1 AND cm.tipo=3 AND  scm.status =1  AND cme.id_encuesta = "'.$id_encuesta.'";';

				$resultCamWhatsapp = $conexion->query($sqlCamWhatsapp);  
				while($rowCampWhatsapp=$resultCamWhatsapp->fetch_assoc()){
					$id_campana_whatsapp = $rowCampWhatsapp['id'];
					$tipo = $rowCampWhatsapp['tipo'];
					...
					$campanas_whatsapp_tipos_categorias_ciudadanosIdDatos=campanas_whatsapp_tipos_categorias_ciudadanosIdDatos('',$id_campana_whatsapp);

					$sql="
						INSERT INTO secciones_ine_ciudadanos_campanas_whatsapp_programadas
