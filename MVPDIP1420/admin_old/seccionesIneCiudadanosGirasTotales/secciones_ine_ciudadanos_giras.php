<?php
	...
	$sql = "SELECT count(*) total FROM secciones_ine_ciudadanos_giras sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sic.id = sicpa.id_seccion_ine_ciudadano WHERE 1 = 1   ";
	...
	$sql = "SELECT
				...
				sic.curp,
				sic.nombre_completo,
				IF((SELECT count(*) FROM secciones_ine_ciudadanos_giras sicpa WHERE sicpa.id_seccion_ine_ciudadano = sic.id AND sicpa.id_seccion_ine_gira = '4')>1,'repetido','sin repetir') repetido,
				sic.sexo,
				...
				sicpa.observaciones
			FROM secciones_ine_ciudadanos_giras sicpa
	...
	$repetidos=$_SESSION['searchTable']['repetidos'];
	if( $repetidos!="" ){   //name
		if($repetidos == 1){
			$post_search=true;
			$sql.=" AND (SELECT count(*) FROM secciones_ine_ciudadanos_giras sicpa WHERE sicpa.id_seccion_ine_ciudadano = sic.id AND sicpa.id_seccion_ine_gira = '{$id_seccion_ine_gira}'  ) > 1 ";
			$sqlContador.=" AND (SELECT count(*) FROM secciones_ine_ciudadanos_giras sicpa WHERE sicpa.id_seccion_ine_ciudadano = sic.id AND sicpa.id_seccion_ine_gira = '{$id_seccion_ine_gira}'  ) > 1 ";
		}else{
			$post_search=true;
			$sql.=" AND (SELECT count(*) FROM secciones_ine_ciudadanos_giras sicpa WHERE sicpa.id_seccion_ine_ciudadano = sic.id AND sicpa.id_seccion_ine_gira = '{$id_seccion_ine_gira}'  ) = 1 ";
			$sqlContador.=" AND (SELECT count(*) FROM secciones_ine_ciudadanos_giras sicpa WHERE sicpa.id_seccion_ine_ciudadano = sic.id AND sicpa.id_seccion_ine_gira = '{$id_seccion_ine_gira}' = 1  ) > 1 ";
		}
	} 
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM secciones_ine_ciudadanos_giras sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sic.id = sicpa.id_seccion_ine_ciudadano WHERE 1 = 1 "; 
		if($tipo_uso_plataforma=='municipio'){
