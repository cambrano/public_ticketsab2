<?php
	...
	$sql = "SELECT 
				...
				e.nombre 
			FROM campanas_mailing e WHERE 1 = 1 "; 
	// getting records as per search parameters
	 

	$id_campana_mailing=$_SESSION['searchTable']['id_campana_mailing'];
	if( $id_campana_mailing!="" ){   //name
		$post_search=true;
		$sql.=" AND e.id LIKE '%{$id_campana_mailing}%' ";
		$sqlContador .= " AND e.id LIKE '%{$id_campana_mailing}%' ";
	}
