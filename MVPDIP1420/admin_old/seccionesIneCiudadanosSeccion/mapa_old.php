<?php
	...
	*/
	...
	if(!empty($_POST)){
		...
		$limit = "LIMIT {$mostrardesde},100";

		$id_seccion_ine = $_POST['searchTable'][0]['id_seccion_ine'] = $_SESSION['id_seccion_ine']; 
		$id_secciones_ine[]=$id_seccion_ine;
		/*
		...
		*/
		...
		$origenM['id_seccion_ine']=$id_seccion_ine;
		$manzanas_ineDatosMapa = manzanas_ineDatosMapa($origenM);
		...
		$limit = ' LIMIT 0,100 ';
		/*$origen['id_municipio']=$id_municipio;*/
		$_POST['searchTable'][0]['id_seccion_ine'] = $_SESSION['id_seccion_ine']; 
		$origen['id_municipio']=$id_municipio;
		/*
		if($tipo_uso_plataforma=='municipio'){
			$_POST['searchTable'][0]['id_municipio'] = $id_municipio;
...
?> 
	<style type="text/css">
		...
		@media screen and (max-width: 1281px) {
			...
			.info_seccion_manzana,.info_seccion_ganador{
				text-align:center;
				width:100%;
			}
			.datos_votos{
			...
			foreach ($secciones_ine_parametrosDatosMapa as $key => $value) {
				...
				*/
				...
				if($tipo_uso_plataforma=='distrito_local'){
					if ($secciones_ineDatosMapa[$key]['distrito_local'] == $distrito_local ){
						if ($secciones_ineDatosMapa[$key]['id'] == $id_seccion_ine ){
							$strokeColor='#00008B';
							$fillColor='#FFFF00';
						}else{
				...
				if($tipo_uso_plataforma=='distrito_federal'){
					if ($secciones_ineDatosMapa[$key]['distrito_federal'] == $distrito_federal ){
						if ($secciones_ineDatosMapa[$key]['id'] == $id_seccion_ine ){
							$strokeColor='#00008B';
							$fillColor='#FFFF00';
						}else{
				...
				$div = '<div class="divMapaSecciones">
							<div class="info_content">
								...
								<div class="info_seccion_manzana">
									<h5>'.$manzanas_ineDatosMapa[$key]['numero'].'</h5>
								</div>
							</div>
							<div class="datos_seccion">
								<p>
