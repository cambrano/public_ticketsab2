<?php
	...
	if(!empty($_POST)){
		...
		$nombre_csv = 'SICSln_'.$_COOKIE['id_usuario']."_I".date(Y_m_d).'-'.date(H_i_s)."I_".$stringAleatorio;
		$nombreArchivo = '../ftpFiles/documentosExport/'.$nombre_csv.'.csv';
		// Abrir el archivo en modo escritura
		$archivoCSV = fopen($nombreArchivo, 'w');
		// Escribir los encabezados en el archivo CSV
		fputcsv($archivoCSV, array_keys($_SESSION['searchTable']));
		// Escribir los valores en otra línea del archivo CSV
		fputcsv($archivoCSV, array_values($_SESSION['searchTable']));
		// Cerrar el archivo
		fclose($archivoCSV);
