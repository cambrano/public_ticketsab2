<?php
	...
	$sql = "SELECT 
				...
				e.nombre 
			FROM campanas_whatsapp e WHERE 1 = 1 "; 
	// getting records as per search parameters
	 

	$id_campana_whatsapp=$_SESSION['searchTable']['id_campana_whatsapp'];
	if( $id_campana_whatsapp!="" ){   //name
		$post_search=true;
		$sql.=" AND e.id LIKE '%{$id_campana_whatsapp}%' ";
		$sqlContador .= " AND e.id LIKE '%{$id_campana_whatsapp}%' ";
	}
