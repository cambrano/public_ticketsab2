<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']='seccionesIneCiudadanosWhatsapp/update.php';
	if(!empty($_GET)){
	...
	echo $redirectSecurity=redirectSecurity($id,'campanas_whatsapp','seccionesIneCiudadanosWhatsapp','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	if($id_seccion_ine_ciudadano==''){
		echo $redirectSecurity=redirectSecurity('','campanas_whatsapp','seccionesIneCiudadanosWhatsapp','index');
		if($redirectSecurity!=""){
			die;
		}
	}
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanosWhatsapp/db_reenviar_masivos_configurada.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Reenviar Whatsapp <br></font>
					<h3> <?= $seccion_ine_ciudadanoDatos['nombre_completo'] ?> </h3>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Camapaña Whatsapp</label><br>
