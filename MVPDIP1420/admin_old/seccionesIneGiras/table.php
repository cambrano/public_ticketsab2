<?php
	...
	@session_start();
	if(!empty($_POST)){
		/*include '../functions/secciones_ine_giras.php';*/
...
?>
	<script type="text/javascript" language="javascript" >
		$(document).ready(function() {
			...
			var dataTable = $('#secciones_ine_giras-tabla').DataTable( {
				"destroy": true,
				...
				"ajax":{
					url :"seccionesIneGiras/secciones_ine_giras.php", // json datasource
					type: "post",  // method  , by default get
					error: function(){  // error handling
						$(".secciones_ine_giras-tabla-error").html("");
						$("#secciones_ine_giras-tabla").append('<tbody class="secciones_ine_giras-tabla-error"><tr><th colspan="8">Registros no encontrados</th></tr></tbody>');
						$("#secciones_ine_giras-tabla_processing").css("display","none");
			...
			$('#secciones_ine_giras-tabla').on( 'order.dt', function () {
				var table = $('#secciones_ine_giras-tabla').dataTable();
				var api = table.api();
			...
			$('#secciones_ine_giras-tabla').css( 'display', 'table' );
			$('#secciones_ine_giras-tabla').resize();
			$('#secciones_ine_giras-tabla').DataTable().columns.adjust().responsive.recalc();
			$("#secciones_ine_giras-tabla_filter").css("display","none");  // hiding global search box
			$('#selectCheckbox').on("click", function(event){ // triggering delete one by one
				if( $('.checkselected:checked').length > 0 ){  // at-least one checkbox checked
					...
					var link="seccionesIneGiras/index.php?cot="+ids_string;   
					var link2="seccionesIneGiras/index.php";
			...
			$('#secciones_ine_giras-tabla').on( 'page.dt', function () {
		...
		function edit(valor){
			link="seccionesIneGiras/update.php?id="+valor; 
			var link2="seccionesIneGiras/update.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function add(){	
			////ajax
			link="seccionesIneGiras/create.php";
			var link2="seccionesIneGiras/create.php";
			dataString = 'urlink='+link2;  
			$.ajax({
				type: "POST",
				url: "functions/backarray.php",
		...
		function borrar(valor){
			link="seccionesIneGiras/delete.php?id="+valor; 
			var link2="seccionesIneGiras/delete.php";
			dataString = 'urlink='+link2; 
			/*
			$.ajax({
				type: "POST",
		...
		function printPdf(valor){
			var link="seccionesIneGiras/print/index.php?cot=<?=$_SESSION['pageService']?>"; 
		...
		function downloadExcel(){
			...
			?>
			var link="seccionesIneGiras/excel/index.php?cot=<?=$_COOKIE['pageService']?>"; 
			//window.open(link);
	...
	<table id="secciones_ine_giras-tabla" class="table table-striped table-bordered  cell-border compact stripe" style="width:100%">
		<thead>
			<tr>
			<?php
