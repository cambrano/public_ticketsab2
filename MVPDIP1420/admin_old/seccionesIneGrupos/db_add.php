<?php
	...
	if(!empty($_POST)){
		...
		$seccion_ine_grupoClaveVerificacion=seccion_ine_grupoClaveVerificacion($_POST["seccion_ine_grupo"][0]['clave'],'',1);
		...
		$inset_secciones_ine_grupos= "INSERT INTO secciones_ine_grupos ($fields_pdo) VALUES ($values_pdo);";
		$conexion->autocommit(FALSE);
		$inset_secciones_ine_grupos=$conexion->query($inset_secciones_ine_grupos);
		$num=$conexion->affected_rows;
		if(!$inset_secciones_ine_grupos || $num=0){
			$success=false;
			echo "ERROR inset_secciones_ine_grupos"; 
			var_dump($conexion->error);
		}
		$id=$_POST["seccion_ine_grupo"][0]['id_seccion_ine_grupo']=$conexion->insert_id;
		$fields_pdo = "`".implode('`,`', array_keys($_POST["seccion_ine_grupo"][0]))."`";
		$values_pdo = "'".implode("','", $_POST["seccion_ine_grupo"][0])."'";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'secciones_ine_grupos',$id,'Insert','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
