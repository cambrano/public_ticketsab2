<?php
	...
	$_SESSION['reporte_Sistema']['columnas_nombres'] = array(
		...
		2 => array('row' => 'fecha_hora' ,'nombre' => 'Fecha Hora' ,'tipo' => 'datetime','mostrar' => 1 ),
	...
	if(!empty($_POST)){
		...
		include '../functions/secciones_ine_grupos.php';
		$punto=false;
		...
		$secciones_ine_gruposDatosArray=secciones_ine_gruposDatosArray($_POST['searchTable'][0],$orderby,$limit);
		...
		if($punto==false){
			$zoom = 14;
			if($secciones_ine_gruposDatosArray[0]['latitud'] != '' ){
				$latitud = $secciones_ine_gruposDatosArray[0]['latitud'];
				$longitud = $secciones_ine_gruposDatosArray[0]['longitud'];
			}
		}else{
			$zoom = 14;
		}
	...
	foreach ($secciones_ine_gruposDatosArray as $key => $value) {
		if($key==0){
					...
					$div = '<div class="divMapa">
									<div class="info_content">
										...
										<div class="info_seccion_ganador">
											<h5>'.strtoupper($value['tipo_grupo']).'</h5>
										</div>
									...
									<div class="datos_top" style="width:100%;">
										...
										Tipo Interes: <b>'.$value['tipo_interes'].'</b><br>
										Tipo Relación: <b>'.$value['tipo_relacion'].'</b><br>
										Partido: <b>'.$value['partido'].'</b><br>
										Miembros: <b>'.$value['miembros'].'</b><br>
									...
									<div class="datos_right">
										<p>
											<b>'.fechaNormalSimpleWDDMMAA_ES($value['fecha']).'</b><br>
											Fecha: <b>'.$value['fecha'].'</b><br>
											Hora: <b>'.$value['hora'].'</b><br>
										</p>
									</div>
