<?php
	...
	$sql = "SELECT count(*) total FROM secciones_ine_grupos sia LEFT JOIN secciones_ine si ON si.id = sia.id_seccion_ine WHERE 1 = 1   "; 
	...
	$id_tipo_seccion_ine_grupo=$_SESSION['searchTable']['id_tipo_seccion_ine_grupo'];
	if( $id_tipo_seccion_ine_grupo!="" ){   //name
		$post_search=true;
		$sql.=" AND sia.id_tipo_seccion_ine_grupo IN ({$id_tipo_seccion_ine_grupo}) ";
		$sqlContador.=" AND sia.id_tipo_seccion_ine_grupo IN ({$id_tipo_seccion_ine_grupo}) ";
	}

	$id_tipo_interes=$_SESSION['searchTable']['id_tipo_interes'];
	if( $id_tipo_interes!="" ){   //name
		$post_search=true;
		$sql.=" AND sia.id_tipo_interes IN ({$id_tipo_interes}) ";
		$sqlContador.=" AND sia.id_tipo_interes IN ({$id_tipo_interes}) ";
	}

	$id_tipo_relacion=$_SESSION['searchTable']['id_tipo_relacion'];
	if( $id_tipo_relacion!="" ){   //name
		$post_search=true;
		$sql.=" AND sia.id_tipo_relacion IN ({$id_tipo_relacion}) ";
		$sqlContador.=" AND sia.id_tipo_relacion IN ({$id_tipo_relacion}) ";
	}
	...
	if( $id_partido_legado!="" ){   //name
		...
		$sqlContador.=" AND sia.id_partido_legado IN ({$id_partido_legado}) ";
	}
	...
	while($row=$result->fetch_assoc()){
		...
		if($modulo_secciones_ine_ciudadanos_grupos){
			$ciudadanos_grupos='<button class="btn btn-primary bt_responsive"  onClick="secciones_ine_ciudadanos_grupos_totales('.$row["id"].');" >Miembros</button>';
		}
		$nestedData[] =  "<div class='opciones_botones_1'>{$edit}{$ciudadanos_grupos}{$google_maps}{$delete}{$select}</div>";
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM secciones_ine_grupos sia LEFT JOIN secciones_ine si ON sia.id_seccion_ine = si.id WHERE 1 = 1   "; 
