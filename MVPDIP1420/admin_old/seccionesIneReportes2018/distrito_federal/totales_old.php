<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = 1 ) votos_nulos_2018,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = 1 ) votos_can_nreg_2018,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = 1 ) casillas_2018,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = 1 ) total_lista_nominal_2018,

				( SELECT id_partido_2018 votos FROM casillas_votos_partidos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = 1 GROUP BY id_partido_2018 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2018,

				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_federal={$id_distrito_federal} ) ciudadanos_totales,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_federal={$id_distrito_federal} AND check_in=1 AND check_out=1 ) ciudadanos_totales_doble_check,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_federal={$id_distrito_federal} AND check_in=1 ) ciudadanos_totales_check_in,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_federal={$id_distrito_federal} AND check_out=1 ) ciudadanos_totales_check_out,
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic LEFT JOIN secciones_ine_ciudadanos_check_2018 sicc2018 ON sic.id = sicc2018.id_seccion_ine_ciudadano WHERE IF(sicc2018.check_in IS NULL, 0,sicc2018.check_in ) = 0 AND IF(sicc2018.check_out IS NULL, 0,sicc2018.check_out ) = 0 AND sic.id_distrito_federal={$id_distrito_federal} ) ciudadanos_totales_sin_check
			FROM
				(SELECT SUM(votos) votos_validos_2018 FROM  casillas_votos_partidos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo =  1 )t
	...
	$distritos_federales_2018 = $row['distritos_federales_2018']-1;
	$distritos_federales_2018 = $row['distritos_federales_2018'];
	$total_lista_nominal_2018 = $row['total_lista_nominal_2018'];
	//aqui cambrano
	$ciudadanos_totales = $row['ciudadanos_totales'];
	$ciudadanos_totales_porcentaje = ($ciudadanos_totales/$total_lista_nominal_2018)*100;
	...
	$total_lista_nominal_ciudadanos_2018 = $total_lista_nominal_2018-$ciudadanos_totales;
	...
	$ciudadanos_lista_nominal_porcentaje = ($ciudadanos_totales / $total_lista_nominal_2018) * 100;
	//$ciudadanos_lista_nominal_porcentaje = truncar($ciudadanos_lista_nominal_porcentaje,2);

	$total_lista_abstencion_2018 = $total_lista_nominal_2018 - $votos_totales_2018;
	if($total_lista_nominal_2018==0){
		...
		$abstencion_ciudadana = ($total_lista_abstencion_2018 / $total_lista_nominal_2018 )*100;
	}

	//partido sistema
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2018.votos) FROM casillas_votos_partidos_2018 cpv2018 WHERE cpv2018.id_partido_2018 = p2018.id AND cpv2018.id_distrito_federal={$id_distrito_federal} AND cpv2018.tipo = 1 ) votos_totales_2018
			FROM partidos_2018 p2018
			WHERE p2018.principal=1 AND p2018.codigo_plataforma = '{$codigo_plataforma}' AND p2018.tipo = 1";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	...
	$sql="
		...
		WHERE cvp2018.id_distrito_federal={$id_distrito_federal} AND cvp2018.tipo = 1
		...
		<div style="background-color: white;padding: 5px;display: table;">
			...
			<div class="div50Reporte" style="padding: 2px; margin: 0px;">
				<table style="table-layout: fixed; width: 100%" cellspacing="0" cellpadding="0">
					<thead>
						<tr>
							<td style="text-align: center;padding: 10px;background-color: #191919;color: white" colspan="2">Totales Votaciones 2018</td>
						</tr>
					</thead>
					<tbody>
						<tr>
