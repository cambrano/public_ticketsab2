<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = 1 ) votos_nulos_2018,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = 1 ) votos_can_nreg_2018,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = 1 ) casillas_2018,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = 1 ) total_lista_nominal_2018,

				( SELECT id_partido_2018 votos FROM casillas_votos_partidos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo = 1 GROUP BY id_partido_2018 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2018,

				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_local={$id_distrito_local} ) ciudadanos_totales,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_local={$id_distrito_local} AND check_in=1 AND check_out=1 ) ciudadanos_totales_doble_check,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_local={$id_distrito_local} AND check_in=1 ) ciudadanos_totales_check_in,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2018 WHERE id_distrito_local={$id_distrito_local} AND check_out=1 ) ciudadanos_totales_check_out,
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic LEFT JOIN secciones_ine_ciudadanos_check_2018 sicc2018 ON sic.id = sicc2018.id_seccion_ine_ciudadano WHERE IF(sicc2018.check_in IS NULL, 0,sicc2018.check_in ) = 0 AND IF(sicc2018.check_out IS NULL, 0,sicc2018.check_out ) = 0 AND sic.id_distrito_local={$id_distrito_local} ) ciudadanos_totales_sin_check
			FROM
				(SELECT SUM(votos) votos_validos_2018 FROM  casillas_votos_partidos_2018 WHERE id_distrito_local={$id_distrito_local} AND tipo =  1 )t
	...
	$distritos_locales_2018 = $row['distritos_locales_2018']-1;
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2018.votos) FROM casillas_votos_partidos_2018 cpv2018 WHERE cpv2018.id_partido_2018 = p2018.id AND cpv2018.id_distrito_local={$id_distrito_local} AND cpv2018.tipo = 1 ) votos_totales_2018
	...
	$sql="
		...
		WHERE cvp2018.id_distrito_local={$id_distrito_local} AND cvp2018.tipo = 1
