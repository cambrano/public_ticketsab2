...
?>
	<script type="text/javascript">
		...
		function searchTable(value){
			...
			if(tipo_seccion==""){
				$('#secciones_reportes-tabla').DataTable().column(14).search(tipo_seccion).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+tipo_seccion+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(14).search(tipo_seccion_array_table.join('|'), true, false).draw();
			}
			...
			if(semaforo==""){
				$('#secciones_reportes-tabla').DataTable().column(9).search(semaforo).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+semaforo+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(9).search(semaforo_array_table.join('|'), true, false).draw();
			}
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneReportes2018MatrizRentabilidad/table.php",
