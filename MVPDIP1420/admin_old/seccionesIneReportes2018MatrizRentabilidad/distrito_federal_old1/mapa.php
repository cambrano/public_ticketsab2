<?php
	...
	if(!empty($_POST)){
		...
		#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['tipo_eleccion'] = '2';// 0 - Ayuntamiento | 1 - Distrito Local | 2 - Distrito Federal
		...
		*/
		...
		if($id_seccion_ine!=''){
			...
			$sqlPartidosRM2018 = " AND cprm.id_seccion_ine IN ({$id_seccion_ine}) ";
		}

		if($id_municipio!=''){
			$sqlMunicipio = " AND cvp.id_municipio = {$id_municipio} ";
			$sqlMunicipioRM2018 = " AND cprm.id_municipio = {$id_municipio} ";
		}
		$sql="
			SELECT
				p.id,
		...
		$sql="SELECT 
			...
			WHERE cprm.id_distrito_federal = '{$id_distrito_federal}'  {$sqlPartidosRM2018} {$sqlMunicipioRM2018}
								...
								<div class="info_content">
									...
									<div class="info_seccion_ganador_button">
										<div style="background-color:'.$color.';padding:5px;margin-top:2px;text-align:center;color:black">
											<b style="color:black">'.strtoupper($value['orden_votos_individual']['semaforo']['diferencia']).'</b>
										</div>
									</div>
								</div>
