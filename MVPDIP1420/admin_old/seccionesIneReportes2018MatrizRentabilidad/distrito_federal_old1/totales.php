<?php
	...
	#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['tipo_eleccion'] = '2';// 0 - Ayuntamiento | 1 - Distrito Federal | 2 - Distrito Federal
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo = '{$tipo}' ) total_lista_nominal,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_federal={$id_distrito_federal} ) ciudadanos_totales,

				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} {$sql_programas_apoyos} ) apoyos_programas,
				(SELECT COUNT(*) FROM secciones_ine_actividades sia LEFT JOIN secciones_ine si ON sia.id_seccion_ine = si.id WHERE si.id_distrito_federal={$id_distrito_federal} {$sql_actividades} ) acciones_obras,
				(SELECT COUNT(*) FROM secciones_ine_grupos sig LEFT JOIN secciones_ine si ON sig.id_seccion_ine = si.id WHERE si.id_distrito_federal={$id_distrito_federal} {$sql_grupos} ) grupos_interes,
				(SELECT COUNT(*) FROM militantes_partidos mp LEFT JOIN secciones_ine_ciudadanos sic ON mp.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} AND mp.id_partido_legado = '{$id_partido_legado}') militantes,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_categorias sicc WHERE sicc.id_distrito_federal={$id_distrito_federal} AND sicc.id_tipo_categoria_ciudadano = '{$id_tipo_categoria_ciudadano}') funcionarios,

				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_federal = {$id_distrito_federal} AND sig.tipo = 'junta' {$sql_giras}  ) juntas,
				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_federal = {$id_distrito_federal} AND sig.tipo = 'visita' {$sql_giras}  ) visitas,
				(SELECT COUNT(*) FROM secciones_ine_giras sig LEFT JOIN secciones_ine s ON sig.id_seccion_ine = s.id WHERE s.id_distrito_federal = {$id_distrito_federal} AND sig.tipo = 'caminata' {$sql_giras}  ) caminatas

			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2018 WHERE id_distrito_federal={$id_distrito_federal} AND tipo =  '{$tipo}' )t
	...
	$datos_generales['territorio_nombre'] = $distrito_federalDatos['numero'];
	$datos_generales['ano'] = $ano;

	$datos_generales = $row + $datos_generales;
	$_SESSION['reporte_Sistema']['pdf']['datos_generales'] = $datos_generales;
	...
	$sql = "SELECT 
			...
			prm.id,
			(SELECT SUM(cvrm.votos_nulos) FROM casillas_votos_2022_revocacion_mandato cvrm WHERE cvrm.id_distrito_federal='{$id_distrito_federal}' ) votos_nulos,
			(SELECT SUM(cvrm.votos) FROM casillas_preguntas_2022_revocacion_mandato cvrm WHERE cvrm.id_pregunta_2022_revocacion_mandato = prm.id AND cvrm.id_distrito_federal='{$id_distrito_federal}' ) votos
	...
	$_SESSION['reporte_Sistema']['pdf']['partidos'] = $datos_partidos;
?>
<style type="text/css">
	.totales {
		display: table;
