<?php
	...
	if(!empty($_POST)){
		...
		$id_partido_2018 = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['id_partido_2018_distrito_local'];// Partidos 2018 PRI
		...
		#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2018Datos['tipo_eleccion'] = '1';// 0 - Ayuntamiento | 1 - Distrito Local | 2 - Distrito Federal
		...
		*/
		...
		$sql="
			...
			WHERE cvp.id_distrito_local='{$id_distrito_local}' AND cvp.tipo = '{$tipo}' {$sqlPartidos} {$sqlMunicipio}
		...
		$sql="SELECT 
			...
			WHERE cprm.id_distrito_local = '{$id_distrito_local}'  {$sqlPartidosRM2019} {$sqlMunicipioRM2019}
		...
		while($row=$result->fetch_assoc()){
			...
			$datos_distritos_locales[$id_distrito_local]['latitud'] = $row['latitud'];
			$datos_distritos_locales[$id_distrito_local]['longitud'] = $row['longitud'];
		...
		*/
		...
		$sql="
			...
			LEFT JOIN partidos_2018 p
			ON p.id = cvp.id_partido_2018
			WHERE cvp.id_distrito_local='{$id_distrito_local}' AND cvp.tipo = '{$tipo}'
			GROUP BY cvp.id_seccion_ine,cvp.id_partido_2018
		";
		...
		$sql="SELECT 
			...
			LEFT JOIN preguntas_2022_revocacion_mandato prm
			ON cprm.id_pregunta_2022_revocacion_mandato = prm.id
			WHERE cprm.id_distrito_local = '{$id_distrito_local}'
			GROUP BY cprm.id_seccion_ine,id_pregunta_2022_revocacion_mandato
			ORDER BY cprm.id_seccion_ine,SUM(cprm.votos) DESC
			";
		$result = $conexion->query($sql);
