<?php
	...
	$tipo = $configuracion_matriz['tipo_eleccion'] = '1';// 0 - Ayuntamiento | 1 - Municipio | 2 - Distrito Federal
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2018 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2018 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_municipio={$id_municipio} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2018 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2018 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) total_lista_nominal,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_municipio={$id_municipio} ) ciudadanos_totales,

				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_municipio={$id_municipio}) apoyos_programas,
				(SELECT COUNT(*) FROM secciones_ine_actividades sia LEFT JOIN secciones_ine si ON sia.id_seccion_ine = si.id WHERE si.id_municipio={$id_municipio} ) acciones_obras,
				(SELECT COUNT(*) FROM secciones_ine_grupos sig LEFT JOIN secciones_ine si ON sig.id_seccion_ine = si.id WHERE si.id_municipio={$id_municipio} ) grupos_interes,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2018 WHERE id_municipio={$id_municipio} AND tipo =  '{$tipo}' )t
...
<div class="totales">
	<div
		...
		foreach ($datos_partidos['orden_votos_individual'] as $key => $value) {
			if ($key !='sistema' && $key !='partidos' && $key !='graficas'){
