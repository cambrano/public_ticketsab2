<?php
	...
	if(!empty($_POST)){
		...
		#$tipo = $configuracion_matriz_rentabilidad_secciones_ine_2021Datos['tipo_eleccion'] = '2';// 0 - Ayuntamiento | 1 - Distrito Local | 2 - Distrito Federal
		...
		*/
		...
		if($id_seccion_ine!=''){
			...
			$sqlPartidosRM2021 = " AND cprm.id_seccion_ine IN ({$id_seccion_ine}) ";
		}

		if($id_municipio!=''){
			$sqlMunicipio = " AND cvp.id_municipio = {$id_municipio} ";
			$sqlMunicipioRM2021 = " AND cprm.id_municipio = {$id_municipio} ";
		}
		$sql="
			SELECT
				p.id,
		...
		$sql="SELECT 
			...
			WHERE cprm.id_distrito_federal = '{$id_distrito_federal}'  {$sqlPartidosRM2021} {$sqlMunicipioRM2021}
		...
		$sql="
			SELECT
				...
				(SELECT GROUP_CONCAT(DISTINCT ls.localidad SEPARATOR ' - ' ) FROM localidades_secciones_ine ls WHERE ls.id_seccion_ine = si.id AND ls.id_estado='{$id_estado}' ) seccion_localidades,
	...
	if(!empty($id_secciones_ine_validos)){
		...
		if(!empty($_POST['mapa'][0]['secciones_ine_giras'])){
			echo "<pre>";
			print_r($id_secciones_ine_validos);
			echo "</pre>";
			unset($search);
