...
?>
	<script type="text/javascript">
		...
		function searchTable(value){
			...
			var id_secciones_ine = document.getElementById("id_secciones_ine").value;
			const id_secciones_ine_array = id_secciones_ine.split(",");
			var id_secciones_ine_array_table = [];
			id_secciones_ine_array.forEach( function(valor, indice, array) {
				id_secciones_ine_array_table.push("(^" + valor + "$)");
			});

			if(id_secciones_ine!='' && id_seccion_ine=='' ){
			...
			if(id_secciones_ine==""){
				$('#secciones_reportes-tabla').DataTable().column(1).search(id_secciones_ine).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+id_seccion_ine+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(1).search(id_secciones_ine_array_table.join('|'), true, false).draw();
			}
			...
			if(tipo_seccion==""){
				$('#secciones_reportes-tabla').DataTable().column(16).search(tipo_seccion).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+tipo_seccion+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(16).search(tipo_seccion_array_table.join('|'), true, false).draw();
			...
			if(semaforo==""){
				$('#secciones_reportes-tabla').DataTable().column(11).search(semaforo).draw();
			}else{
				//$('#secciones_reportes-tabla').DataTable().column(0).search("(^"+semaforo+"$)",true,false).draw();
				$('#secciones_reportes-tabla').DataTable().column(11).search(semaforo_array_table.join('|'), true, false).draw();
			}
	...
	<div class="sucForm" >
		...
		<select class="selectpicker" data-live-search="true" data-size="5" data-actions-box="true" title="Seleccione" id="id_secciones_ine" onchange="localidadesSeccionesIne();">
			<?php
