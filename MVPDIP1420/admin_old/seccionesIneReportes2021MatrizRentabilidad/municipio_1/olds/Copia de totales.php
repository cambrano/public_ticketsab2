<?php
	...
	$sql = "SELECT 
				t.votos_validos,
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_nulos,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) votos_can_nreg,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_municipio={$id_municipio} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) casillas,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo = '{$tipo}' ) total_lista_nominal,
			...
			FROM
				(SELECT SUM(votos) votos_validos FROM  casillas_votos_partidos_2021 WHERE id_municipio={$id_municipio} AND tipo =  '{$tipo}' )t
