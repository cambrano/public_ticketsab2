<?php
	...
	$tipo_eleccion = $configuracion_matriz['tipo_eleccion'] = '2';// 0 - Ayuntamiento | 1 - Distrito Local | 2 - Distrito Federal
	...
	if(!empty($_POST)){
		...
		*/
		...
		$sql="
			...
			WHERE  m.id = '{$id_distrito_federal}'
		";
		...
		$sql="SELECT 
			...
			WHERE cvp2021.tipo = '{$tipo_eleccion}'  AND cvp2021.id_distrito_federal = '{$id_distrito_federal}' {$sql_secciones_ine}
		...
		*/
		...
		$sql="SELECT 
			...
			FROM casillas_votos_partidos_2021 cvp2021 
			LEFT JOIN partidos_2021 p2021 ON cvp2021.id_partido_2021 = p2021.id
			WHERE cvp2021.tipo = '{$tipo_eleccion}'  AND cvp2021.id_distrito_federal = '{$id_distrito_federal}' 
			GROUP BY cvp2021.id_seccion_ine,id_partido_2021 
			ORDER BY cvp2021.id_seccion_ine,SUM(cvp2021.votos) DESC
			";
		$result = $conexion->query($sql);
