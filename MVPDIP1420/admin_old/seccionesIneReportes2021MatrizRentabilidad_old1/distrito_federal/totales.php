<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 ) votos_nulos_2021,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 ) votos_can_nreg_2021,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_municipio={$id_municipio} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 ) casillas_2021,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 ) total_lista_nominal_2021,

				( SELECT id_partido_2021 votos FROM casillas_votos_partidos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 GROUP BY id_partido_2021 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2021
			FROM
				(SELECT SUM(votos) votos_validos_2021 FROM  casillas_votos_partidos_2021 WHERE id_municipio={$id_municipio} AND tipo=0 )t
	...
	if($total_lista_nominal_2021==""){
		$total_lista_nominal_2021=0;
	}
	if($extranjeros_mode){
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2021.votos) FROM casillas_votos_partidos_2021 cpv2021 WHERE cpv2021.id_partido_2021 = p2021.id AND cpv2021.id_municipio={$id_municipio} AND cpv2021.tipo=0 ) votos_totales_2021
			FROM partidos_2021 p2021
			WHERE p2021.principal=1 AND p2021.codigo_plataforma = '{$codigo_plataforma}' AND p2021.tipo=0 ";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	...
	if($votos_validos_2021==0){
		$partido_sistema_votos_porcentaje_2021 = 0;
	}else{
	...
	$sql="
		...
		WHERE cvp2021.id_municipio={$id_municipio} AND p2021.tipo=0 AND cvp2021.tipo=0
