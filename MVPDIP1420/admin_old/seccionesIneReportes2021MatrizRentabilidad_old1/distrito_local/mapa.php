	...
	if(!empty($_POST)){
		...
		*/
		...
		if($id_seccion_ine!=''){
			...
			$sqlPartidosRM2022 = " AND cprm.id_seccion_ine IN ({$id_seccion_ine}) ";
		}

		if($id_municipio!=''){
			$sqlMunicipio = " AND cvp.id_municipio = {$id_municipio} ";
			$sqlMunicipioRM2022 = " AND cprm.id_municipio = {$id_municipio} ";
		}
		$sql="
			SELECT
				p.id,
		...
		$sql="SELECT 
			...
			WHERE cprm.id_distrito_local = '{$id_distrito_local}'  {$sqlPartidosRM2022} {$sqlMunicipioRM2022}
		...
		$sql="
			SELECT
				...
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic WHERE sic.id_distrito_local={$id_distrito_local} AND sic.id_seccion_ine = si.id ) ciudadanos_totales,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_local={$id_distrito_local} AND sic.id_seccion_ine = si.id) apoyos_programas,
				...
				(SELECT COUNT(*) FROM militantes_partidos mp LEFT JOIN secciones_ine_ciudadanos sic ON mp.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_local={$id_distrito_local} AND mp.id_partido_legado = '{$id_partido_legado}' AND sic.id_seccion_ine = si.id) militantes,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_categorias sicc WHERE sicc.id_distrito_local={$id_distrito_local} AND sicc.id_tipo_categoria_ciudadano = '{$id_tipo_categoria_ciudadano}' AND sicc.id_seccion_ine = si.id) funcionarios,
				...
				(SELECT SUM(cvrm2022.votos_can_nreg) FROM casillas_votos_2022_revocacion_mandato cvrm2022 WHERE cvrm2022.id_seccion_ine = si.id) consulta_rvm_2022_votos_can_nreg,
				(SELECT SUM(cpvrm2022.votos) FROM casillas_preguntas_2022_revocacion_mandato cpvrm2022 WHERE cpvrm2022.id_seccion_ine = si.id) consulta_rvm_2022_votos_validos
		...
		$result = $conexion->query($sql); 
		while($row=$result->fetch_assoc()){
			$row['consulta_rvm_2022_votos_totales'] = $row['consulta_rvm_2022_votos_nulos'] + $row['consulta_rvm_2022_votos_validos'];
			...
			if(!empty($consulta_2022_rnm[$row['id']])){
				$datos_secciones_ine[$row['id']]['revocacion_mandato_2022']['preguntas']=$consulta_2022_rnm[$row['id']];
				...
				foreach ($orden_preguntas as $votos => $orden) {
					foreach ($orden as $index => $pregunta) {
						$datos_secciones_ine[$row['id']]['revocacion_mandato_2022']['orden_preguntas'][$pregunta]=$votos;
						if($primera_pregunta==''){
							$primera_pregunta = $pregunta;
							$datos_secciones_ine[$row['id']]['revocacion_mandato_2022']['principales']['prinicipal']=$primera_pregunta;
						}elseif ($primera_fuerza!='' && $segunda_pregunta=='') {
							$segunda_pregunta = $pregunta;
							$datos_secciones_ine[$row['id']]['revocacion_mandato_2022']['principales']['secundaria']=$segunda_pregunta;
						}
					}
				}
			}
								...
								if(!empty($value['revocacion_mandato_2022'])){
									...
									$div .= '
										<div class="datos_consulta" style=" background: rgba(190, 195, 201, 0.9) "> 
											...
											<br>
											Casillas: <b>'.number_format($value['consulta_rvm_2022_casillas_rvm_2022'], 0, '.', ',').'</b>
											<br>
