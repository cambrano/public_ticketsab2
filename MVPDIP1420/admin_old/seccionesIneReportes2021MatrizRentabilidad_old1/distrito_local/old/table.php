...
?> 
	<script type="text/javascript">
		...
		function downloadExcel(){
			...
			?>
			var link="seccionesIneReportes2021MatrizRentabilidadMatrizRentabilidad/distrito_local/excel/index.php?cot=<?=$_COOKIE['pageService']?>"; 
			//window.open(link);
	...
	<table id="secciones_reportes-tabla" class="table table-striped table-bordered  cell-border compact stripe" style="width:100%">
		<thead>
			<tr>
				...
				<th>Consulta 2022 Revocación Mandato</th> 
				<th>Tipo Sección</th> 
			</tr>
		</thead>
		<tbody>
			<?php
				foreach ($datos_secciones_ine as $key => $value) {
					...
					if($value['consulta_2022_rnm']['casillas']>0){
						$consulta_2022_rm = "<div class='div_info_seccion'>
							Lista Nominal :<b>".number_format($value['consulta_2022_rnm']['lista_nominal'], 0, '.', ',')."</b><br>
							Casillas :<b>".$value['consulta_2022_rnm']['casillas']."</b><br><br> 


							Votos Siga :<b>".number_format($value['consulta_2022_rnm']['consulta']['SIGA']['votos'], 0, '.', ',')." (".number_format($value['consulta_2022_rnm']['consulta']['SIGA']['porcentaje_votos'], 2, '.', ',')."%)</b><br>

							Votos No Siga :<b>".number_format($value['consulta_2022_rnm']['consulta']['NO_SIGA']['votos'], 0, '.', ',')." (".number_format($value['consulta_2022_rnm']['consulta']['NO_SIGA']['porcentaje_votos'], 2, '.', ',')."%)</b><br>


							Votos Nulos:<b>".number_format($value['consulta_2022_rnm']['votos_nulos'], 0, '.', ',')." (".number_format($value['consulta_2022_rnm']['porcentaje_nulos'], 2, '.', '')."%)</b><br>
							Votos Totales:<b>".number_format($value['consulta_2022_rnm']['votos_totales'], 0, '.', ',')."</b><br><br>
							P. Ciudadana:<b>".number_format($value['consulta_2022_rnm']['participacion_ciudadana'], 2, '.', ',')."%</b><br>

						</div>";
					}else{
						$consulta_2022_rm = "<b>SIN DATA</b>";
					}
