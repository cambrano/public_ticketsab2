		...
		*/
		...
		$sql="
			SELECT
				...
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic WHERE sic.id_distrito_federal={$id_distrito_federal} AND sic.id_seccion_ine = si.id ) ciudadanos_registrados,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} AND sic.id_seccion_ine = si.id) apoyos_programas,
				...
				(SELECT COUNT(*) FROM militantes_partidos mp LEFT JOIN secciones_ine_ciudadanos sic ON mp.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal} AND mp.id_partido_legado = '{$id_partido_legado}' AND sic.id_seccion_ine = si.id) militantes,
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_categorias sicc WHERE sicc.id_distrito_federal={$id_distrito_federal} AND sicc.id_tipo_categoria_ciudadano = '{$id_tipo_categoria_ciudadano}' AND sicc.id_seccion_ine = si.id) funcionarios,
			...
			if($validador <= 0){
				...
				if(!empty($datos_secciones_ine[$row['id']]['orden_votos_individual']['primera_fuerza']) && !empty($datos_secciones_ine[$row['id']]['orden_votos_individual']['segunda_fuerza']) ){
					...
					if($partido_primera_fuerza ==$searh_partido){
						...
						$diferencia = $votos_primera_fuerza - $votos_sistema_fuerza;
					...
					////////////////////////////////////////////////////////////////////////////////////
					////////////////////////////////////////////////////////////////////////////////////
					///? esto es para en votos en individual y es para saber las diferencias entre primera y segunda fuerza aunque no sea del
								...
								if(!empty($value['revocacion_mandato_2022'])){
									...
									$div .= '
										<div class="datos_consulta" style=" background: rgba(190, 195, 201, 0.9) "> 
											...
											<br>
											Casillas: <b>'.number_format($value['consulta_rvm_2022_casillas_rvm_2021'], 0, '.', ',').'</b>
											<br>
