<?php
	...
	$sql = "SELECT 
				...
				(SELECT COUNT(*) FROM secciones_ine_ciudadanos_programas_apoyos sicpa LEFT JOIN secciones_ine_ciudadanos sic ON sicpa.id_seccion_ine_ciudadano = sic.id WHERE sic.id_distrito_federal={$id_distrito_federal}) apoyos_programas,
				(SELECT COUNT(*) FROM secciones_ine_actividades sia LEFT JOIN secciones_ine si ON sia.id_seccion_ine = si.id WHERE si.id_distrito_federal={$id_distrito_federal} ) acciones_obras,
				(SELECT COUNT(*) FROM secciones_ine_grupos sig LEFT JOIN secciones_ine si ON sig.id_seccion_ine = si.id WHERE si.id_distrito_federal={$id_distrito_federal} ) grupos_interes,
...
<div class="totales">
	<div
					...
					<tbody>
						...
						<tr>
							...
							<td
								style="text-align: right; width: 25%;padding: 0px 5px 0px 5px;background-color: rgba(176,176,176,0.3); border-bottom: 1px solid white">
								<font class="fontDataReporte" style="font-size: 12px">
									<?=number_format($grupos_interes, 0, '.', ','); ?>
								</font>
							</td>
						</tr>
					</tbody>
		...
		foreach ($datos_partidos['orden_votos_totales'] as $key => $value) {
			if ($key !='sistema' && $key !='partidos' && $key !='graficas' && $key !='diferencia_votos_fuerzas' && $key !='diferencia_votos_sistema'){
				$value =$datos_partidos['partidos'][ $datos_partidos['orden_votos_totales'][$key]];
				?>
			<div class="div50Reporte">
				<table
					style="table-layout: fixed; width: 100%"
		...
		<div style="background-color: white;padding: 5px;display: table;width:100%">
			<div class='div60Reporte' >
				<div id="appPartidosTotales">
					<div id="chart">
						<apexchart type="bar" height="380" :options="chartOptionsPartidosTotales" :series="series"></apexchart>
					</div>
				</div>
				<script>
					new Vue({
			...
			<div class='div40Reporte' >
				<div id="appPartidosPorcentajes">
					<div id="chart">
						<apexchart type="donut" height="300" :options="chartOptionsPartidosPorcentajes" :series="series"></apexchart>
					</div>
				</div>
				<script>
					new Vue({
						...
						data: {
							...
							chartOptionsPartidosPorcentajes: {
								tooltip: {
									enabled: false,
								},
