<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) votos_nulos_2021,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) votos_can_nreg_2021,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) casillas_2021,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) total_lista_nominal_2021,

				( SELECT id_partido_2021 votos FROM casillas_votos_partidos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} GROUP BY id_partido_2021 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2021,

				(SELECT COUNT(id) FROM secciones_ine_ciudadanos WHERE id_distrito_federal={$id_distrito_federal} ) ciudadanos_totales,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE id_distrito_federal={$id_distrito_federal} AND check_in=1 AND check_out=1 ) ciudadanos_totales_doble_check,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE id_distrito_federal={$id_distrito_federal} AND check_in=1 ) ciudadanos_totales_check_in,
				(SELECT COUNT(id) FROM secciones_ine_ciudadanos_check_2021 WHERE id_distrito_federal={$id_distrito_federal} AND check_out=1 ) ciudadanos_totales_check_out,
				(SELECT COUNT(sic.id) FROM secciones_ine_ciudadanos sic LEFT JOIN secciones_ine_ciudadanos_check_2021 sicc2021 ON sic.id = sicc2021.id_seccion_ine_ciudadano WHERE IF(sicc2021.check_in IS NULL, 0,sicc2021.check_in ) = 0 AND IF(sicc2021.check_out IS NULL, 0,sicc2021.check_out ) = 0 AND sic.id_distrito_federal={$id_distrito_federal} ) ciudadanos_totales_sin_check
			FROM
				(SELECT SUM(votos) votos_validos_2021 FROM  casillas_votos_partidos_2021 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} )t
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2021.votos) FROM casillas_votos_partidos_2021 cpv2021 WHERE cpv2021.tipo = 2 AND cpv2021.id_partido_2021 = p2021.id AND cpv2021.id_distrito_federal={$id_distrito_federal}  ) votos_totales_2021
			FROM partidos_2021 p2021
			WHERE p2021.principal=1 AND p2021.tipo = '2' AND p2021.codigo_plataforma = '{$codigo_plataforma}'";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	...
	$sql="
		...
		WHERE cvp2021.id_distrito_federal={$id_distrito_federal} AND p2021.tipo = 2 AND cvp2021.tipo = 2
