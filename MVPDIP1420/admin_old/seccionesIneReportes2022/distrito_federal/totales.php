<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2022 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) votos_nulos_2022,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2022 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) votos_can_nreg_2022,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_federal={$id_distrito_federal} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2022 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) casillas_2022,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2022 WHERE  tipo = 2 AND id_distrito_federal={$id_distrito_federal} ) total_lista_nominal_2022,

				( SELECT id_partido_2022 votos FROM casillas_votos_partidos_2022 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} GROUP BY id_partido_2022 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2022
			FROM
				(SELECT SUM(votos) votos_validos_2022 FROM  casillas_votos_partidos_2022 WHERE tipo = 2 AND id_distrito_federal={$id_distrito_federal} )t
	...
	$distritos_federales_2022 = $row['distritos_federales_2022']-1;
	$distritos_federales_2022 = $row['distritos_federales_2022'];
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2022.votos) FROM casillas_votos_partidos_2022 cpv2022 WHERE cpv2022.tipo = 2 AND cpv2022.id_partido_2022 = p2022.id AND cpv2022.id_distrito_federal={$id_distrito_federal}  ) votos_totales_2022
			FROM partidos_2022 p2022
			WHERE p2022.principal=1 AND p2022.tipo = 2";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	$partido_sistema_nombre_2022 = $row['nombre'];
	$partido_sistema_nombre_corto_2022 = $row['nombre_corto'];
	$partido_sistema_logo_2022 = $row['logo'];
	$partido_sistema_id_2022 = $row['id'];
	$partido_sistema_color_background_2022 = $row['color_background'];
	$partido_sistema_votos_2022 = $row['votos_totales_2022'];

	if($votos_validos_2022==0){
		$partido_sistema_votos_porcentaje_2022 = 0;
	}else{
		$partido_sistema_votos_porcentaje_2022 = ($partido_sistema_votos_2022 / $votos_validos_2022 )*100;
	...
	$sql="
		...
		WHERE cvp2022.id_distrito_federal={$id_distrito_federal} AND p2022.tipo = 2 AND cvp2022.tipo = 2 
