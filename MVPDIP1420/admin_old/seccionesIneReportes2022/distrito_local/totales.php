<?php
	...
	$sql = "SELECT 
				...
				(SELECT SUM(votos_nulos) FROM casillas_votos_2022 WHERE tipo = 1 AND id_distrito_local={$id_distrito_local} ) votos_nulos_2022,
				(SELECT SUM(votos_can_nreg) FROM casillas_votos_2022 WHERE tipo = 1 AND id_distrito_local={$id_distrito_local} ) votos_can_nreg_2022,
				(SELECT COUNT(id) FROM secciones_ine WHERE id_distrito_local={$id_distrito_local} ) secciones,
				(SELECT COUNT(id) FROM casillas_votos_2022 WHERE tipo = 1 AND id_distrito_local={$id_distrito_local} ) casillas_2022,
				(SELECT SUM(lista_nominal) FROM casillas_votos_2022 WHERE  tipo = 1 AND id_distrito_local={$id_distrito_local} ) total_lista_nominal_2022,

				( SELECT id_partido_2022 votos FROM casillas_votos_partidos_2022 WHERE tipo = 1 AND id_distrito_local={$id_distrito_local} GROUP BY id_partido_2022 ORDER by SUM(votos) DESC LIMIT 1) id_partido_ganador_2022
			FROM
				(SELECT SUM(votos) votos_validos_2022 FROM  casillas_votos_partidos_2022 WHERE tipo = 1 AND id_distrito_local={$id_distrito_local} )t
	...
	$distritos_locales_2022 = $row['distritos_locales_2022']-1;
	$distritos_locales_2022 = $row['distritos_locales_2022'];
	...
	$sql = "SELECT
			*,
			(SELECT SUM(cpv2022.votos) FROM casillas_votos_partidos_2022 cpv2022 WHERE cpv2022.tipo = 1 AND cpv2022.id_partido_2022 = p2022.id AND cpv2022.id_distrito_local={$id_distrito_local}  ) votos_totales_2022
			FROM partidos_2022 p2022
			WHERE p2022.principal=1 AND p2022.tipo = 1";
	$resultado = $conexion->query($sql);
	$row=$resultado->fetch_assoc();
	...
	$sql="
		...
		WHERE cvp2022.id_distrito_local={$id_distrito_local} AND p2022.tipo = 1 AND cvp2022.tipo = 1 
