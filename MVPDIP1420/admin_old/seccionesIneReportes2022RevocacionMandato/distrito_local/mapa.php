	...
	if(!empty($_POST)){
		...
		*/
		...
		$sql="
			 SELECT
				...
				dl.latitud,
				dl.longitud,
				(SELECT SUM(cvp2022.votos)  FROM casillas_preguntas_2022_revocacion_mandato cvp2022 WHERE cvp2022.tipo = 0 AND cvp2022.id_distrito_local = dl.id GROUP BY cvp2022.id_pregunta_2022_revocacion_mandato ORDER BY SUM(cvp2022.votos) DESC LIMIT 1) partido_ganador_votos,

				(SELECT cvp2022.id_pregunta_2022_revocacion_mandato  FROM casillas_preguntas_2022_revocacion_mandato cvp2022 WHERE cvp2022.tipo = 0 AND cvp2022.id_distrito_local = dl.id GROUP BY cvp2022.id_pregunta_2022_revocacion_mandato ORDER BY SUM(cvp2022.votos) DESC LIMIT 1) partido_ganador_id,
				...
				(SELECT SUM(cvp2022.votos)  FROM casillas_preguntas_2022_revocacion_mandato cvp2022 WHERE cvp2022.tipo = 0 AND cvp2022.id_distrito_local = dl.id AND cvp2022.id_pregunta_2022_revocacion_mandato = p2022Sistema.id ) partido_sistema_votos,

				(SELECT SUM(cv2022.votos_nulos) FROM casillas_votos_2022_revocacion_mandato cv2022 WHERE cv2022.tipo = 0 AND cv2022.id_distrito_local = dl.id ) votos_nulos,

				(SELECT SUM(cv2022.votos_can_nreg) FROM casillas_votos_2022_revocacion_mandato cv2022 WHERE cv2022.tipo = 0 AND cv2022.id_distrito_local = dl.id ) votos_can_nreg,

				(SELECT SUM(cv2022.lista_nominal) FROM casillas_votos_2022_revocacion_mandato cv2022 WHERE cv2022.tipo = 0 AND cv2022.id_distrito_local = dl.id ) lista_nominal,

				(SELECT SUM(cvp2022.votos) FROM casillas_preguntas_2022_revocacion_mandato cvp2022 WHERE cvp2022.tipo = 0 AND cvp2022.id_distrito_local = dl.id ) votos_validos

			FROM distritos_locales dl
			LEFT JOIN preguntas_2022_revocacion_mandato p2022Sistema
			ON p2022Sistema.principal = 1
			/*WHERE dl.id = '{$id_distrito_local}' AND p2022Sistema.tipo = 0 */
		...
		$sql="SELECT 
			...
			WHERE cvp2018.tipo = '{$tipo_eleccion}'  AND cvp2018.id_distrito_local = '{$id_distrito_local}' {$sql_secciones_ine}
		...
		*/
		...
		$sql="SELECT 
			...
			FROM casillas_preguntas_2022_revocacion_mandato cvp2018 
			LEFT JOIN preguntas_2022_revocacion_mandato p2018 ON cvp2018.id_pregunta_2022_revocacion_mandato = p2018.id
			WHERE cvp2018.tipo = '{$tipo_eleccion}'  AND cvp2018.id_distrito_local = '{$id_distrito_local}'
			GROUP BY cvp2018.id_seccion_ine,id_pregunta_2022_revocacion_mandato 
			ORDER BY cvp2018.id_seccion_ine,SUM(cvp2018.votos) DESC
			";
		$result = $conexion->query($sql);
						...
						if($value['id'] != $id_distrito_local){
							...
							$div = '<div class="divMapa">
										<div class="info_content">
											<h4>Distrio Local: '.$value['distrito_local'].'</h4>
