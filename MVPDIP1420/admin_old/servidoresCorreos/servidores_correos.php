<?php
	...
	$sql = "SELECT count(*) total FROM servidores_correos WHERE 1 = 1   "; 
	...
	////////////////////////////
	////////////////////////////
	$sql = "SELECT id,clave,nombre,sufijo FROM servidores_correos WHERE 1 = 1  "; 
	...
	$sufijo=$_SESSION['searchTable']['sufijo'];
	if( $sufijo!="" ){   //name
		$post_search=true;
		$sql.=" AND sufijo LIKE '%{$sufijo}%' ";
		$sqlContador.=" AND sufijo LIKE '%{$sufijo}%' ";
	}
	...
	while($row=$result->fetch_assoc()){
		...
		$nestedData[] = "<div style='text-transform: none;'>".$row["sufijo"]."<div>";
		if($option_delete){
		...
		//$nestedData[] =  "<div style='width:13vw'>{$edit}{$delete}{$select}</div>";
	...
	if($post_search){
		$sqlContadorScript = "SELECT count(*) total FROM servidores_correos WHERE 1 = 1   "; 
