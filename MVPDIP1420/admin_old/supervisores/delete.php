<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="supervisores/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'supervisores','supervisores','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('supervisores',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "supervisores/db_delete.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Supervisor</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Supervisor</label><br>
			<label class="descripcionForm">
				<strong><?= $supervisorDatos['nombre_completo']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
