<?php
	...
	if(!empty($_POST)){
		...
		$delete_tipos_casillas = "DELETE FROM tipos_casillas  WHERE  id='{$id}' ";
		$delete_tipos_casillas=$conexion->query($delete_tipos_casillas);
		$num=$conexion->affected_rows;
		if(!$delete_tipos_casillas || $num=0){
			$success=false;
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'tipos_casillas',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
