<?php
	...
	@session_start(); 
	$_SESSION['Paguinasub']="tiposTerritorios/delete.php";
	if(!empty($_GET)){
		$id=$_SESSION['paguinaId']=$_GET['id'];
	}else{
		$id=$_SESSION['paguinaId'];
	}
	echo $redirectSecurity=redirectSecurity($id,'tipos_territorios','tiposTerritorios','index');
	if($redirectSecurity!=""){
		die;
	}
	...
	$tablasRelacionadas = tablasRelacionadas('tipos_territorios',$id);
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "tiposTerritorios/db_delete.php",
				data: dataString,
		...
		<div class="bodyform">
			<div class= "bodyheader">
				<label class="tituloForm">
					<font style="font-size: 25px;">Eliminar Tipo Territorio</font>
				</label>
			</div>
		</div>
		<div class="bodyinput">
			<label class="labelForm" id="labeltemaname">Tipo Territorio</label><br>
			<label class="descripcionForm">
				<strong><?= $tipo_territorioDatos['nombre']?></strong>
			</label><br>
			<?php
			if($registros_tablas!=''){
