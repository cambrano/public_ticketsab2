<?php
	...
	if(!empty($_POST)){
		...
		$sqlClave = "SELECT id FROM secciones_ine_ciudadanos WHERE curp = '{$curp}' ";
		...
		if(!empty($row)){
			...
			$myObj->ocr = $row['ocr'];
			$myObj->curp = $row['curp'];
			$myObj->clave_elector = $row['clave_elector'];
			...
			$myObj->ocr = 'No Encontrado';
			$myObj->curp = 'No Encontrado';
			$myObj->clave_elector = 'No Encontrado';
