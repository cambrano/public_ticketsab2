<?php
	...
	if(!empty($_POST)){
		...
		$delete_secciones_ine_ciudadanos_seguimientos = "DELETE FROM secciones_ine_ciudadanos_seguimientos  WHERE  id='$id' ";
		...
		if($success){
			$log= logUsuario($_COOKIE["id_usuario"],'secciones_ine_ciudadanos_seguimientos',$id,'Delete','',$fechaH);
			if($log==true){
				echo "SI";
				$conexion->commit();
				$conexion->close();
