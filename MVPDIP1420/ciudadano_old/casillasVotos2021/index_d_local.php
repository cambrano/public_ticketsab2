<?php
	...
	@session_start();
	$_SESSION['Paguinasub']="casillasVotos2021/index_ayuntamiento.php";

	if(!empty($_GET)){
		$tipo=$_SESSION['paguinaId']=$_GET['tipo'];
	}else{
		$tipo=$_COOKIE['paguinaId']=1;
	}

	if($_COOKIE["id_casilla_voto_partidos_tipo"] == $tipo){
		$entra=true;
	}
	...
	$id_casilla_voto_2021 = $_COOKIE["id_casilla_voto_partidos"];



	if($id_casilla_voto_2021!="" && $entra){
		$input_dis ='disabled="disabled"';
		$boton_dis = 'style="background-color: black; color: yellow; cursor: not-allowed;"';

		$casilla_voto_2021Datos=casilla_voto_2021Datos($id_casilla_voto_2021,'',$tipo);
		...
		foreach ($partidos_2021Datos as $key => $value) {
			foreach ($casillas_votos_partidos_2021Datos as $keyT => $valueT) {
				if($value['id']==$valueT['id_partido_2021']){
					$partidos_2021Datos[$key]['votos'] = $valueT['votos'];
					$votos_validos = $valueT['votos'] + $votos_validos;
				}
			}
		}
		$votos_totales = $votos_validos + $casilla_voto_2021Datos['votos_can_nreg'] + $casilla_voto_2021Datos['votos_nulos'];
	}

	
	$casilla_voto_2021_statusDatosAll = casilla_voto_2021_statusDatosAll('',$_COOKIE["id_casilla_voto_partidos"],'ORDER BY fecha_hora DESC LIMIT 1');
	$casilla_voto_2021_incidenciasDatos = casilla_voto_2021_incidenciasDatos('',$_COOKIE["id_casilla_voto_partidos"],'ORDER BY fecha_hora DESC LIMIT 1');
...
?>
	<script type="text/javascript">
		function asignar_casilla(){
			...
			var id_casilla_voto_2021 = document.getElementById("id_casilla_voto_2021").value;
			...
			$.ajax({
				type: "POST",
				url: "casillasVotos2021/asignar_casilla.php",
				...
				success: function(data) {
					...
					document.getElementById("id_casilla_voto_2021").disabled = true;
					location.reload();
				}
			});
		}
		function desbloquear_casilla(){
			...
			document.getElementById("id_casilla_voto_2021").disabled = false;
		}
	</script>
	<script type="text/javascript">
	...
	<div id="bodymanager" class="bodymanager">
		...
		<div class="sucForm">
			...
			<select id="id_casilla_voto_2021" class='myselect' <?= $input_dis ?> >
				<?= casillas_votos_2021($id_casilla_voto_2021,$id_seccion_ine,$tipo) ?>
			</select>
		</div>
		<div class="sucForm">
		...
		<script language="javascript" type="text/javascript">
			function guardar() {
				...
				*/
				...
				casilla_voto_2021.push(data);


				///partidos
				///partidos
				var votos_partidos_2021 = [];
				...
				$.ajax({
					type: "POST",
					url: "casillasVotos2021/db_edit.php",
					data: {casilla_voto_2021: casilla_voto_2021,votos_partidos_2021:votos_partidos_2021},
					success: function(data) {
						if(data=="SI"){
							document.getElementById("sumbmit").disabled = true;
							document.getElementById("mensaje").classList.remove("mensajeError");
							...
							document.getElementById("mensaje").classList.add("mensajeSucces");
							//$("#homebody").load('casillasVotos2021/index.php');
						}else{
							document.getElementById("sumbmit").disabled = false;
							$("#mensaje").html(data);
							document.getElementById("mensaje").classList.add("mensajeError");
