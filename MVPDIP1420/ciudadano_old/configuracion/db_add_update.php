<?php
	...
	if(
		moduloAccion('configuracion','configuracion_inicial',$_COOKIE["id_usuario"],'Update') ||
		moduloAccion('configuracion','configuracion_inicial',$_COOKIE["id_usuario"],'Insert') ){
	}else{
	...
	if(!empty($_POST)){
		...
		if($success && $imagen!="" && $configuracionDatos['file_size']<=$_FILES['imagen']['size'] ){
			...
			$rutaDestino = "../ftpFiles/files/logo_principal.png";
