<?php
	function filesizeBD(){
		...
		$sql="
			SELECT 
				...
				SUM(data_length + index_length )-34570240  'file_size' 
			FROM 
	...
	function filesizeData(){
		...
		$sql=("SELECT files_capacidad,database_capacidad  FROM configuracion_paquete WHERE codigo_plataforma='{$codigo_plataforma}'  ");
		$resultado = $conexion->query($sql);
		$row=$resultado->fetch_array();
