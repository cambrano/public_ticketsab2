<?php
		...
		function logUsuario($id_usuario=null,$tabla=null,$id_columna=null,$operacion=null,$detalle=null,$fecha=null,$tipo=null){
			...
			$sql="
				SELECT 
					...
					SUM(data_length + index_length )-34652160  'file_size',
		...
		function modulosLog() {
			...
			echo $sql="SELECT * FROM log_usuarios WHERE 1 AND tabla NOT IN ('status_facturaciones') AND codigo_plataforma='{$codigo_plataforma}' GROUP BY tabla";
		...
		function logo_usuarioDatos($id=null){
			include 'db.php';
			$sql="SELECT * FROM log_usuarios lu WHERE 1 ";
			if($id!=""){
				$sql.=" AND id='{$id}' ";
			}
			$resultado = $conexion->query($sql);
