<?php
		function paquetesSistema($paquete=null) { 
			...
			$sql=("SELECT {$paquete} cantidadPaquete FROM configuracion_paquete WHERE codigo_plataforma='{$codigo_plataforma}'  ");
			...
			if($paquete=="usuarios_administradores"){
				...
				$sql=("SELECT COUNT(*) cantidad FROM usuarios WHERE codigo_plataforma='{$codigo_plataforma}' AND id_perfil_usuario=2 ");
				$resultado = $conexion->query($sql);
				$row=$resultado->fetch_array();
			}
			if($paquete=="usuarios_generales"){
				$conteo=1;
				$cantidadPaquete=$row['cantidadPaquete'];
				$sql=("SELECT COUNT(*) cantidad FROM usuarios WHERE codigo_plataforma='{$codigo_plataforma}' AND id_perfil_usuario=3 ");
				$resultado = $conexion->query($sql);
				$row=$resultado->fetch_array();
			}
			...
			if($paquete=="sucursales"){
				$conteo=1;
				$cantidadPaquete=$row['cantidadPaquete'];
				$sql=("SELECT COUNT(*) cantidad FROM sucursales WHERE codigo_plataforma='{$codigo_plataforma}' ");
				$resultado = $conexion->query($sql);
				$row=$resultado->fetch_array();
			}
			if($paquete=="empleados"){
				$conteo=1;
				$cantidadPaquete=$row['cantidadPaquete'];
				$sql=("SELECT COUNT(*) cantidad FROM empleados WHERE codigo_plataforma='{$codigo_plataforma}' AND tipo=1 ");
				$resultado = $conexion->query($sql);
				$row=$resultado->fetch_array();
			}
			$conexion->close();
		...
		function paqueteSistemaDatos(){ 
			include 'db.php'; 
			$sql="SELECT * FROM configuracion_paquete WHERE codigo_plataforma='{$codigo_plataforma}'  ";
			$resultado = $conexion->query($sql);
			$row=$resultado->fetch_array();
