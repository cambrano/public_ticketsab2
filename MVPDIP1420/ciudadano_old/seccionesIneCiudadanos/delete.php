<?php
	...
	foreach ($tablasRelacionadas['tablas'] as $key => $value) {
		if($value['registros']>0 && $value['tabla']!='usuarios' && $value['tabla']!='secciones_ine_ciudadanos_permisos'  && $value['tabla']!='secciones_ine_ciudadanos_categorias' && $value['tabla']!='secciones_ine_ciudadanos_check_2021'){
...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		function guardar() {
			...
			$.ajax({
				type: "POST",
				url: "seccionesIneCiudadanos/db_delete.php",
				data: dataString,
				success: function(data) { 
					if(data=="SI"){
						document.getElementById("sumbmit").disabled = true;
	...
	<div class="bodymanager" id="bodymanager"> 
		...
		<div class="bodyinput">
			...
			if($switch_operacionesPermisos['registro']==true){
				?>
				<input style="width: 100%;margin-bottom: 10px" type="button" id="sumbmit" onclick="guardar()" value="SI">
				<?php
			}
			?>
			<input style="width: 100%;margin-bottom: 10px" type="button" onclick="cerrar()" value="NO">
		</div>
	</div>