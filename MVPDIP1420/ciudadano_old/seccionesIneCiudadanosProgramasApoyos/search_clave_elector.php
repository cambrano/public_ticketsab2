<?php
	...
	if(!empty($_POST)){
		...
		$id_programa_apoyo = $_POST['id_programa_apoyo'];
		...
		$sql="
			SELECT 
				...
				ln.calle,
				ln.id_estado,
				(SELECT e.abreviacion_2 FROM estados e WHERE e.id = ln.id_estado) estado_dig_2,
				...
				ln.num_int,
				ln.num_ext,
				ln.correo_electronico,
				ln.whatsapp,
				ln.celular,
				ln.telefono
			FROM secciones_ine_ciudadanos ln
		...
		if(!empty($row)){
			...
			if($row['num_int']==''){
				$row['num_int']='No Encontrado';
			}
			if($row['num_ext']==''){
				$row['num_ext']='No Encontrado';
			}
			$myObj->no_exterior = $row['num_ext'];
			$myObj->no_interior = $row['num_int'];
			$myObj->colonia = $row['colonia'];
			$myObj->codigo_postal = $row['codigo_postal'];
			...
			$myObj->correo_electronico = $row['correo_electronico'];
			$myObj->whatsapp = $row['whatsapp'];
			$myObj->celular = $row['celular'];
			...
			$myObj->correo_electronico = 'No Encontrado';
			$myObj->whatsapp = 'No Encontrado';
			$myObj->celular = 'No Encontrado';
			$myObj->telefono = 'No Encontrado';
		}
		//// buescar si existe en el sistema
		$id_seccion_ine_ciudadano = $row['id'];
		$sqlClave = "SELECT count(*) contador FROM secciones_ine_ciudadanos_programas_apoyos WHERE id_seccion_ine_ciudadano = '{$id_seccion_ine_ciudadano}' AND id_programa_apoyo='{$id_programa_apoyo}' ";
		$resultClave = $conexion->query($sqlClave);
		$rowClave=$resultClave->fetch_assoc();
		
		$myObj->contador = $rowClave['contador'];
