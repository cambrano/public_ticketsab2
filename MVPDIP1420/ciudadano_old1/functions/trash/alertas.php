<?php
		function alertas($tabla=null,$id_empleado=null) {
			...
			if($tabla=='empleados'){
				...
				$sql="SELECT e.id,CONCAT_WS(' ',e.nombre,e.apellido_paterno,e.apellido_materno) nombreEmpleado,e.clave FROM empleados e WHERE NOT EXISTS (SELECT * FROM sucursales_horarios sh WHERE sh.id_sucursal = e.id_sucursal AND e.id_turno=sh.id_turno) AND e.id_sucursal IS NOT NULL AND e.codigo_plataforma='{$codigo_plataforma}' ";
