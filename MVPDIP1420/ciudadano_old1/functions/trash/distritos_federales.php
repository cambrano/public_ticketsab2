<?php
	...
	function distritos_federales($id=null) {
		include 'db.php'; 
		$id;
		$select[$id]='selected="selected"';
		$return ="<option ".$select[$sel]." value='' >Seleccione</option> ";
		$sql="SELECT * FROM distritos_federales WHERE 1 AND codigo_plataforma='{$codigo_plataforma}'";
	...
	function filtrosSelect($columna=null) {
		...
		$return ="<option ".$select[$sel]." value='' >Seleccione</option> ";
		$sql = "SELECT {$columna} columna FROM distritos_federales WHERE 1  ";
	...
	*/
	...
	function distrito_federalDatos($id=null){
			include 'db.php';
			$sql=("SELECT * FROM distritos_federales WHERE codigo_plataforma='{$codigo_plataforma}' ");
			if($id!=""){
				$sql.=" AND id='{$id}' ";
			}
