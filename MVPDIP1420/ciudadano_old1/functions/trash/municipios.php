<?php
		function municipios($id_municipio=null,$id_estado=null) {
			include 'db.php'; 
			$id;
			$select[$id_municipio]='selected="selected"';
			$return ="<option ".$select[$sel]." value='' >Seleccione</option> ";
			if(!empty($id_estado)){
				$sql="SELECT * FROM municipios WHERE 1 AND id_estado={$id_estado} ";
		...
		function municipioNombre($id_municipio=null){
			include 'db.php'; 
			$sql=("SELECT * FROM municipios WHERE 1 ");
			if($id_municipio!=""){
		...
		function municipioId($municipio=null,$id_estado=null){
			include 'db.php'; 
