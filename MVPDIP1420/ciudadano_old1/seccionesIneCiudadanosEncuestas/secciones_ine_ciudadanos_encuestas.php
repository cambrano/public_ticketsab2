<?php
	...
	$sql = "SELECT 
				...
				e.clave,
				e.nombre,
				(SELECT sice.id FROM secciones_ine_ciudadanos_encuestas sice WHERE sice.id_encuesta = e.id AND sice.id_seccion_ine_ciudadano ='{$id_seccion_ine_ciudadano}' LIMIT 1) id_seccion_ine_ciudadano_encuesta,
				(SELECT sice.clave FROM secciones_ine_ciudadanos_encuestas sice WHERE sice.id_encuesta = e.id AND sice.id_seccion_ine_ciudadano ='{$id_seccion_ine_ciudadano}' LIMIT 1) clave_seccion_ine_ciudadano_encuesta,
				(
					IF(
						(SELECT sice.clave FROM secciones_ine_ciudadanos_encuestas sice WHERE sice.id_encuesta = e.id AND sice.id_seccion_ine_ciudadano ='{$id_seccion_ine_ciudadano}' LIMIT 1) IS NULL ,
						'No Realizada',
						(SELECT sice.fecha_hora FROM secciones_ine_ciudadanos_encuestas sice WHERE sice.id_encuesta = e.id AND sice.id_seccion_ine_ciudadano ='{$id_seccion_ine_ciudadano}' LIMIT 1) )
	...
	$id_encuesta=$_SESSION['searchTable']['id_encuesta'];
	if( $id_encuesta!="" ){   //name
		$post_search=true;
