<?php
	...
	$randomString = str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
	$_GET['oh21'] = $randomString;

	// Obtén todos los parámetros GET actuales
	$parametros_get = http_build_query($_GET);

	// Nuevo enlace al que deseas redireccionar con los parámetros GET
	$nuevo_enlace = "m.php?" . $parametros_get;

	// Redirecciona a la nueva página
	header("Location: " . $nuevo_enlace);
	exit();
	die;
	...
	if (!empty($codigo_seccion_ine_ciudadano) && !empty($id_seccion_ine_ciudadano) && !empty($id_militante_partido)) {
	    ...
	    if ($stmt = mysqli_prepare($conexion, "
	        ...
	        WHERE sic.id = ? AND sic.codigo_seccion_ine_ciudadano = ? AND mp.id = ?
	        LIMIT 1"
	    )) {
