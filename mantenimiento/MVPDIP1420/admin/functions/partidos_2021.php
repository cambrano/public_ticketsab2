<?php
		function partidos_2021($id=null,$tipo=null,$coalicion=null,$sin_seleccione=null) {
			include 'db.php'; 
			$id;
			...
			$sql="SELECT * FROM partidos_2021 WHERE 1 = 1 ";
			if($tipo!=''){
				$sql .=" AND tipo = '{$tipo}' ";
			}
		...
		function partido_2021PrincipalDatos(){
			include 'db.php';
			$sql=("SELECT * FROM partidos_2021 WHERE principal=1 ");
			$resultado = $conexion->query($sql);
			$row=$resultado->fetch_assoc();
			$datos=$row; 
			return $datos;
		...
		function partidos_2021CompletaDatos($id=null,$nombre_corto=null){
			include 'db.php';
			$sql=("SELECT * FROM partidos_2021 WHERE 1 ");
			if($id!=''){
				$sql.= " AND id = '{$id}' ";
			}
			if($nombre_corto!=''){
		...
		function partido_2021PrincipalTipoEleccionDatos($tipo=null){
			include 'db.php';
			$sql=("SELECT * FROM partidos_2021 WHERE principal=1 ");
			if($tipo!=""){
				$sql.=" AND tipo={$tipo} ";
		...
		function partidos_2021Datos($id=null,$orden=null,$tipo=null){
			include 'db.php'; 
			$sql=("SELECT * FROM partidos_2021 WHERE 1 = 1 ");
			if($id!=""){
				$sql.=" AND id={$id} ";
