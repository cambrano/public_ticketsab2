<?php
	function secciones_ine_ciudadanosCheck2024DatosArray($registros=null,$orderby=null,$limit=null) {
		...
		$sql="
			...
			(SELECT sicc2024.check_out FROM secciones_ine_ciudadanos_check_2024 sicc2024 WHERE sicc2024.id_seccion_ine_ciudadano = e.id  LIMIT 1) check_out,
			(SELECT sicc2024.check_out_hora FROM secciones_ine_ciudadanos_check_2024 sicc2024 WHERE sicc2024.id_seccion_ine_ciudadano = e.id LIMIT 1 ) check_out_hora,
		...
		foreach ($registros as $key => $value) {
			...
			if($value !=""){
				if($key!="fecha_1" && $key!="fecha_2"){
					if($key=="sexo" || $key=="id_seccion_ine_ciudadano_compartido" ){
						$sql.= " AND  e.{$key} = '{$value}' ";
					}elseif($key=="nombre_completo"){
						$sql.=" AND e.nombre_completo LIKE '%{$value}%' ";
					}elseif($key=="nombre" || $key=="apellido_paterno" || $key=="apellido_materno" || $key=="clave"){
						$sql.=" AND e.{$key} LIKE '%{$value}%' ";
	...
	function seccion_ine_ciudadano_check_2024Datos($id=null,$id_seccion_ine=null,$id_municipioL=null,$id_casilla_voto_2024=null,$id_usuario=null,$id_empleado=null,$id_seccion_ine_ciudadano=null,$id_seccion_ine_ciudadano_relacionado=null){ 
		include 'db.php'; 
