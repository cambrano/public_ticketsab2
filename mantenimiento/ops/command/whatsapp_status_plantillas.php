<?php
	...
	Cuando envias imagenes/ archivos /
	*/
	if(!empty($_POST)){
		include __DIR__."/../db.php";
		/*
		$dir = __DIR__.'/plantilla'.date('H:i_s').'.txt';
		$fp = fopen($dir, 'w');
		...
		*/
		...
		$scriptSQL=" SELECT * FROM api_whatsapp_mensajes WHERE sid = '{$SmsSid}'; ";
		...
		if($tipo == '0'){
			///plantilla
			$scriptSQL=" SELECT * FROM api_whatsapp_envios WHERE id = '{$id_whatsapp_envio}'; ";
			$resultado = $conexion->query($scriptSQL);
			$row=$resultado->fetch_assoc();
			$id_seccion_ine_ciudadano_campana_whatsapp_programada = $row['id_seccion_ine_ciudadano_campana_whatsapp_programada'];
			$id_seccion_ine_ciudadano = $row['id_seccion_ine_ciudadano'];
			//// ENVIADO
			// en campañas whastasapp ciudadanos 
			// identificador  = SmsSid
			//edit api_whatsapp_mensajes
			//edit campañas whastasapp ciudadanos 
			...
			// creamos con los valores de estatus
