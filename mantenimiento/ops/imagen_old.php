...
if($file!=""){
	...
	$ruta ='';
	$total = count($path_array)-2;
	for ($i=1; $i <= $total ; $i++) { 
		$ruta .= "/".$path_array[$i];
	}

	$archivo= $_SERVER['DOCUMENT_ROOT']."/{$dir_base}/admin/ftpFiles/files/{$file}";

	$archivo= $ruta."/{$dir_base}/admin/ftpFiles/files/{$file}";
	if (file_exists($archivo)){
		...
		*/
		...
		$archivo= $ruta."/{$dir_base}/admin/ftpFiles/file_roto.gif";
		$mime=mime_content_type($archivo);
		header("Content-type: {$mime}");
		header("Content-length: ".filesize($archivo));
		header("Content-Disposition: inline; filename=$file");
