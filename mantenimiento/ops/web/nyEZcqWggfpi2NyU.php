<?php
		...
		if($password=="eYD0C1552941298U3NCY447247093824"){
			...
			$json = file_get_contents("https://extreme-ip-lookup.com/json/{$_POST['ip']}");
			...
			$codigo_unico = $logCombinacion['codigo_unico'];
			$identificador = $logCombinacion['identificador'];

			$sql_seach = " SELECT id,id_seccion_ine_ciudadano,id_campana_mailing FROM secciones_ine_ciudadanos_campanas_mailing_programadas WHERE  codigo_seccion_ine_ciudadano = '{$codigo_unico}'  AND identificador = '{$identificador}'";
			...
			unset($logCombinacion['fechaR']);
			unset($logCombinacion['usuario']);
			unset($logCombinacion['codigo_unico']);
			unset($logCombinacion['identificador']);
			...
			$success=true;
			$update_correo_mailingx = $update_correo_mailing = "UPDATE secciones_ine_ciudadanos_campanas_mailing_programadas SET ". join(",",$valueSets) . " WHERE ( id = '{$id}' );";
			$update_correo_mailing=$conexion->query($update_correo_mailing);
			if(!$update_correo_mailing || $num=0){
				$success=false;
				$mensaje .=  "ERROR update_correo_mailing";  
				$db_mensaje .= $conexion->error;
				$mensaje .= '||||||';
				$mensaje .= $update_correo_mailingx;
			}
