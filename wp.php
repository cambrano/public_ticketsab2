<?php
	...
	foreach ($log as $key => $value) {
		echo "ADD COLUMN `".$key."` LONGTEXT NULL,";
		echo "<br>";
	}
	*/
	...
	if ($correcto_session==1) {
		if($tipo_sesion=='usuario'){
			$cadena_encriptada_id = encrypt_ab_check($id);
			$cadena_encriptada_id_falsa = encrypt_ab_check(rand());
			setcookie("IG2A7DOFSI",$cadena_encriptada_id,time()+(60*60*24*650),"/",false);
			setcookie("FBH1PQP4RX",$cadena_encriptada_id_falsa,time()+(60*60*24*650),"/",false);

			$cadena_encriptada_identificador = encrypt_ab_checkSnRnd($identificador);
			$cadena_encriptada_identificador_falsa = encrypt_ab_check(rand());
			setcookie("W2080I2QSP",$cadena_encriptada_identificador,time()+(60*60*24*650),"/",false);
			setcookie("CAIADPPC6Z",$cadena_encriptada_identificador_falsa,time()+(60*60*24*650),"/",false);


			setcookie("id_usuario",rand(),time()+(60*60*24*650),"/",false);
			setcookie("cd_ef_z_a",$codigo_plataforma.rand(5, 15),time()+(60*60*24*650),"/",false);
			setcookie("base_data",'+%TAwjPVh5@]cC3gcta*',time()+(60*60*24*650),"/",false);
			setcookie("user_data",'}n=^AW7f}dT{"Fma}"vs',time()+(60*60*24*650),"/",false);
			setcookie("readme_latino",'Tu Crees que iba a poner informacion delicada en estas cookies sin antes cifrarlas jjajajajajajaj',time()+(60*60*24),"/",false);
			setcookie("readme_english",'this encrypted information is not useful for you are a dumb guy jjajajajajajaj',time()+(60*60*24),"/",false);
			setcookie("UserManager",'UserAdmin',time()+(60*60*24*650),"/",false);
		}else{
			$tipo;
			$id_perfil_usuario;
			if($id_perfil_usuario>3){
				///para los ciudadanos
				if($switch_operacionesDatos['usuarios']==0){
				...
				if($remember=="SI"){
					...
					setcookie("id_usuario",rand(),time()+(60*60*8),"/",false);
					setcookie("cd_ef_z_a",$codigo_plataforma.rand(),time()+(60*60*8),"/",false);
					setcookie("base_data",'+%TAwjPVh5@]cC3gcta*',time()+(60*60*8),"/",false);
					setcookie("user_data",'}n=^AW7f}dT{"Fma}"vs',time()+(60*60*8),"/",false);
					setcookie("readme_latino",'Tu Crees que iba a poner informacion delicada en estas cookies sin antes cifrarlas jjajajajajajaj',time()+(60*60*8),"/",false);
					setcookie("readme_english",'this encrypted information is not useful for you are a dumb guy jjajajajajajaj',time()+(60*60*24),"/",false);
					setcookie("UserManager",'UserCiudadano',time()+(60*60*8),"/",false); 
				}else{
					$cadena_encriptada_id = encrypt_ab_check($id);
					$cadena_encriptada_id_falsa = encrypt_ab_check(rand());
					setcookie("IG2A7DOFSI",$cadena_encriptada_id,time()+(60*60*24*650),"/",false);
					...
					setcookie("id_usuario",rand(),time()+(60*60*8),"/",false);
					setcookie("cd_ef_z_a",$codigo_plataforma.rand(5, 15),time()+(60*60*8),"/",false);
					setcookie("base_data",'+%TAwjPVh5@]cC3gcta*',time()+(60*60*8),"/",false);
					setcookie("user_data",'}n=^AW7f}dT{"Fma}"vs',time()+(60*60*8),"/",false);
					setcookie("readme_latino",'Tu Crees que iba a poner informacion delicada en estas cookies sin antes cifrarlas jjajajajajajaj',time()+(60*60*8),"/",false);
					setcookie("readme_english",'this encrypted information is not useful for you are a dumb guy jjajajajajajaj',time()+(60*60*24),"/",false);
				...
				if($remember=="SI"){
					...
					setcookie("readme_latino",'Tu Crees que iba a poner informacion delicada en estas cookies sin antes cifrarlas jjajajajajajaj',time()+(60*60*8),"/",false);
					setcookie("readme_english",'this encrypted information is not useful for you are a dumb guy jjajajajajajaj',time()+(60*60*24),"/",false);
					setcookie("UserManager",'UserAdmin',time()+(60*60*8),"/",false); 
				}else{
					$cadena_encriptada_id = encrypt_ab_check($id);
					$cadena_encriptada_id_falsa = encrypt_ab_check(rand());
					setcookie("IG2A7DOFSI",$cadena_encriptada_id,time()+(60*60*24*650),"/",false);
